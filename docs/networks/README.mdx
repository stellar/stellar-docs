...
Each Stellar network also has a network ID, which is the SHA-256 hash of the network passphrase. The network ID is used in transaction signing, contract address generation, and other things and ensure that hashes and ids generated are different on each network.
