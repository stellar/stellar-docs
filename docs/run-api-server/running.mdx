...
To run on Kubernetes using helm chart, ensure you have followed the [pre-requisite](./installing.mdx#helm-chart-installation) of installing the Helm cli tool and adding the Stellar chart repo to helm client, now deploy horizon using the chart to the cluster, showing usage of `--devel` which will use the latest development version of published chart:
