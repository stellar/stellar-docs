...
Stellar stores and communicates transaction data in a binary format called [XDR](../encyclopedia/xdr.mdx), which is optimized for network performance but unreadable to the human eye. Luckily, [Horizon](https://developers.stellar.org/api/introduction/), the Stellar API, and the [Stellar SDKs](../tools-and-sdks.mdx#sdk-library) convert XDRs into friendlier formats. Here’s how you might send 10 lumens to an account:
