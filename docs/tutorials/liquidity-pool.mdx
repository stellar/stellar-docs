...
<head>
  ...
  <link
    rel="canonical"
    href="https://soroban.stellar.org/docs/tutorials/liquidity-pool"
  />
</head>
...
- **`token_wasm_hash`:** The contract will end up [creating its own `POOL` token]
  as well as [interacting with contracts for `token_a` and `token_b`].
...
- **`token_a`** and **`token_b`**: Will be the two "reserve tokens" that users
  ...
  really care, as long as the functions it needs from the common [Token Interface]
  are available in the token contract.
...
the `POOL` token. This means it follows all the conventions of the [Token Interface],
and can be treated just like any other token. They could be
