...
Previously, we took a look on starting withdrawal flow. Now, let's take a look at full example working with the withdrawal.
...
Next, open interactive url as an iframe:
...
Next, sign and submit Stellar transfer:
...
Finally, let's track transaction status updates. In this example we simply check if transaction has been completed:
