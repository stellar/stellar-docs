...
First, we need our application to know how it can communicate with anchors to get asset and infrastructure information. Do this by first querying an anchor’s stellar.toml file.
...
The first thing we'll do is determine whether the user holds trustlines to any assets that have a `home_domain` field set on the Stellar network. If so, we'll display some interactive elements to the user for that asset/domain.
...
Now we know if there are any assets the user holds that may have the necessary infrastructure for authenticated transfers and we can query the domain in question for any relevant details.
