...
In our `*.svelte` component files, we will not dive too deeply into the HTML markup outside of the `<script>` tags. The Svelte syntax is primarily used for iterating and is quite understandable.
