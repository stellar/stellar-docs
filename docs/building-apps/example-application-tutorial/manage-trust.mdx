...
The `/dashboard/assets` page will allow the user to manage the Stellar assets their account carries trustlines to. On this page, they can select from several pre-suggested or highly ranked assets, or they could specify their own asset to trust using an asset code and issuer public key. They can also remove trustlines that already exist on their account.
...
{/* TODO: Use the actual link to the section once we have that ready. */}

[`fetchaccountbalances()` function]: https://www.example.com
...
```js title="/src/utils/stellar/transactions.js"
