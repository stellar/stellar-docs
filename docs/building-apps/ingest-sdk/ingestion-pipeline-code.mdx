...
Complete code for a small sample of a Stellar network ingestion pipeline using the Stellar Go [Ingestion Sdk](overview.mdx#the-ingestion-sdk-packages) to publish derived data to a remote message broker.
...
```
module github.com/stellar/example-ingestion-pipeline
...
print("Collecting next 10 payents from pipeline ...")
