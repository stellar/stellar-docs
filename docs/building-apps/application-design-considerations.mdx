...
- Mixture of both - with the use of [multisig](https://developers.stellar.org/docs/encyclopedia/signatures-multisig), this option is useful for maintaining non-custodial status while still allowing for account recovery
...
Building an application with [multi-signature](<(https://developers.stellar.org/docs/encyclopedia/signatures-multisig)>) capabilities allows you to have a non-custodial service with account recovery. If the user loses their secret key, they can still sign transactions with other authorized signatures, granted the signature threshold is high enough.
