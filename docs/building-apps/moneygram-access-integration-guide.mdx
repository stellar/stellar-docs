...
import TsInstall from "@site/docs/building-apps/wallet/component/ts/install.mdx";
...
Below is highly simplified code for submitting a payment ("transfer") transaction. It does not use timebounds, handle a transaction’s expiration, or handle 504 status codes (unless you're using the Wallet SDK).
...
// some of the watcher calls, or, we can also fetch the transaction object
// using either getTransactionBy or getTransactionsForAsset as exposed in
// previous step.
onMessage: (transaction) => {
  if (transaction.status === "pending_user_transfer_start") {
