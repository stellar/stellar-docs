...
The token contract contains three kinds of operations that follow the token [interface](../custom/token-interface.mdx#code):
...
[sep-41 token interface]: ../custom/token-interface.mdx
