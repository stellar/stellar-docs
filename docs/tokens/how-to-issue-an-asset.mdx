...
Be careful when working with raw secret keys. If you don't have issuer trustline [clawback](../build/guides/transactions/clawbacks.mdx) enabled, any misstep here could permanently render assets lost. Many users put their first few projects on [testnet](../learn/fundamentals/networks#testnet) or try out [Quests](../learn/interactive/quest.mdx) which provide a low-stakes introductory sandbox.
