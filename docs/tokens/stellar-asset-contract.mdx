...
The `trust` function is useful any time a contract distributes an asset to accounts that may not hold it yet: for example, an airdrop or payout contract calling `trust` before `mint` or `transfer`, or bridging and chain abstraction flows that deliver assets to freshly created accounts. See [setting a custom SAC admin](../build/guides/tokens/custom-sac-admin.mdx) for a worked example of a contract calling `trust` before minting to a recipient.
