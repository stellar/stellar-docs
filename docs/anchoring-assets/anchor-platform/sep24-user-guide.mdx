...
In order to update transaction statuses, the observer makes `PATCH /transactions` requests to the platform, so lets configure the URL is should use.

<CodeExample>

``` bash
# dev.env
PLATFORM_API_BASE_URL=http://platform-server
```
