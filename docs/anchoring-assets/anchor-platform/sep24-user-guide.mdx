...
If your business has an existing on & off ramp product, you likely have an existing system for user authentication. However, because the Anchor Platform authenticates the user prior to providing the business' URL, requiring the user to go through another form of communication is actually unnecessary. In this way, the Anchor Platform can be thought of as providing an alternative form of authentication.

The business is free to continue requiring users to authenticate using their existing system, but the ideal user experience would skip this step and create an authenticated session for the user if they have already authenticated using the Stellar account.
...
1. Pass the `token` added to the URL your backend system
...
Lets add a backend server to our compose file that will be used to verify the token and create authenticated web sessions for users initiating transactions.
...
Lets create a simple Docker container for our application.
...
Now lets create a minimal NodeJS application.
...
Below is an example of a backend server authenticating a user in using NodeJS.
...
* Your business processes a refund for the user's transaction
* Your business experiences an unexpected error
...
In the next major version release of the Anchor Platform, we'll be replacing the `PATCH /transactions` Platform API endpoint with a JSON RPC endpoint that will simplify the request schema, provide better validiation of data, and make the API easier to understand in general.

:::

Lets write endpoint for our business server that accepts the information collected in our UI.
...
app.post("/transaction", async (req, res) => {
  ...
  // assuming this is a withdrawal transaction, we'll provide our distribution account as
  // our destination account, and a memo so we can match the on-chain payment with the
  ...
  let patchTransactionBody = {
    ...
    "destination_account": "GBLSAHONJRODSFTLOV225NZR4LHICH63RIFQTQN37L5CRTR2IMQ5UEK7",
...
This will update the Anchor Platform's database with the information provided and enable wallet applications to fetch this updated information to relay it back to the user. The user should already be aware of the transaction amounts and that your business is waiting for the fiat deposit to arrive, but providing these updates allows wallets to display the current status of all transactions the user has with your business.
...
However, if you use the Anchor Platform to monitor the Stellar network for incoming payments (associated with withdrawal transactions), the Anchor Platform will update transaction statuses when a payments are received.
...
First, lets configure the Anchor Platform to observe the Stellar network for incoming payments.
...
The `--stellar-observer` command starts a process that monitors the distribution accounts configured in your `config.yaml` file as well as those provided as the `destination_account` in `PATCH /transactions` requests for withdrawals.
...
Lets make some additions to the `server.js` file so we can poll the Anchor Platform for our expected payments.
