...
By default, platform API such as `PATCH /transactions` and `GET /transaction` endpoints are not protected, and accessible by anyone who has access to the Anchor Platform.
...
After it's enabled, all requests must contain valid `X-Api-Key` header, set to the configured API key.
...
Before diving into making PATCH requests, let's establish a template for making a request to the Anchor Platform.
...
It will make a `PATCH /transactions` request to the Anchor Platform hosted on the default port (8080). In the examples below, JSON transaction data will be used as body (wrapped into records singleton array).

:::tip

It's possible to provide multiple updates in a single `PATCH /transactions` call (by placing multiple transaction objects in the records array). When update is done in such manner, all updates will be done sequentially.
...
After KYC is being collected, deposit is ready to be fulfilled by user (creating a transfer). Therefore, except status, a couple more fields should be updated.
...
Next, send a transaction in the Stellar network to fulfill user request. Currently, Anchor Platform doesn't offer such functionality. You may use a custodian service, or manage distribution account yourself. In latter case, you may want to use the [Stellar SDK](/docs/tools-and-sdks#sdk-library) on your server side to create, sign and submit transaction.
...
Using Payment Observer allows delegating this step to the Anchor Platform. To enable Payment Observer use `--stellar-observer` flag in the command section of the [compose file](#configuration)
