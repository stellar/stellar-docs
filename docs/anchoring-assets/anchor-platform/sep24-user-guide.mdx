...
Once deposit flow is finished, implementing withdrawal is quite easy. Some parts of the flow are similar, and can be shared.

Starting points for withdrawal and deposit are the same, it's [collecting and verifying KYC](#kyc-being-collectedverified).

#### Ready to receive funds

Similar to deposit, next step would be notifying user that Anchor is ready to receive funds. However, this your service will be receiving transaction over Stellar network, and the update would look a bit different.
...
Setting `memo` and `memo_type` is optional. When status is changed to `pending_user_transfer_start`, Anchor Platform sets `memo` and `memo_type` automatically (only if it's not included in the request). Note that the memo must be unique, this is what helps to associate stellar transactions with SEP transaction.

:::

:::note

Stellar account that will be used to receive funds should be configured in [assets](#specify-your-services-assets)
...
Using Payment Observer allows to delegate this step to Anchor Platform. To enable Payment Observer use `--stellar-observer` flag in the command section of the [compose file](#configuration)

Payment Observer will track all transactions sent to distribution account. When transaction with correct memo is detected in the network, status will automatically change to `pending_anchor` and event will be emitted (if Kafka is used).

:::

:::caution

Payment Observer won't validate amounts, and it's your responsibility to verify amount being sent by the user is correct.

:::

First, wait for transaction to be included in the ledger (using SDK). This transaction must have correct memo and destination address (distribution account). Once this transaction has been detected and verified, notify user that the funds have been received:
...
Next step is sending external transfer, such as bank transfer, to the user. When done, change status to notify user transfer has been sent, but has not been confirmed yet.
