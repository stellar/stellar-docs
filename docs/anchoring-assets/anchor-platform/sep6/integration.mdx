...
SEP-6 does not require the user submit KYC information prior to initiating a transaction. If there are any KYC requirements for a transaction, the required SEP-9 fields can be communicated to the user by making a `request_customer_info_update` JSON-RPC request and providing the required fields names.
...
Sometimes, funds need to be sent back to the user (refund). You can refund the whole sum (full refund) or do a set of partial refunds. Also, if user sent more money than expected, you can refund a part of the sum back to the user and send the rest as onchain funds.
