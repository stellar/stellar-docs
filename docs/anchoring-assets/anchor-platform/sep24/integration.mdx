...
Your business may want to expire transactions that have been abandoned by the user after some time. It's a good practice to clean up transactions in `inactive` status. To do so, simply change the transaction status to `expired`
