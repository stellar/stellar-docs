...
SEP-24 deposit flow diagram defines sequence/rules of transaction's status transition and JSON-RPC methods that should be called to change status. You can't define the status you want to set for a specific transaction in your requests.
Each JSON-RPC method defines a set of data that it expects in request. If request doesn't contain a required attributes, anchor platform will return and error and won't change status/state of the transaction.
...
Information abouts amounts (in/out/fee) is required if want to move transaction from status `incomplete` to `pending_user_transfer_start`.
If transaction status is changed from `pending_anchor` to `pending_user_transfer_start`, you can skip amounts
...
Next, send a transaction on the Stellar network to fulfill a user request.
After transaction completion, its necessary to send `notify_onchain_funds_sent` RPC call to notify user that funds were successfully sent.
...
### Sending Payment Via Custody Provider

The Anchor Platform provides a possibility to send a payment via custody providers, such as Fireblocks.
To make a payment via custody provider it's necessary to make the following RPC call:
...
```
After successful processing of payment on a custody provider, anchor platform will automatically make `notify_onchain_funds_sent` RPC call and status of the transaction will be transferred to `completed`
...
This status has to be set if payment requires asset trustline which wasn't configured by user.
There are two ways how transaction may be moved into `pending_trust` status.
First one is a processing of payment via custody provider in case if it detected that trustline isn't configured.
Second way is when business itself detects that trustline is missing and wants to notify user that it has to be configured.
To move transaction into `pending_trust` status it's necessary to make the following RPC call:
...
Payment via custody provider periodically checks if trustline was configured and if it was, it will automatically send a payment to a custody provider and change status of the transaction to `pending-stellar`
...
Transaction status can be changed from `error/expired` to `pending-anchor`. After recovery, you can refund received assets or proceed processing of transaction.
To recover transaction, it's necessary to make the following RPC call:
...
[
  {
    ...
    "params": {
      "transaction_id": "<transaction_id>",
      "message": "The user has to initiate transfer to the anchor",
      "amount_in": {
        "amount": 10,
        "asset": "stellar:USDC:GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5"
      },
...
### Do Stellar Refund
Integration with custody provider allows you to do refund via custody provider, such as Fireblocks.
...
First, wait for the transaction to be included in the ledger (using an SDK). This transaction must have the correct memo and destination address (distribution account). Once this transaction has been detected and verified, notify the user that the funds have been received using the corresponding [notify_onchain_funds_received](#onchain-funds-received) JSON-RPC call:
