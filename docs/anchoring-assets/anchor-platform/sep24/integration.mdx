...
As part of the flow, once user makes an interactive withdrawal/deposit request, it will be processed by the Anchor Platform and forwarded to your service. Anchor Platform will make a `GET` call to `<configured interactive url>?token=<jwt token>`.

This JWT token will contain:

1. `exp` is the expiration time of the token. You should check that provided token has not expired.
2. `sub` is the account associated with this transaction. It can be used to identify the user account. Note, that this value may be different from account that will be used to receive/send funds.
...
Your business may want to expire transactions that has been abandon by the user after some time. It's a good practice to clean up transactions in `inactive` status. To do so, simply change transaction status to `expired`
