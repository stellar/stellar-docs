...
The Anchor Platform provides a possibility to send a payment via custody providers, such as Fireblocks. To make a payment via custody provider it's necessary to make the following RPC call:
...
This status has to be set if payment requires asset trustline which wasn't configured by user. There are two ways how transaction may be moved into `pending_trust` status. First one is a processing of payment via custody provider in case if it detected that trustline isn't configured. Second way is when business itself detects that trustline is missing and wants to notify user that it has to be configured. To move transaction into `pending_trust` status it's necessary to make the following RPC call:
