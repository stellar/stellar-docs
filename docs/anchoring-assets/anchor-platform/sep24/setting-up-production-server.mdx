...
To switch to Stellar's public (mainnet) network, all you have to do is change the network [passphrase](../../../encyclopedia/network-passphrases.mdx) (for authenticating requests) and [Horizon URL](https://horizon.stellar.org/).

First, you need to change your info file (`dev.stellar.toml`):
...
Next, change your Anchor Platform configuration in `dev.env` file:
...
KYC information should be linked to the session created through [Stellar Web Authentication](../sep10/configuration.mdx) and, consequently, to the user, so you only need to ask the user for it once. After the first KYC flow is complete, a user shouldn't have to input the information again.
...
All SEP-9 data that was sent from the wallet is a part of the [Interactive JWT](./faq#how-to-use-interactive-jwts), send by the Anchor Platform
