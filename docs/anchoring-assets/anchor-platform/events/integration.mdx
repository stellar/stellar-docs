...
First, the event processor needs to be configured in the `event_processor` section of the Anchor Platform Configuration file.
...
Next, the event service's Kafka producer need to be configured using the `event.queue` section of the configuration file. The following is an example of how to configure the event service's Kafka producer:
...
The event service's callback API can be configured using the `callback_api` section of the Anchor Platform configuration file. The following is an example of how to configure the event service's callback API with JWT authentication:
...
The `http_header` is configurable as of version `2.5.2` and earlier. The default value is `Authorization` for JWT and `X-Api-Key` for API key.
