...
To switch to Stellar's public (mainnet) network, all you have to do is change the network [passphrase](../../encyclopedia/network-passphrase.mdx) (for authenticating requests) and [Horizon URL](https://horizon.stellar.org/). Stellar SDKs have all the built-in logic to switch from test to public with minor changes to the codebase.
