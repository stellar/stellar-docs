...
The example mentioned here leverages the [`ledgerbackend`](https://github.com/stellar/go/tree/master/ingest/ledgerbackend) package within the ingest library to fetch ledgers. This example fetches the ledgers for the Test network. You can configure it to run against the Pubnet or your own standalone network

For each ledger,we compute -
...
var ( config = captiveCoreConfig() )

func captiveCoreConfig() ledgerbackend.CaptiveCoreConfig { archiveURLs := network.TestNetworkhistoryArchiveURLs networkPassphrase := network.TestNetworkPassphrase captiveCoreToml, err := ledgerbackend.NewCaptiveCoreToml(ledgerbackend.CaptiveCoreTomlParams{ NetworkPassphrase: networkPassphrase, HistoryArchiveURLs: archiveURLs, }) panicIf(err)
...
func panicIf(err error) { if err != nil { panic(fmt.Errorf("An error occurred, panicking: %s\n", err)) } }

func statistics() { ctx := context.Background() // Only log errors from the backend to keep output cleaner. lg := log.New() lg.SetLevel(logrus.ErrorLevel) config.Log = lg
