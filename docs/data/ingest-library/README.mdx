...
Ledger Metadata is the origin for any ingestion pipeline that wants to derive historical or current state data models from stellar network. The library provides packages which clients can use to consume the metadata in streaming data format, with readers and callback functions.
...
![Ingest Library](/assets/ingest_lib.png) _\* Diagram: Depicts ingest library performing real-time or historical network data acquistion from multiple sources (watcher core node as captive core, history archives, and CDP datastore)._
