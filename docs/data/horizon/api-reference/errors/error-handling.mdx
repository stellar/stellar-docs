...
2. `/transactions`: Horizon submits a transaction to Stellar-Core and then waits for it to be ingested into its database. It will either return a success (200) or failure (400) response, or return a timeout response (504), after which it is the client's responsibility to poll for the status of the transaction.
...
- Transaction fees are also a safe adjustment. If you get a `tx_insufficient_fee` error, you can modify the fees using a [fee-bump transaction](docs/learn/encyclopedia/transactions-specialized/fee-bump-transactions). Refer to the [Insufficient Fees and Surge Pricing](/docs/data/horizon/api-reference/errors/error-handling#insufficient-fees-and-surge-pricing) section later in this document for more information on managing fees and strategies around it.
