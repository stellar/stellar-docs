...
If you are running locally on a macOS for instance, you could create a volume in your home directory: `~/test-rpc-config`

Then you would add the following config files to that local directory:
- soroban-rpc-config.toml
- stellar-captive-core.cfg

Then you would mount that volume using the following parameter: `-v /Users/<USERNAME>/test-rpc-config:/opt/stellar`.

Your running container would access that volume at the path `/opt/stellar`
...
You can this [Stellar RPC subcommand](https://github.com/stellar/stellar-rpc/blob/main/cmd/stellar-rpc/README.md#configuring-and-running-rpc-server) to generate a starter configuration file:
