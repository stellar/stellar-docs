...
These track ledger entry changes, meaning they store how each ledger entry evolved over time instead of just the final state of the ledger entry.
...
Note that this is different than what the Horizon endpoint would return
