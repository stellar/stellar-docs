...
- **[Galexie](#galexie)** - CLI tool, export raw ledger metadata files
- **[Ingest Library](#ingest-library)** - SDK for acquiring and parsing network data

| Features                | RPC | Hubble | Horizon | Galexie | Ingest Library |
| ----------------------- | --- | ------ | ------- | ------- | -------------- |
| Real-time Data          | ✅  | ❌     | ✅      | ✅      | ✅             |
| Historical Data         | ❌  | ✅     | ❌\*    | ✅      | ✅             |
| Smart Contracts         | ✅  | ✅     | ❌      | ✅      | ✅             |
...
\*_Please note that only a self-hosted instance of Horizon with retention count set to unlimited can provide full historical data but is not the recommended tool for full historical data access._
...
The library is a Go SDK which provides packages for acquiring and parsing real-time and historical data from the Stellar network. It provides language level bindings which convert the binary XDR streams from the network into fluent, programmatic data models. Applications can leverage this SDK to quickly obtain and traverse network data, such as: history archive buckets, ledgers, transactions, operations, ledger state changes, and events. Providing an intuitive, compile-time, type-safe developer experience.
