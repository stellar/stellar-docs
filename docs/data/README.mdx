...
- **[Ingest Library](#ingest-library)** - SDK for parsing network data
...
The library is a Go SDK which provides packages for efficiently acquiring and parsing real-time and historical data from the Stellar network. It provides language level bindings which convert the binary XDR streams from the network into programmatic data models. Applications can leverage this SDK to quickly obtain and traverse the object heirarchy of history archive buckets, ledgers, transactions, operations, ledger state changes, and events, providing an intuitive, compile-time, type-safe developer experience.
