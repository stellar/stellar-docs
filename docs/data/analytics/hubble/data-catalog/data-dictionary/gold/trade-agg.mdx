...
| --- | --- |
| Natural Key(s) | day_agg, asset_a, asset_b |
...
| airflow_start_ts | The timestamp when the Airflow job starts. | STRING |  | No | The Airflow job writes data to the BigQuery tables. Can be used to tell whether data was added as a backfill; e.g. when `day_agg` is old but `airflow_start_ts` is recent. |
