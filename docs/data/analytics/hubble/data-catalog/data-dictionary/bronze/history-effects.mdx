...
| details.authorized_to_maintain_liabilites | Auth value for set trustline flags. | boolean |  |  | Spelled `liabilites` (missing an `i`) in the warehouse; the misspelling is part of the column name and must be reproduced in queries. |
...
| details.seller_muxed_id | Account multiplexed id | string |  |  |  |
