...
| details | Structured, sparse record holding the operation-specific fields for each operation type. The populated subset depends on `type`. | record |  | No | Individual fields are documented as the `details.*` rows below. The same payload is also available as a native BigQuery `JSON` value in the `details_json` column, which is easier to query when the operation type is not known ahead of time. |
...
| details.account_muxed_id | Integer representation of the virtual address of the account if the account is multiplexed | string |  | No |  |
...
| details.claimant_muxed_id | If the account is multiplexed, an integer representation of the muxed account which claimed the balance | string |  | No |  |
...
| details.from_muxed_id | If the account is multiplexed, the integer representation of the virtual address of the sender account | string |  | No |  |
...
| details.funder_muxed_id | If the account is multiplexed, the integer representation of the virtual address of the funding account | string |  | No |  |
...
| details.into_muxed_id | If the account is multiplexed, the integer representation of the account receiving the deleted account's lumens | string |  | No |  |
...
| details.price | The ratio of selling asset to buying asset. This is a number representing how many units of a selling asset it takes to get 1 unit of a buying asset | float |  | No |  |
...
| details.to_muxed_id | If the account is multiplexed, the integer representation of the virtual address of the recipient account | string |  | No |  |
...
| details.trustee_muxed_id | If the issuing account address is multiplexed, the integer representation of the virtual address | string |  | No |  |
...
| details.trustor_muxed_id | If the trusting account is multiplexed, the integer representation of the virtual address | string |  | No |  |
...
| details.begin_sponsor_muxed_id | If the initiating sponsorship account is multiplexed, the integer representation of the virtual address | string |  | No |  |
...
| details.asset_balance_changes.amount | The amount of token minted, transferred or burned using the SAC contract | float |  | No |  |
...
| details_json | Native BigQuery `JSON` value holding the same operation-specific details as the `details` record. Each operation returns only its own relevant keys. | json |  |  |  |
