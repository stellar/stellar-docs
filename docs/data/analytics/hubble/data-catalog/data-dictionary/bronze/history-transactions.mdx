...
| tx_signers | The public keys of the signers who authorized the transaction. This field lists all the signatories that validated and approved the transaction, ensuring it meets the required authorization thresholds | array[string] |  |  |  |
| soroban_resources_disk_read_bytes | The number of bytes the Soroban transaction read from disk. | integer |  | No | Protocol 23 ([CAP-66](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0066.md)) successor to `soroban_resources_read_bytes`, which now covers in-memory state only. |
| soroban_resources_archived_entries | The indexes of the footprint entries that were archived and restored as part of this transaction. | array[integer] |  | No | Protocol 23 automatic restore. Repeated field. |
