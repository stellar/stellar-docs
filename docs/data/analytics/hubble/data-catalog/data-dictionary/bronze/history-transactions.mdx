...
| soroban_resources_disk_read_bytes | The number of bytes the Soroban transaction read from disk. | integer |  | No | Protocol 23 (CAP-62/63) successor to `soroban_resources_read_bytes`, which now covers in-memory state only. |
