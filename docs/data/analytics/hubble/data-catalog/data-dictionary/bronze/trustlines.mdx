...
| asset_type | The identifier for type of asset code held by the trustline: an alphanumeric code of 4 or 12 characters, or a liquidity pool share. | string | <ul><li>credit_alphanum4</li><li>credit_alphanum12</li><li>pool_share</li></ul> | Yes | Trustlines never report `native`. XLM is held directly on the account and is reported in the `accounts` table, not here. `pool_share` identifies a trustline holding shares in a liquidity pool. |
...
| liquidity_pool_id_strkey | The StrKey (`L...`) representation of the liquidity pool id. | string |  | No | Same pool as `liquidity_pool_id`, StrKey-encoded rather than hex. StrKey encoding is defined by [SEP-23](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0023.md). |
