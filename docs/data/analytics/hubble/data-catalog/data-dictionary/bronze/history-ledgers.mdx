...
| evicted_ledger_keys_type | The list of type of keys which have been evicted in a given ledger entry. | array[string] |  | No |  |
| evicted_ledger_keys_hash | The list of keys hash which have been evicted in a given ledger entry. | array[string] |  | No |  |
| total_byte_size_of_live_soroban_state | The total size in bytes of the live Soroban state as of this ledger. | integer |  | No | Added in Protocol 23 alongside `total_byte_size_of_bucket_list`, which is retained. The two are currently identical on pubnet, but they are not the same measure: this column tracks the live Soroban state that Soroban rent fees are calculated against, while `total_byte_size_of_bucket_list` measures the full BucketList. |
