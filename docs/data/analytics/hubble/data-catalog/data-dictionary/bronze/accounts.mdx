...
| num_subentries | The account's subentry count, a weighted total rather than a row count. Subentries are the account's trustlines, offers, signers, and data entries (claimable balances are counted under sponsoring entries, not subentries). Each offer, signer, data entry, or ordinary trustline adds 1. A pool share trustline adds 2 because it holds two sides, so it requires two base reserves (see [CAP-38](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0038.md)). | integer |  | Yes | Each entry on a ledger takes up space, which is expensive to store on the blockchain. For each entry, an account is required to hold a [minimum XLM balance](../../../../../../learn/glossary.mdx#minimum-balance). The reserve is calculated by (2 + num_subentries + num_sponsoring - num_sponsored) \* base_reserve, currently 0.5 XLM |
...
| master_weight | The weight of the master key, which is the private key for this account. If a master key is `0,` the account is locked and cannot be used. | integer | Integers from 0 to 255 | Yes |  |
