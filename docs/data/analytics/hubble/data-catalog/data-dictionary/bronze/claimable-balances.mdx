...
| balance_id_strkey | The StrKey (`B...`) representation of the claimable balance id. | string |  | No | StrKey encoding for claimable balance ids is defined by [SEP-23](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0023.md). Prefer this over the raw hex `balance_id` when joining to Horizon or RPC responses. |
