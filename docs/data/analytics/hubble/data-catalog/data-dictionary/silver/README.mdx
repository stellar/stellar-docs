...
Some silver tables are modeled as [SCD Type 2](https://github.com/stellar/stellar-dbt-public/blob/master/docs/snapshot.md) tables.
This means they track the full history of changes to a record over time by keeping multiple versions with `valid_from` / `valid_to` ranges.
...
Most of the time, the data is correct and captures the day-to-day changes accurately.
However, there can be cases where not all inter-day changes were captured, usually due to backfill issues or missing snapshot runs.

3. Usage Considerations:
- Use valid_from / valid_to ranges to reconstruct historical states.
- Use clustering fields to improve query performance.
