...
| num_subentries | The account's subentry count, a weighted total rather than a row count. Subentries are the account's trustlines, offers, signers, and data entries. Each offer, signer, data entry, or ordinary trustline adds 1. A pool share trustline adds 2 and therefore requires two base reserves (see [CAP-38](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0038.md)). | INTEGER |  | Yes | Each entry on a ledger takes up space, which is expensive to store. Minimum balance is calculated by (2 + num_subentries + num_sponsoring - num_sponsored) \* base_reserve, currently 0.5 XLM. |
