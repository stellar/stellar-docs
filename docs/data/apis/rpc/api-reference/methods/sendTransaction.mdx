---
hide_title: true
description: Submits a signed transaction to the network and returns its submission status.
---
...
Unlike Horizon, this method **does not wait for confirmation**. Instead, it **validates and enqueues** the transaction. To track its final outcome, clients should follow up with a call to [`getTransaction`](../methods/getTransaction.mdx).
