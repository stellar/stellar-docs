...
Use the `sendTransaction` method to **submit a transaction to the Stellar network**. This is the only way to make **on-chain changes** using RPC.

Unlike Horizon, this method **does not wait for transaction completion**—it simply validates and enqueues the transaction for processing. To check whether a transaction was successfully included in a ledger, you should call [`getTransaction`](#view-transaction-details-on-the-lab) afterward.

This method supports **all types of transactions**, not just those related to smart contracts.
...
![Lab: sendTransaction](/assets/api/rpc/sendtx.gif)
