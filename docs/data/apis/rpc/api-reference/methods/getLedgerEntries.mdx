...
You can query live ledger state on both **Testnet** and **Mainnet** using the `getLedgerEntries` RPC method directly in the [Stellar Laboratory](https://laboratory.stellar.org/). You can either use the XDR or directly update the input using the inputs.
