...
You have two options for utilizing a data lake: use a public service or host your own.
...
3.  **Configure Retention Window:** The `HISTORY_RETENTION_WINDOW` must be a non-zero value, even though data is fetched from the datastore. A minimum value of `1` is sufficient.

#### Configuration Examples

Below are examples for configuring GCS and S3 backends. You will need to add the `[datastore_config]` and `SERVE_LEDGERS_FROM_DATASTORE` parameters to your existing RPC configuration file.
