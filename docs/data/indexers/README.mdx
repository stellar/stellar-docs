...
Additionally, even some information that is currently available from [Horizon] will also require indexing services in the future, as Horizon has been deprecated but is still currently the only practical way to obtain some data.
...
- [Alchemy]: most popular choice on Ethereum; now under contract to expand their service to Stellar, with a launch date of Q2 2026. (TODO: is this accurate?)
- [Allium]: currently building Stellar support, launching Q1 2026 (accurate?)
- [Goldsky]: mostly focused on indexer-use-case-#2 (see below), but also under contract to provide an API service specifically for Stellar. Planned launch: Q1 2026. (accurate?)
- [OBSRVR]: a home-grown Stellar-native offering, providing both RPC services and [Obsrvr Gateway](https://www.withobsrvr.com/products/gateway), promising "powerful APIs for real-time data fetching, transaction processing, and easy integration into existing systems." The current maturity of this offering is hard to determine.
- [Horizon]: as mentioned above, Horizon has been deprecated, but for now it is the only way to get some of this data. And it's free! And well-known by the Stellar ecosystem.
...
- [Goldsky]: one of the currently-most-loved options on Ethereum for this use-case. Goldsky provides [two main products](https://docs.goldsky.com/subgraph-vs-mirror):
  ...
  2. **Mirror**, also called **Pipelines**: a highly efficient tool to Extract, Transform, & Load (ETL) data into your own database. Goldsky [already supports Stellar](https://docs.goldsky.com/chains/stellar), and as of Q4 2025 is working closely with SDF to make their core Stellar datasets more accurate and useful.
...
- [SubQuery]: Decentralized Indexer SDK, Decentralized RPCs, & AI Apps. Supports 300+ chains; claims to be 3.9x faster than The Graph. The most direct and aggressive challenger to The Graph's decentralized model. Differentiates on speed and multi-chain support. Current Stellar support is so-so, with some early adopters leaving for Goldsky due to unoptimized RPC calls running up large bills with RPC providers.

- [OnFinality]: a big player in the Polkadot ecosystem, now expanding to other blockchains. OnFinality provides data _hosting_ services for your SubQuery data. SubQuery powers your Extract, Transform, and Load (ETL) pipeline; OnFinality is where the data can be loaded _to_.

- [Allium]: in addition to their Portfolio APIs offering, Allium is also under contract with SDF to build out tools for both Indexing Use Case #2 (this one) and Indexing Use Case #3 (see below), with target launch date of Q1 2026. (TODO: accurate?)

- [Space and Time]: one challenge with most indexing approaches is the reintroduction of trusted 3rd parties into what is otherwise a verifiable, trustless software stack. Space and Time aims to fix this with "Proof of SQL", using Zero-Knowledge proofs to offer tamper-proof computation for enterprises and dapps. Space and Time's Stellar offering is under development, with target launch in Q2 2026. (TODO: accurate?)
...
[Horizon]: ../../learn/fundamentals/stellar-stack.mdx#horizon
