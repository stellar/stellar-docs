...
The token transfer processor is a [package](https://github.com/stellar/go/tree/ttp-v1.0.0/ingest/processors/token_transfer) in the [ingest-sdk](../ingest-sdk/README.mdx) that tracks transfers, mints, burns, clawbacks, and fees for assets/tokens on the Stellar network.

The token_transfer package is responsible for processing and generating standardized token transfer events from Stellar ledger operations. It supports both classic Stellar operations as well as Soroban contract events, providing a unified view of all token movements within the network.

The processor parses ledger operations and their results to create structured events that follow the [CAP-67](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0067.md) Unified Events specification.
