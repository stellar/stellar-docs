...
:::note Means of creating new tokens
Distributor accounts are part of a controlled design pattern centered on issuing assets through a transfer, contract, or claimable balance. While basic transfers are standard practice, functionally, you can create a market directly from the issuing account and issue via trading.
:::

:::info Regulated assets only
someting etc conclusion intro flag
:::
...
payment ot the distribution account, acutally issuing the asset
...
can configure exact number of tokens in circulation in alignment with stellar info file details and any lockups eisting in [contracts]() or existing [claimable balances]()
e.h. for original founder airdrops or such - but onyl one time
lock issuer account down without setting an admin in the Stellar Asset Contract so the asset’s supply is permanently fixed
