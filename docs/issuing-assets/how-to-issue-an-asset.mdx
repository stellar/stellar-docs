...
First, you must generate a unique keypair. The public key will act as your [issuing identity](../learn/fundamentals/stellar-data-structures/assets#issuer) on the network, while you use the secret key to sign transactions.
...
Be careful when working with raw secret keys. If you don't have issuer trustline [clawback](../learn/encyclopedia/clawbacks) enabled, any misstep here could permanently render assets lost. Many users put their first few projects on [testnet](../learn/fundamentals/networks#testnet) or try out [Quests](../learn/interactive/quest.mdx) which provide a low-stakes introductory sandbox.
...
While anyone can create an asset, there may be real-world [compliance](../issuing-assets/anatomy-of-an-asset#compliance) implications relevant to your use case.
