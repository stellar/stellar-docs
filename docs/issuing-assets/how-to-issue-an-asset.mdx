...
First, you must generate a unique keypair. This will act as your [issuing identity](../learn/fundamentals/stellar-data-structures/assets#issuer) on the network. The public key will be used to identify the issuing account on the Stellar network, while the secret key will be used to sign transactions.
...
Your account address will not change once you issue your asset, even if you modify its [signers](../learn/encyclopedia/signatures-multisig). Many issuers employ [vanity public keys](https://github.com/JFWooten4/py-stellar-vanity-toolkit) related to their asset. For instance, you might configure a custom signer in [base32](../learn/glossary.mdx#account-id) like `GASTRO...USD`.
...
todo: Means of creating new tokens Distributor accounts are part of a controlled design pattern centered on issuing assets through a transfer, contract, or claimable balance. While basic transfers are standard practice, functionally, you can create a market directly from the issuing account and issue via trading.
...
The asset object is a combination of your [code](./control-asset-access.mdx#naming-an-asset) and your issuing public key. Once you create an initial issuance on the network, anyone can search for your unique asset.
...
todo: Regulated assets only someting etc conclusion intro flag
...
The payment operation is the most popular tool to actually issue (or mint) your asset. It creates the amount of an asset specified, up to [the maximum 64-bit integer](../learn/fundamentals/stellar-data-structures/assets.mdx#amount-precision). Relevantly, you do not need to scale up the issuing amount of your asset by the XDR minimum increment.
