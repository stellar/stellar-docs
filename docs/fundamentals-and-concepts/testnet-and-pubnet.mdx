...
- A testing infrastructure that requires more control over the test environment, such as:
  ...
  - The need to secure private or sensitive data (before launching on the pubnet)
You can always run your own test network for use cases that don’t work well with SDF’s testnet.
