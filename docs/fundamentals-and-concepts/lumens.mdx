...
Lumens (XLM) are the native currency of the Stellar network. The lumen is the only token that doesn’t require an issuer or trustline, and it pays all transaction fees, rent, and covers minimum balances on the network.
...
Rent is equivalent to a base reserve for smart contract data. Every smart contract entry must pay rent to exist on the ledger based on how long the entry should live on the ledger before expiration and how big the entry is. There are different rent requirements for each storage type `Persistent`, `Temporary`, and `Instance`, which you can read about in the [Fees and Metering section](https://soroban.stellar.org/docs/fundamentals-and-concepts/fees-and-metering#refundable-resources) of the Soroban docs.
