...
An asset’s identifying code. There are three different formats: Alphanumeric 4, Alphanumeric 12, and liquidity pool shares. Learn about liquidity pool shares in the [Liquidity Pool Encyclopedia Entry](../../encyclopedia/liquidity-on-stellar-sdex-liquidity-pools).
...
The smallest non-zero amount unit, also known as a stroop, is 0.0000001 (one ten-millionth) represented as an integer value of one. The largest amount unit possible is (2<sup>63</sup>-1) ÷ 10<sup>7</sup> (derived from the maximum 64-bit integer, scaled down) which is 922,337,203,685.4775807.
