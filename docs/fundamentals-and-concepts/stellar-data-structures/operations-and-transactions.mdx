...
### Transactions
In the Stellar network, transactions are encoded using a standardized protocol called External Data Representation (XDR). You can read more about this in our [XDR Encyclopedia Entry](#).
...
To learn more about how transactions work, see our [Transaction Lifecycle Encyclopedia Entry](#).
...
Once Stellar Core has determined that a transaction is valid, it will propagate the transaction to all other servers to which it’s connected. In this way, a valid transaction is flooded to the entire Stellar network.
