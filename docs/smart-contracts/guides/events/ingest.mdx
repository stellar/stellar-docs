...
, using [Prisma](https://www.prisma.io) as a database abstraction layer. By using Prisma here, it should be relatively trivial to scale this approach up to any other database software of your choosing.
...
First, we'll need to query the events from Soroban RPC. This simple makes an RPC request using the `getEvents` method, filtering for all `transfer` events that are emitted by the native XLM contract.
...
with orm.Session(engine) as session:
    stmt = sqlalchemy.select(Event.ledger).where(Event.contract_id == contract_id)
    ledger = session.scalar(stmt).last()
...
for event in res.events:
    topic_ = event.topic
    value = event.value
    events_.append(models.Event(contract_id=contract_id, ledger=latest_ledger, topics=topic_, value=value))
