...
interface SimulateTransactionResult {
  ...
  restorePreamble?: { // There is a restorePreamble if a ledgerkey or contract code is expired, and you can use the transactionData to build a new transaction.
...
async function simulateTransaction(transactionXDR) {
  ...
  const response = await fetch('https://soroban-testnet.stellar.org/simulateTransaction', {
    method: 'POST',
    headers: {
