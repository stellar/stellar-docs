---
title: Guide to Deploying a Contract from Installed Wasm Bytecode Using Code
...
<head>
  ...
  <meta
    property="og:title"
    content="Guide to Deploying a Contract from Installed Wasm Bytecode Using Code"
  />
  <meta
...
This guide walks through the process of deploying a smart contract from installed wasm bytecode using code . We will cover setting up your environment, uploading Wasm bytecode, and deploying and initializing a contract atomically.

### Prerequisites:

- Basic understanding of Rust programming language.
- Familiarity with Soraban smart contracts.
- Installed Cargo, Stellar CLI and Soroban SDK.

### Setup Environment

The [deployer example] demonstrates how to deploy contracts using a contract. [deployer example]: https://github.com/stellar/soroban-examples/tree/v20.0.0/deployer
...
**NOTE**: for this example , navigate to `deployer/contracts` and `deployer/deployer` and run the command `stellar contract build` to generate the target files.
...
- `env: Env`: The Env object represents the current contract execution environment. It provides methods to interact with the blockchain, other contracts, and perform various operations.
...
- `wasm_hash: BytesN<32>`: The hash of the Wasm bytecode of the contract to be deployed. This hash is used to uniquely identify the contract's code.
...
- `init_args: Vec<Val>`: A vector of arguments to be passed to the initialization function of the deployed contract.
...
- The function checks if the `deployer` address is the same as the current contract’s address.
- If it is not, it requires authorization from the deployer address to ensure that the deployer has permitted this deployment.
...
```sh
stellar contract install --source-account 0xkodesage  --wasm  contract/target/wasm32-unknown-unknown/release/soroban_deployer_test_contract.wasm --rpc-url https://soroban-testnet.stellar.org:443 --network-passphrase "Test SDF Network ; September 2015"   --network testnet
```

- When you deploy a smart contract to a network, you need to specify an identity that will be used to sign the transactions. change the `0xkodesage` to your own [identity]

[identity]: ../../../smart-contracts/getting-started/setup#configure-an-identity
...
stellar contract deploy \
  --wasm deployer/target/wasm32-unknown-unknown/release/soroban_deployer_contract.wasm  --source 0xkodesage  \
  --network testnet
```

This will return the deployer address: `CDKYZMA3OXR54YAHOQ5D4EWDFDT3ZNKKRYKQT7MGPWH22ZVD2DX2BJID`.
...
- replace `0xkodesage` with your own identity

it returns a Contract identifier `CCTVFX6BFTQHTGAHA5TY4YZQJRUKRE2RRNUTGVBNKE3PJF5C7CI53APY` of the deployed test contract.

Invoke the deployed test contract using the identifier returned from the previous command.
