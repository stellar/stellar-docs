...
The `ContractSpec` class from the Stellar SDK provides a powerful way to handle and interact with the smart contract specifications. Here are some of the key functionalities:

1. **Convert Native Values to ScVals**: 
   The `funcArgsToScVals` method converts native JavaScript arguments to `ScVal` objects required for calling the contract functions.
...
2. **Convert ScVals to Native Values**: 
   The `funcResToNative` method converts the result `ScVal` from a function call to a native JavaScript value.
...
3. **Get Function Specifications**:
   You can retrieve the function specifications defined in the contract.
...
4. **Handle Error Cases**: 
   The `errorCases` method retrieves the error cases defined in the contract, which can be used to handle different error scenarios.
...
5. **Convert to JSON Schema**: 
   The `jsonSchema` method converts the contract specification to a JSON schema, which can be useful for documentation or generating client code.
...
Use the Soroban CLI to generate the TypeScript bindings which include the contract specification.
...
Inside the generated bindings directory (`testbindings/src/index.ts`), you'll find the contract specification. Look for a line that initializes a new `ContractSpec` object, such as:
...
const contractAddress = "CBEUVPRZKMCX5MPUAF2JMKMRIFEXXJXQXBLUUW6LMN5GDDQQSX3UHYXA";
...
const adminAddress = Address.fromString("GB4UG5H5MCYPIH7GM7CBZBLOOIE2K7HMDMPK4TPZ5UEIZ2HOI5RL535V");
...
export const networks = {
  testnet: {
    ...
    contractId: "CBEUVPRZKMCX5MPUAF2JMKMRIFEXXJXQXBLUUW6LMN
...
```typescript
import { SorobanRpc, TransactionBuilder, TimeoutInfinite, xdr, Transaction } from '@stellar/stellar-sdk';
