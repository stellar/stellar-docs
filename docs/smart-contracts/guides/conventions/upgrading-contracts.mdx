...
- `new_wasm_hash: BytesN<32>`: The hash of the new WASM code for the contract.
...
stellar contract deploy \
  --wasm target/wasm32-unknown-unknown/release/soroban_upgradeable_contract_old_contract.wasm  --source 0xkodesage  \
  --network testnet
```
...
Navigate to to `upgradeable_contract/new_contract` and run this following command

```
stellar contract install --source-account 0xkodesage  --wasm target/wasm32-unknown-unknown/release/soroban_upgradeable_contract_new_contract.wasm  --rpc-url https://soroban-testnet.stellar.org:443 --network-passphrase "Test SDF Network ; September 2015"   --network testnet
```
...
You also need to call the `init` method so the `admin` is set. This requires us to setup some [identities]. [identities]: ../../../smart-contracts/getting-started/setup#configure-an-identity

Given that I have previously setup my own identity, to get the identity address, we run the following command

```sh
stellar keys address 0xkodesage
```
...
- `CAS6FKBXGVXFGU2SPPPJJOIULJNPMPR6NVKWLOQP24SZJPMB76TGH7Y3` is the contract ID we got when we deployed the `old_contract`
