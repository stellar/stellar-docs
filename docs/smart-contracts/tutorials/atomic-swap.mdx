...
<head>
  ...
  <link
    rel="canonical"
    href="https://soroban.stellar.org/docs/tutorials/atomic-swap"
  />
</head>

The [atomic swap example] swaps two tokens between two authorized parties atomically while following the limits they set.

This is example demonstrates advanced usage of Soroban auth framework and assumes the reader is familiar with the [auth example](../tutorials/auth.mdx) and with Soroban token usage.
