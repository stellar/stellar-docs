...
- You have identified the [Installation](./installing.mdx) method for host system.
  ...
  For bare metal, you have the two executables: `stellar-horizon` and `stellar-core` both on host OS' `$PATH`.

  For running image, using the [stellar/stellar-horizon](https://hub.docker.com/r/stellar/stellar-horizon) direct on `docker run`, you have pulled the stellar/stellar-horizon image via `docker pull stellar/stellar-horizon#<tag>` onto host.
