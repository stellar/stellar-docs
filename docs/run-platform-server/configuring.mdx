...
- [Installation](./installing.mdx) is completed on host machine, it should have two executables, `stellar-horizon` and `stellar-core`. Per the installation instructions, `stellar-core` should be included in your host o/s PATH as these configurations will result in Horizon searching for `stellar-core` to execute.
...
- **serving requests** a regular web-based HTTP API,
...
You choose from one of two following deployment modes to perform the roles and proceed with configuration parameters to that means. A note on configuration parameters, the same parameter can be provided either as command line argument or an environment variable.
...
You can run none or multiple instances to serve read only API requests. **TODO - create --tx-sub-enabled flag defaulted to false, then don't specify it here.**
...
You can run none or multiple instances to serve transaction submission requests. If you run an instance with transaction submission enabled, yuur deployment is required to perfrom the ingestion role on the same datbase referenced here. Horizon transaction submission depends on ingestion to confirm tx submission status. **TODO - Confirm Horizon can use captive core to send tx sub requests and skip stellar-core-url?**
...
| `--network` | `NETWORK` | pubnet |
| `--log-level` | `LOG_LEVEL` | info |
| `--stellar-core-url` | http://localhost:11626 | info |
...
### If you have congigured your deployment to perform the ingestion role, then it is strongly recommended to review [Ingestion](./ingestion.mdx) next and update your configuration accordingly to achieve best performance before proceeding further.
