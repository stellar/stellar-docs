...
- temporal limits, we recommend limiting historical retention of ingested data to a sliding window of 1 month(`HISTORY_RETENTION_COUNT=518400`) and this is the [default set by Horizon](./configuring.mdx#ingestion-role-instance). This is based on metrics and trend analysis of our own API servers, we have seen most requests are for near term data.
