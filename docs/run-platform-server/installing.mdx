...
Edit the charts/horizon/values.yml and set the `image:horizon:tag` to a tag from the [stellar/stellar-horizon dockerhub repo](https://hub.docker.com/r/stellar/stellar-horizon) for the image version you want to run. Review the other settings as well to confirm the network config.
