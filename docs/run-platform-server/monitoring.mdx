...
- `go_`: golang specfici runtime performance
- `horizon_txsub_`: attributes of horizon transaction submission sub system if enabled.
- `horizon_stellar_core_`: runtime attributes of stellar network reported by the captive core or a remote stellar core which horizon is configured.
