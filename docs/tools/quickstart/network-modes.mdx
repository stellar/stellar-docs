...
- `--enable-core-manual-close` to turn off automatic ledger closing, so ledgers advance only when you trigger them yourself (defaults to `false`). See [Operation Modes](./advanced-usage/operation-modes.mdx#manual-close-mode).
