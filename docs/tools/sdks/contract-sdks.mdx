...
The Stellar Router SDK is a lightweight smart contract that enables developers to execute multiple Soroban contract calls within a single transaction. While Soroban normally allows only one contract invocation per transaction, the Router bundles multiple calls through a single entry point, effectively enabling complex, multi-step workflows to be executed atomically.
