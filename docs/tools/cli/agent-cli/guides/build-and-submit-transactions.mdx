...
1. Build, on a machine with network access:
   ...
   stellar tx new payment --source <SOURCE> --destination <ADDRESS> --amount <AMOUNT> \
     --network <NETWORK> --build-only > unsigned.xdr
   ```

   This writes unsigned base64 XDR to `unsigned.xdr` and exits. It contacts RPC to read the source account's sequence number, but nothing is signed and nothing is submitted.
...
2. Sign it. This step needs no RPC connection, so it is the one stage you can run offline. Always pass `--network` or `--network-passphrase` on this stage:
   ...
   ```bash
   stellar tx sign --sign-with-key <SOURCE> --network <NETWORK> < unsigned.xdr > signed.xdr
   ```
