...
Plugins extend the functionality of the Stellar CLI by adding custom commands. The Stellar CLI automatically detects and loads any executable in your `PATH` that starts with `stellar-`. All you need is an executable (it doesn't have to be a binary) in your PATH that starts with `stellar-`.
