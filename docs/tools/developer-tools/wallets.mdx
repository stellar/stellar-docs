---
title: Wallet Integration
description: Explore a suite of developer tools for Stellar wallets.
sidebar_label: Wallet Integration
sidebar_position: 130
---

# Wallet Integration

### [Stellar Wallet Kit](https://github.com/Creit-Tech/Stellar-Wallets-Kit/)

A simple-to-use wallet kit to manage integration to multiple Stellar ecosystem wallet. Learn more about how to integrate this library from [the Stellar Wallet Kit Docs](https://stellarwalletskit.dev/).

![Stellar Wallet Kit](/assets/dev-tools/stellar-wallet-kit.gif)

The Stellar Wallet Kit supported many wallets including:

- Albedo
- Freighter
- Hana
- Ledger Hardware Wallet
- Trezor Handware Wallet
- Lobstr
- Rabet
- WalletConnect
- xBull
- HOT Wallet

### [Account Viewer](https://accountviewer.stellar.org/)

A stripped-down wallet where you can check an account’s XLM balance and send simple payments on Testnet and Mainnet. Account Viewer's [source code](https://github.com/stellar/account-viewer-v2) is also useful and instructive for developers building wallet-related features into their applications.

### [Freighter Wallet](https://www.freighter.app/)

SDF’s flagship non-custodial wallet extension that allows users to sign Stellar transactions via their browser. Freighter wallet's codebase is opensourced here: https://github.com/stellar/freighter.
