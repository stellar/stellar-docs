...
Even if you _do_ want to run your [own instance of Horizon](/platforms/horizon/admin-guide), it bundles its own version of Core and manages its lifetime entirely, so there's no need to run a standalone instance.
