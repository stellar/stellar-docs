...
If the deployment can be done on Kubernetes, there is a [core helm chart](https://github.com/stellar/helm-charts/tree/main/charts/core) available.

If kubernetes is not an option, the manifests in those charts may still be good references for showing how to configure and run the docker container. Just run the helm command with `template` to print the container config to screen:
