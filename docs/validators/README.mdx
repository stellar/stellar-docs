...
1. [**Horizon Nodes**](/network/horizon) can be used for submitting transactions, as well as exposing a REST API service to query and retrieve network state.
2. [**Soroban RPC Nodes**](/network/soroban-rpc) can be used for simulating and/or submitting transactions, as well as exposing an RPC service to query and retrieve current network state.
