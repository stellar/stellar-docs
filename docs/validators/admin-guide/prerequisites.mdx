...
A Stellar Core node needs to allow all IPs to connect to its `PEER_PORT` over TCP. You can specify a port when you [configure](./configuring.mdx) Stellar Core, but most people use the default, which is **11625**.
...
- Stellar Core exposes an _unauthenticated_ HTTP endpoint on its `HTTP_PORT`. You can specify a port when you [configure](./configuring.mdx) Stellar Core, but most people use the default, which is **11626**.
...
- Stellar Core requires access to a database (PostgreSQL, for example). If that database resides on a different machine on your network, you'll need to allow that connection. You'll specify the database when you [configure](./configuring.mdx) Stellar Core.
