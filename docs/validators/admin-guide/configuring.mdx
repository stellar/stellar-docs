...
Auditing of the P2P network is enabled by default. Learn more about disabling it in the [Overlay Topology section](./monitoring.mdx#overlay-topology-survey).
...
You specify your node's database by using the aptly named `DATABASE` field of your config file. Learn more in the [complete example config][complete-example-database]. It defaults to an in-memory database, but you can specify a path as per the example.
