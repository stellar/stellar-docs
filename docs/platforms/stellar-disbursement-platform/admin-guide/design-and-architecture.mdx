...
## Distribution Accounts {/* #distribution-accounts */}

Each tenant sends funds from one or more distribution accounts. A tenant starts with a single account, created when the tenant is provisioned, which is marked as the default. Tenants using the other account types described in [Multi-tenant Distribution Accounts](./advanced-configuration.mdx#multi-tenant-distribution-accounts) keep a single account.

Tenants configured with the `DISTRIBUTION_ACCOUNT.STELLAR.DB_VAULT` account type can hold up to 20 accounts, and Owners can add more at any time from the dashboard.

- Each account has its own Stellar keypair, encrypted separately in the database.
- Disbursements, payments, and receivers each record the account they belong to.
- Accounts are archived rather than deleted, so past disbursements and payments keep pointing at the account that sent them.
- Exactly one account is the default, and it cannot be archived. It is what a tenant starts with, and what pre-multi-distribution account records are attributed to.

Access to an account is granted per user, and is separate from the user's role. See [Security](./security.mdx) for how the two combine.
