...
- **Multiple accounts**: This is the only account type that supports more than one distribution account per tenant. The account created here becomes the tenant's default, and Owners can add more from the dashboard. See [Distribution Accounts](./design-and-architecture.mdx#distribution-accounts).
