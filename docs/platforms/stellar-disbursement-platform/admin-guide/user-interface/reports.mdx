...
The Reports menu item and page are optional. To show them only for specific organizations, set the **frontend** environment variable `REACT_APP_ENABLE_REPORTS_FEATURE` to a pipe-separated list of organization names (case-insensitive). Use the pipe character `|` as the separator (with or without spaces around it); names may contain spaces inside them (e.g. "Demo Organization"). For example, `"Demo Organization | Acme Corp"` enables Reports only for those orgs. Set to `true` to enable for all organizations, or `false` (or leave unset) to hide Reports for everyone. When the current organization is not in the list, the Reports entry is hidden from the sidebar and the `/reports` route is not available.
