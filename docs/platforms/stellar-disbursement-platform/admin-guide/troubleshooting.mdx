...
# Trace one payment, or everything sent from one distribution account
docker logs <tss-container-name> 2>&1 | grep -E "payment_id=<payment-id>|wallet_id=<account-id>"
```
...
The distribution account must hold enough of the asset being sent to cover the payment amount, plus enough XLM for transaction fees and the account's base reserve. If your organization has more than one, check the account named in the payment's **Account** column.
...
| `op_underfunded` | Source account doesn't have enough of the asset | Fund the payment's source distribution account with more of the asset |
...
Accounts added from the dashboard get their trustlines automatically. If that step fails and the account is active with no trustline for the asset, add it from the Trustlines panel on the [Distribution Accounts](./user-interface/distribution-accounts.mdx) page.
