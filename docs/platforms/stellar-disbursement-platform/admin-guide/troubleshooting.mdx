...
Channel accounts may disappear after testnet resets. Learn more in [Recreating Channel Accounts](#recreating-channel-accounts).
...
For a complete list of operation result codes, learn more in the [Stellar documentation](https://developers.stellar.org/docs/data/apis/horizon/api-reference/errors/result-codes/operation-specific/payment).
...
If the contact info matches but the OTP still isn't arriving, the issue is likely with the messaging provider. Learn more about common provider issues and how to diagnose them in [Check SDP Logs for Submission Failures](#check-logs-submission-failures).
