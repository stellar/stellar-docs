...
The SDP logs will show whether the invitation was sent and if the messaging provider accepted or rejected it.

```bash
# Look for invitation-related entries
docker logs <sdp-container-name> 2>&1 | grep -i "invitation"
```
