...
The `rotate-wallet-dek` command re-encrypts one distribution account's private key under a new key, without changing the Stellar account itself. Use it to roll key material after a suspected exposure, or as part of routine key rotation.
