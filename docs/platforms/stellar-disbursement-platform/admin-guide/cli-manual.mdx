...
| `--log-level` | The log level used in this project. Options: `TRACE`, `DEBUG`, `INFO`, `WARN`, `ERROR`, `FATAL`, or `PANIC`. Defaults to `INFO`. |
| `--log-shipping-url` | The Loki-compatible push endpoint (for example, a Grafana Alloy `loki.source.api` receiver) that structured logs are shipped to over HTTP, in addition to normal stdout logging. If not provided, log shipping is disabled. |
...
The `distribution-account` command manages a tenant's distribution accounts.
...
| :-- | :-- |
| `--crash-tracker-type` | Crash tracker type. Options: `SENTRY`, `DRY_RUN`. Defaults to `DRY_RUN`. |
| `--distribution-public-key` | The public key of the HOST's Stellar distribution account. It pays the fees for the transactions this command submits. Required by both subcommands; may be set with `DISTRIBUTION_PUBLIC_KEY` instead. |

### Subcommands

| Command | Description |
| :-- | :-- |
| `rotate` | Rotate the Stellar account behind one of a tenant's distribution accounts. |
| `rotate-wallet-dek` | Re-encrypt one distribution account's private key under a new key. |

---

### Distribution Account Rotate

The `rotate` command replaces the Stellar account behind one of a tenant's distribution accounts, moving its balances to a newly created account. Use `--wallet-id` to choose which one; when omitted, the tenant's default distribution account is rotated.
...
| `--channel-account-encryption-passphrase` | A Stellar-compliant ed25519 private key used to encrypt/decrypt the channel accounts' private keys. When not set, it will default to the value of the `distribution-seed` option. |
| `--distribution-account-encryption-passphrase` | A Stellar-compliant ed25519 private key used to encrypt and decrypt the private keys of tenants' distribution accounts. |
| `--distribution-seed` | The private key of the HOST's Stellar distribution account, used to sign the fee-bump that pays for the rotation. |
| `--horizon-url` | The URL of the Stellar Horizon server where this application will communicate with. Defaults to `https://horizon-testnet.stellar.org/`. |
| `--max-base-fee` | The max base fee for submitting a Stellar transaction. Defaults to `10000`. |
| `--tenant-id` | The tenant ID where the command will be applied. Required. |
| `--tenant-xlm-bootstrap-amount` | The starting XLM balance of the new account, sent from the account being rotated out. Acts as a floor: if the account's base and trustline reserves require more, the higher amount is sent instead. Defaults to `5`. |
| `--wallet-id` | ID of the distribution account to rotate. When omitted, the tenant's default distribution account is rotated. |

Only accounts that are active and use the `DISTRIBUTION_ACCOUNT.STELLAR.DB_VAULT` account type can be rotated.

#### Example

To rotate the default distribution account for a specific tenant:

```bash
stellar-disbursement-platform distribution-account rotate --tenant-id 'f347e6b0-249c-4960-b0d2-aebcf4c6a60d'
```

To rotate one specific distribution account:

```bash
stellar-disbursement-platform distribution-account rotate --tenant-id 'f347e6b0-249c-4960-b0d2-aebcf4c6a60d' --wallet-id '8d1f6a2e-3c4b-4a5d-9e6f-7a8b9c0d1e2f'
```

---

### Distribution Account Rotate Wallet DEK

The `rotate-wallet-dek` command re-encrypts one distribution account's existing private key under a fresh data-encryption key, without changing the Stellar account or the configured encryption passphrase. Use it for routine data-encryption-key rotation. If the Stellar signing key may have been exposed, use `rotate` to replace the account instead.

#### Usage

```bash
stellar-disbursement-platform distribution-account rotate-wallet-dek [flags]
```
...
| :-- | :-- |
| `--distribution-account-encryption-passphrase` | A Stellar-compliant ed25519 private key used to encrypt and decrypt the private keys of tenants' distribution accounts. |
| `--dry-run` | Verify the account's key decrypts under the current passphrase without writing any changes. |
| `--wallet-public-key` | Public key of the distribution account whose key should be re-encrypted. |

This command makes no changes on the Stellar network; it only re-encrypts stored key material, and identifies the account by its public key. It still requires the parent command's `--distribution-public-key`.

#### Example

To check that an account's key can be decrypted, without changing anything:

```bash
stellar-disbursement-platform distribution-account rotate-wallet-dek --wallet-public-key 'GCQVDMPOEN3HHJJ64V42KUTONOXN7C6V2PSVPALEVJ7PIYOB6M3DJV7N' --dry-run
```
