...
Communication from the Anchor Platform about transaction updates, customer updates, and quote creation is handled through the event service. This is an optional feature that needs to be configured separately from the SEP-6 integration. For more information, see [Event Handling](../../admin-guide/events/README.mdx).

You can find out more about transaction flow and statuses in the [SEP-24 protocol document](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md)
...
[![Sep24 Deposit Flow](/assets/anchor-platform/sep24/deposit-flow-diagram.png)](/assets/anchor-platform/sep24/deposit-flow-diagram.png)
...
The first step of the deposit flow after starting the deposit itself is collecting KYC. It's usually done in the web-app, but can also be optionally provided by the wallet application, using [SEP-9](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0009.md). Once the necessary KYC is collected, a `request_offchain_funds` JSON-RPC request should be made.
...
The business server periodically checks if the trustline was configured. If it was, it may send a payment and change the status of the transaction to `pending_stellar`.
...
[![Sep24 Withdrawal Flow](/assets/anchor-platform/sep24/withdrawal-flow-diagram.png)](/assets/anchor-platform/sep24/withdrawal-flow-diagram.png)
...
This method will be called automatically by the Stellar payment observer when it detects that onchain funds have been received.
