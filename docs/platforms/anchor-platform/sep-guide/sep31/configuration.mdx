...
Let's start by modifying our `stellar.toml` file created [earlier](../sep1/README.mdx). Wallets need to know that SEP-31 functionality is supported by your business, and they also need to know all currencies you support.
...
The information provided in the `sep31` and `send` objects closely map to the information that will be exposed to the wallet application using the [`GET /info`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0031.md#get-info) SEP-31 endpoint. The Anchor Platform also uses this information to validate requests made to your service. `sep31.fields.transaction` should be left empty and will be removed in a future release, but you can adjust the `send.min_amount` and `send.max_amount` values according to your service's limits.

The `sep31.quotes_supported` and `sep31.quotes_required` determine whether or not sending organizations can and are required to request an FX rate using the [SEP-38 `POST /quote`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0038.md#post-quote) endpoint. Almost all senders prefer this approach so that they can communicate the rate to their customers prior to proceeding.
...
See the [Anchor Platform KYC API specification](../../api-reference/callbacks/get-customer.api.mdx) for details on the endpoints that must be implemented on your business' server.
...
Businesses need to provide their send-side counterparts with a [Rate](../../api-reference/callbacks/get-rates.api.mdx) API to check the exchange rates they're offering between the on-chain asset being used for settlement and the fiat asset being used to pay the recipient. If the rate is competitive, senders also need to be able to request a commitment to the rate currently being offered from business for a short period of time.

The Anchor Platform provides the [SEP-38 RFQ API](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0038.md) to senders for this purpose.
