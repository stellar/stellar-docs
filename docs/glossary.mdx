...
The fee you’re willing to pay per operation in a transaction. When the network is in surge pricing mode, the base fee varies based on an auction mechanism. When it's not, the base fee defaults to the network base fee, currently set at 100 stroops per operation.
