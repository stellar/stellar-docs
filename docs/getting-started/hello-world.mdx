...
This tutorial will walk you through writing Sorobon contracts in Rust, deploying those contracts on Testnet, and then interacting with those contracts via a web app.
...
The `contracts` directory is where our Soroban contracts live. Each contract will have its own sub-directory with source code, test code and a `Cargo.toml` file.

```toml title="contracts/hello-soroban/Cargo.toml"
[package]
name = "hello-soroban"
version = "0.1.0"
edition = "2021"
...
We need to set the `crate-type` to `"cdylib"` in order to build the contract.

The dependencies are managed through the Rust workspace's `Cargo.toml` file at the root of the project, which we will take a closer look at below.
...
The boilerplate Astro web app files live in the `src` directory. This includes the Astro components, layouts, and pages. We'll look at these pieces a bit closer later in this tutorial.

As expected for a JavaScript web app, there is also a `package.json` file with JavaScript dependencies and scripts. We'll take a closer look at some of the scripts in that file later in the tutorial.

### Rust Workspace

There is also a `Cargo.toml` file at the root of the project to configure the Rust workspace for the full project. This is where workspace dependencies and profiles are defined, most notably the `soroban-sdk`.
...
## Hello Soroban Contract

Now that we have our project created, let's take a look a the hello-soroban contract in `/contracts/hello-soroban/src/lib.rs` in more depth.
...
```rust title=contracts/hello-soroban/src/lib.rs
#![no_std]
```

The contract then imports the types and macros that it needs from the `soroban-sdk` crate.
...
A simple test for hello-soroban will look like this:
