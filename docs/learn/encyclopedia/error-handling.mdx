...
3. Horizon responds with a `504 Timeout`. Due to the blocking nature of the endpoint, there could be many possible scenarios behind the timeout - transient outages, congestion on the network, issues on Core side, issues on Horizon side etc.... There is no way to cancel a transaction after submitting it. So, you need to carefully consider before resubmitting your transactions.
   ...
   3. `Resubmitting the transaction`: If the transaction has already successfully made it to the ledger, Horizon will return any attempted resubmission. Only in cases where a transaction’s status is unknown (and thus will have a chance to make it on the ledger) will a resubmission to the network occur.
