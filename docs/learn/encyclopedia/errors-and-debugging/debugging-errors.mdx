...
1. **Transaction Simulation (optional):**
   ...
   - **What happens:** This step involves executing the RPC endpoint [`simulateTransaction`](../../../../docs/data/rpc/api-reference/methods/simulateTransaction). The endpoint executes a transaction in 'simulation' mode of host and records the necessary ledger entries, CPU instructions, required authorizations. The end result is akin to running transactions in Core with maxed-out resources, unrestricted ledger access, and no fees.
