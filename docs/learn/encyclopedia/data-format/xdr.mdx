...
[Upcoming Feature] For the Soroban RPC, API endpoints will provide JSON format to unpack XDR base64 strings as long as a flag for `xdrFormat: "json"` is passed to the endpoints.
...
The canonical JSON and XDR Schema is defined by the [rs-stella-xdr library](https://github.com/stellar/rs-stellar-xdr) and by the [@stellar/stellar-xdr-json-web](https://www.npmjs.com/package/@stellar/stellar-xdr-json-web) npm package.

This conversion is visible in the output/input of Stellar CLI, Laboratory View XDR, Soroban RPC, and Hubble events table. The schema provides a round-trippable mean for converting any Stellar XDR type to JSON and converting that JSON back to the identical XDR.
...
- Self-Describing: The JSON format describes itself within the type but does not identify the type that is encoded. This is similar to XDR, which also does not identify the encoded type.

- Single Valid Representation: For any given XDR, there is a single valid JSON representation, and vice-versa. This ensures reproducibility and consistency. However, there are some exceptions.
