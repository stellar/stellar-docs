...
A JSON XDR Schema is defined by the [stellar-xdr crate](https://docs.rs/stellar-xdr) and also exposed by the [@stellar/stellar-xdr-json-web npm package](https://www.npmjs.com/package/@stellar/stellar-xdr-json-web).
...
The defined schema (linked above) is **not** backwards compatible between a given protocol version and prior versions. The best practice is to store required ledger data in XDR format.
