...
**Note:** there is no recovery mechanism for a claimable balance in general&mdash;if none of the predicates can be fulfilled, the balance cannot be recovered. The reclaim example below acts as a safety net for this situation.
...
```java
// Method 1: Not available in the Java SDK yet.
