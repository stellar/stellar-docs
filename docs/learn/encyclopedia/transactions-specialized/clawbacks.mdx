...
The issuer sets up their account to enable clawbacks using the `AUTH_CLAWBACK_ENABLED` flag. This causes every subsequent trustline established to the asset (being issued by the issuer) to have the `TRUSTLINE_CLAWBACK_ENABLED_FLAG` set automatically.
