...
This operation claws back a claimable balance, returning the asset to the issuer account, burning it. You must claw back the entire claimable balance, not just part of it. Once a claimable balance has been claimed, use the regular clawback operation to claw it back. Clawback claimable balances require the [claimable balance ID](../transactions-specialized/claimable-balances.mdx#additional-parameters). For more details, refer the [Clawback Claimable Balance operation](../../fundamentals/transactions/list-of-operations.mdx#clawback-claimable-balance).
