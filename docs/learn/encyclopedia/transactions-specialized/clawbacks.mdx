...
- **[Example 1](#example-1-payments):** Issuing account (Account A) creates a clawback-enabled asset and sends it to Account B. Account B sends that asset to Account C. Account A will then clawback the asset from C.
- **[Example 2](#example-2-claimable-balances):** Account B creates a claimable balance for Account C, and Account A claws back the claimable balance.
- **[Example 3](#example-3-selectively-enabling-clawback):** Account A issues a clawback-enabled asset to Account B. A claws back some of the asset from B, then removes the clawback enabled flag from the trustline and can no longer clawback the asset.

### Preamble: Issuing a Clawback-able Asset

First, we’ll set up an account to enable clawbacks and issue an asset accordingly. Properly issuing an asset with separate [issuer and distributor accounts](../../../tokens/control-asset-access.mdx#issuing-and-distribution-accounts) is a little more involved. We’ll use a simpler method here as an example.
