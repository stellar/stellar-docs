...
- Ensure `[]`channel indexing is clear
img showing logic structure as needed
- Clear chain of custody between ea.
2 id should be fine per section on
...
const channelKeypairs = channelAccountSecrets.map(
  (secret) => Keypair.fromSecret(secret),
);

const channelAccounts = await Promise.all(
  channelKeypairs.map(
    (keypair) => server.loadAccount(keypair.publicKey())
  )
);
