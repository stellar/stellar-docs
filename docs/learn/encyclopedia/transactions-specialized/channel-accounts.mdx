...
The bundle size is how many operations you fit in each transaction sent by channel accounts. While the protocol defines a maximum of 100, it may take longer than desired for your flow of operations to reach this threshold. If that's the case, each channel can submit their own transaction each ledger with all incoming operations.
