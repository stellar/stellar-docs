...
This approach works because of a unique native network feature. By design, accounts can submit a transaction with operations originating from any account. This flexibility allows you to encode different sources throughout the transaction:
...
You can distribute transaction submissions evenly across channel accounts to maximize throughput. This example walks through an example to monitor and manage the lifecycle of channel accounts. It combined our preparations with funding, transaction submission, and securing each account.

Groups of channel accounts use two states for effective rotation:
...
Introduce high-freq batching through our example

How high you want operations per each transaction
