...
Notice that, in contrast to the following example, we didn't need to do simulation separately. This is because we can sign the transaction as-is rather than need to inspect its authorization entries.
...
To keep things really simple, we won't much coding. Instead, we'll just show the difference to the previous example: all we need to do is run simulation once more.
