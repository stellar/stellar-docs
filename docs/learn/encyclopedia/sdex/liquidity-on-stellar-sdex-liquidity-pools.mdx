...
def newTxBuilder(source):
  sourceAccount = server.load_account(source)
...
public static Transaction.Builder newTxBuilder(KeyPair source) throws Exception {
...
func newTxBuilder(source *horizonclient.AccountRequest) (*txnbuild.Transaction, error) {
	account, err := server.AccountDetail(*source)
	check(err)
...
func distributeAssets(issuerKp, recipientKp *keypair.Full, assets []txnbuild.Asset) error {
	source := &horizonclient.AccountRequest{AccountID: issuerKp.Address()}
	builder, err := newTxBuilder(source)
	check(err)
	...
	builder.Params.Operations = ops
	tx, err := builder.BuildSignEncode(issuerKp, recipientKp)
	check(err)
...
```go
func establishPoolTrustline(source *keypair.Full, poolAsset txnbuild.LiquidityPoolAsset) { // source *keypair.Full since global definition DNE at `newTxBuilder`
  txParams := newTxBuilder(source)
  txParams.Operations = []txnbuild.Operation{
    &txnbuild.ChangeTrust{
      Line: poolAsset,
    },
  }
...
async function main() {
  ...
  const { balances } = await server.accounts().accountId(
    kps[1].publicKey()
  ).call();

  const balance = balances.find(bal => 
...
```go
func main() {
  accounts := make([]horizonclient.Account, len(kps))
	for i, kp := range kps {
		account, err := server.AccountDetail(
      horizonclient.AccountRequest{AccountID: kp.Address()}
    )
		check(err)
		accounts[i] = account
	}
  kps := make([]*keypair.Full, len(secrets))
	...
	for i := 1; i < len(kps); i++ {
	  ...
	  // Establish pool trustline
		establishPoolTrustline(kp, poolID)

		// Add liquidity
		depositA := float64(i+1) * 1000 // Incremental deposit scalar
		depositB := float64(i+1) * 3000 // Maintain 1:3 ratio
	...
	// kp1 removes all liquidity
	kp1 := kps[1]
	account, err := server.AccountDetail(horizonclient.AccountRequest{AccountID: kp1.Address()})
	check(err)

	var balance float64
	for _, bal := range account.Balances {
		if bal.AssetType == "liquidity_pool_shares" && bal.LiquidityPoolID == poolID {
