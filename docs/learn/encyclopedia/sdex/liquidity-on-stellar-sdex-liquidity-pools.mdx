---
title: "The Stellar DEX and Automated Market Makers: Provide Liquidity and Enable Asset Swaps"
...
If we want to trade some AstroDollars and AstroPesos, the first step is generally determiing the current market "spot" price. Once we know the going exchange rate, we can offer our own assets up for trade:
...
While there is no concept of a "market order" in the traditional sense, you can submit a trade to the SDEX above the lowest asking offer. Our example uses an arbitrary excess margin of 5%.

:::

The orderbook will only fill you at the price you specify or better, when avaliable.[^best-ex] So, you can rest assured that you will receive best execution if you know that your trade is worth 1,000 AstroDollars and there are 5,000 AstroDollars avaliable in counter-listing depth avaliable at a 1% variance.

[^best-ex]: Since the network's [protocol rules](https://github.com/stellar/stellar-core/blob/fbb53d8ad42dcc12a046c9be949d654821a24d38/src/transactions/OfferExchange.cpp#L227-L550) favor new offers over existing liquidity, you receive the better limit price avalible in the orderbook even if your offer specifies a worse rate. For example, if you specify a 10 USD/MXN purchase but someone is selling already at 9.6 USD/MXN, then you will get the lower rate up to the amount of AstroDollar specified in the lower offer.

We can find a midpoint price now that we have the depth of the market, up to the [paging token limit](../../../data/horizon/api-reference/structure/pagination/README.mdx#larger-queries). We'll calculate this number as the average of the highest, best bid and the lowest, best offer:
...
### Scanning Recent Trades

A more complete understanding of market liquitiy might incldue volume analysis. To affirm the fairness of this example price, we can also check up to 25 recent trade prices:
...
def sanityCheck(midpointPrice, averageRecentPrice, threshold=0.05):
...
use $`orderbook` and price = str(`offerPrice`)
```
...
Now that we know how much someone will pay for our AstroPesos, we can convert into AstroDollars. Existing or new orders counteract and "fulfill" our trade:
...
class firstTrade {
  public static void main(String[] args) {
    ...
    System.out.println("Transaction successful: " + response.isSuccess());
