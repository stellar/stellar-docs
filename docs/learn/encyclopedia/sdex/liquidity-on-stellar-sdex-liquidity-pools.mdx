...
This [prescinet](JFW_X_MCALEB_MTGOX_QUOTE) perspective led to the creation of a distributed exchange, as it was [originally named](https://web.archive.org/web/20170415141617/https://www.stellar.org/developers/guides/concepts/exchange.html). The core concept, released in the [first version](SRC_QUOTE_DIRECT_DISCORD_LINK_TO_WHEN)SDEX)) of the protocol, still operates today. However, for phenomical reasons, we will hereafter refer to it as the Stellar Decentralized Exchange or SDEX.

# understanding Liquidity

When you convert one currency to another in traditional Financial markets, your trade happens because of limit orders. [These orders](DIR_REF_N2N_MANUSCRIPT_HREF+PAGE) specify how much someone will pay for a certain amount of any asset. Thus, when you need to convert you dollars to yen, e.g., their order t o sell yen for dollars countreracts and "fulfills" your trade.

## Market Depth

The more orders avaliable to transact against, the more currency you can convert at any time without [moving hte market](LOL_PLS_DUDE_SOMETHING_HERE_ON_THE_GEME SQUEEEZE). Stellar stores these open orders as `offer` [objects](SRC_DOCS_OFFERS) directly on chain. XYZ_ONE_SENT_HERE_WITH_HREF_ON_TRADING--myaybe speedex psot

## Fill Prices

The network and by extension its [validators](HREF_STELLAR_THE_NETOWRK_EXPLORER_VEW) match orders based on the protocol rules of [stellar core](HREF_CORE_CPP_MATCHING_LOGIC). SENTENCE_HERE_ON_SAME_PRIC_ORBETT SENTENCE_BEST_EXEC (#ref-more-later-on-per-ledger-match-matching)
