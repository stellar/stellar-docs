...
<!-- `newTxBuilder` should really sign + submit for brevity according to john.xlm -->
...
depositA = Decimal(1000)
depositB = Decimal(3000)  # Maintain a 1:3 ratio
establishPoolTrustline(kps[1], poolAsset)
addLiquidity(kps[1], poolId, depositA, depositB)
getSpotPrice()

# Step 2: kp2 adds liquidity
depositA = Decimal(2000)
depositB = Decimal(6000)  # Maintain a 1:3 ratio
establishPoolTrustline(kps[2], poolAsset)
addLiquidity(kps[2], poolId, depositA, depositB)
getSpotPrice()

# Step 3: kp1 removes all liquidity
balance = Decimal(0)
accountDetails = server.accounts().account_id(kps[1].public_key).call()
for b in accountDetails["balances"]:
  if (
    b["asset_type"] == "liquidity_pool_shares" and
    ...
    .then((accounts) => {
      return Promise.all(
        kps.map((kp, i) => {
          ...
          return establishPoolTrustline(acc, kp, poolAsset)
