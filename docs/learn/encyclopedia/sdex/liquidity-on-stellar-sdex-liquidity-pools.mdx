...
This section is scoped specifically to liquidity regarding the Stellar Decentralized Exchange and Automated Market Makers built into the Stellar protocol. It does not include information regarding smart contracts. Rather, it explains the network's native layer-1 atomic swap infrastructure.
...
Users can trade assets across the Stellar network using [priced orders](NEW_SELL_OFF), [path payments](../transactions-specialized/path-payments.mdx), and [native AMMs](PROT_19_SPEC). These freatures leverage Stellar’s decentralized exchange, which has existed since the launch of the network, and liquidity pools, introduced in Protocol 18.
