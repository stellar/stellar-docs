...
# Stellar Networks Overview

Stellar offers three networks for different stages of development:

- **Mainnet (Public Network):** Used for production applications. Interacts with real financial systems and requires real XLM.
- **Testnet:** A stable environment for testing with free resources, including test XLM from Friendbot.
- **Futurenet:** An experimental network for previewing and testing new or unreleased features.

## Network Comparison

| Feature | **Mainnet** | **Testnet** | **Futurenet** |
| --- | --- | --- | --- |
| **Purpose** | Production use | Testing stable features | Testing experimental features |
| **Validators** | Run by the public | 3 SDF-run core validators | SDF-run core validators |
| **Horizon Access** | Provided by ecosystem partners or self-hosted [Horizon](../../data/apis/horizon/admin-guide/overview.mdx) | Free SDF-provided [Horizon](https://horizon-testnet.stellar.org) | Free SDF-provided [Horizon](https://horizon-futurenet.stellar.org) |
| **Account Funding** | Fund with real XLM from another account | Fund using Friendbot (free test XLM) | Fund using Friendbot (free test XLM) |
| **Operations per Ledger** | 1,000 total; smart contract txs limited by [resource limits](./fees-resource-limits-metering.mdx#resource-limitations) | 100 total; 1 smart contract transaction max | 100 total; 1 smart contract transaction max |
| **RPC Availability** | No public RPC; see [RPC providers](../../data/apis/api-providers.mdx) | Free public RPC from SDF ([info here](../../data/apis/api-providers.mdx#sdf-provided-rpc)) | Free public RPC from SDF ([info here](../../data/apis/api-providers.mdx#sdf-provided-rpc)) |
| **Friendbot** | ❌ | ✅ [Friendbot](../../tools/lab/account.mdx) funds with 10,000 test XLM | ✅ [Friendbot](../../tools/lab/account.mdx) funds with 10,000 test XLM |
| **Reset Frequency** | Never | Regularly scheduled | As needed, unpredictable |

## What is Friendbot?

Friendbot is a developer tool that sends **10,000 fake XLM** to new accounts on **Testnet** or **Futurenet**. You can use it via:

- [Stellar Lab](../../tools/lab/account.mdx)
- SDKs in various programming languages

Note: Friendbot usage is rate-limited. For multiple accounts, fund the first account with Friendbot and use that to fund others via the Create Account operation.
