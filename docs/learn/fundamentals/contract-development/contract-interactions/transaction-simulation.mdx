...
Please refer to the [authorization overview](../authorization.mdx) and transaction authorization [section](stellar-transaction.mdx#authorization-data) for general information on Soroban authorization: this section pertains specifically to how simulation works alongside authorization requirements.

Soroban's transaction [simulation mechanism](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx) can be used to precompute the [`SorobanAuthorizedInvocation`](https://github.com/stellar/stellar-xdr/blob/v22.0/Stellar-transaction.x#L558) trees that must be authorized by the `Address`es for all the `require_auth` checks to pass. It can be invoked in two different ways:
...
Note that the "recording" auth mode _never_ emulates authorization failures. This is because failing authorization is always an "exceptional" situation (i.e., the `Address`es for which you don't anticipate successful authorization shouldn't be used in the first place). It is similar to how, for example, the [`simulateTransaction`](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx) mechanism doesn't emulate failures caused by the incorrect footprint.

If you'd like to validate signatures, you should use [`simulateTransaction`](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx) in authorization ["enforcement" mode](#enforcing-mode), which will verify the signatures before executing the transaction on-chain.

### Enforcing Mode

The recording auth mode is one option for [`simulateTransaction`](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx). However, when dealing with the custom account contracts, for example, it may be necessary to simulate the custom account's `__check_auth` code (which is simply _omitted_ in the recording auth mode), to get its ledger footprint.
...
In this example, we'll leverage transaction [simulation](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx)'s auth ["enforcement" mode](#enforcing-mode), which, when given signed authorization entries, will ensure that they are the necessary and sufficient signatures for the transaction's execution.
