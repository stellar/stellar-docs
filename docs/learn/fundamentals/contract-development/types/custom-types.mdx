...
The `export` argument on `#[contracttype]` (e.g., `#[contracttype(export = false)]`) is deprecated when using the `experimental_spec_shaking_v2` feature (Protocol 27). Under spec shaking v2, type visibility in the contract spec is determined automatically by reachability. Remove `export = ...` from `#[contracttype]` annotations. See the [SDK migration guide](https://docs.rs/soroban-sdk/latest/soroban_sdk/_migrating/v27_export/index.html) for details.
