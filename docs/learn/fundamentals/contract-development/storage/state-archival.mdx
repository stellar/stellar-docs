...
- When a `Persistent` or `Instance` entry TTL is 0, it is "archived" but can be "restored" automatically via the `InvokeHostFunction` operation.
...
- Archived when TTL goes to 0, is automatically restored via the `InvokeHostFunction` operation.
...
Starting in Protocol 23 ([CAP-0066: Soroban In-Memory Read Resource](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0066.md)), archived `Persistent` or `Instance` contract entries are automatically restored whenever they appear in a footprint during `InvokeHostFunctionOp`. Developers no longer need to issue a separate restore operation -- restoration happens before the host function runs, with the usual fees and rent still applying.
...
This manual operation (`RestoreFootprintOp`) is no longer needed starting in Protocol 23, as archived entries are automatically restored when they appear in a footprint during `InvokeHostFunctionOp`. This section is left here for historical context.
...
The manual operation (`RestoreFootprintOp`) is (for the most part) no longer needed starting in Protocol 23, as archived entries are automatically restored when they appear in a footprint during `InvokeHostFunctionOp`. These examples are left here for historical context.
...
2. **Prepare your operation**. This means describing the ledger entries within the corresponding operation (i.e., `ExtendFootprintTTLOp`) and its ledger footprint (the `SorobanTransactionData` field), then simulating it to fill out fee and resource usage information (when restoring, you usually have simulation results already).
