...
- Prefer `Temporary` over `Persistent` and `Instance` storage
  - Anything that can have a timeout should be `Temporary` with TTL set to the timeout. Learn more in the [resource limits table](https://lab.stellar.org/network-limits) on the Stellar Lab for the current maximum TTL/timeout.
