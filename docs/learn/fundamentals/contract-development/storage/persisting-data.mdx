---
sidebar_position: 10
title: Smart Contract Data Storage
...
An unbounded collection is the usual way to get this wrong. A `Vec` or `Map` held under a single key is read and written in full on every update, so its cost grows with the whole collection, and the write fails once the entry passes the maximum contract-data entry size. Every caller that changes the collection writes that same entry, so their transactions contend and execute sequentially. A collection that contract callers can grow has no natural bound, so give each item its own ledger entry instead. The [storage strategies guide](../../../../build/guides/storage/storage-strategies.mdx) shows the layouts that do this, and its [red flags in review](../../../../build/guides/storage/storage-strategies.mdx#-red-flags-in-review) list the storage mistakes to look for.
