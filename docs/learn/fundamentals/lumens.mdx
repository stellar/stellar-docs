...
**sdfMandate** The SDF Mandate is described in detail on the [Stellar Foundation mandate page](https://stellar.org/foundation/mandate), which is the authoritative source for the mandate's current structure and the accounts associated with it. The Foundation was funded by lumens generated at Stellar’s inception; all of those lumens will eventually be spent or distributed to enhance and promote Stellar. The list below is a **pre-2025 snapshot** — it is **not current** and **not exhaustive**. It predates the mandate's 2025 bucket structure and diverges from it in both directions (it includes accounts no longer listed and omits accounts that are now part of the mandate). Do not treat these addresses as the current mapping or combine them as one; always use the mandate page above for the authoritative, up-to-date breakdown. Retained here only as historical reference:
