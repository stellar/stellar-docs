...
**sdfMandate** The SDF Mandate is described in detail on the [Stellar Foundation mandate page](https://stellar.org/foundation/mandate), which is the authoritative source for the mandate's current structure and the accounts associated with it. The Foundation was funded by lumens generated at Stellar’s inception; all of those lumens will eventually be spent or distributed to enhance and promote Stellar. The addresses below are retained here as a historical reference and may not reflect the current mandate structure or account set — always refer to the mandate page above for the up-to-date breakdown:
