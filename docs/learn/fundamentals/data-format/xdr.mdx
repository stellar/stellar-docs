...
Stellar stores and communicates ledger data, transactions, results, history, and messages in a binary format called External Data Representation (XDR). XDR is defined in [RFC4506](https://tools.ietf.org/html/rfc4506.html). XDR is optimized for network performance but not human readable. The Stellar SDKs convert XDRs into friendlier formats.
...
Stellar XDR is encodable into JSON using the [XDR-JSON](./xdr-json.mdx) schema.
