...
When multiple XDR objects are stored sequentially in a file or stream, each object is framed using the Record Marking Standard defined in [RFC 5531 Section 11][RFC5531s11]. Each record is composed of one or more fragments. Each fragment begins with a 4-byte header followed by the fragment data:
...
In Stellar's usage each record contains exactly one XDR object, encoded as a single fragment with the last-fragment bit always set. This framing is used in:

- **History archives**: Bucket files and checkpoint ledger files written and read by stellar-core.
- **Streaming XDR files**: Files read and written by the Rust XDR library's `Frame` type in rs-stellar-xdr, and any tools built on it.
