...
If we want to trade some AstroDollars and AstroPesos, the first step is generally determiing the current market "spot" price. Once we know the going exchange rate, we can offer our own assets up for sale.

While this is a necessary intermediate step, it might ntot always apply to your use case. While there is no concept of a "market order" in the traditional sense, you can submit a trade to the SDEX above the lowest asking offer by a margin of say 5%.
...
Now that we have seen the depth of the market (up to the [paging token limit](../../../data/horizon/api-reference/structure/pagination/README.mdx#larger-queries)), we can find the midpoint price. This number comes from the average of the best (highest) bid and (lowest) offer. To affirm the fairness of this price, we might also check the recent trade prices.
