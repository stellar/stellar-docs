...
You do not need to run your own Horizon instance — when you're getting started, you can use the free SDF Horizon instance to access the network — but it is recommended that you do when you’re ready to launch a finished product. Check out how to do so here: [Run Platform Services Tutorial](../../data/apis/horizon/admin-guide/README.mdx)

Learn all there is to know about using Horizon in the Horizon [documentation](../../data/apis/horizon/README.mdx).
...
SDF does not provide a publicly available RPC endpoint for Mainnet. Developers should [select an ecosystem provider](../../data/apis/rpc/rpc-providers.mdx) that works for their project before migrating to Mainnet. In some cases, projects may choose to run their own RPC instance.
