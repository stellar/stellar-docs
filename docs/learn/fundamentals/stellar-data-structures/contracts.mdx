...
### Wasm Compilation and Installation

Once a developer writes a smart contract in a supported language, such as Rust, the contract is compiled into a Wasm executable file. This file contains low-level bytecode that can be efficiently executed within a sandboxed environment. After compilation, the Wasm executable is "installed" on the Stellar network, where it is stored as a `CONTRACT_DATA` [ledger entry](./ledgers.mdx) . The unique identifier for this entry is derived from the hash of the compiled binary and is called `Wasm hash`.

To retrieve Wasm executable file from the Stellar network to "install" in your local network, it is possible to do so using the RPC's `getLedgerEntries` or use [Stellar Lab's Contract Explorer](https://lab.stellar.org/smart-contracts/contract-explorer).
