...
Number of times [inflation](../../../glossary.mdx#inflation) has been run. The inflation operation was disabled when validators voted to upgrade the network to Protocol 12 on 28 Oct 2019. This number does not change without inflation.
...
A bitmask stored by extension, with static functionality. Validators vote to activate flags during major software errors, although they have never been used. They define three emergency switches that can respond to serious native-AMM problems. Unlike configuration settings, these flags are not separate ledger entries: they have no keys or tunable parameter value.
