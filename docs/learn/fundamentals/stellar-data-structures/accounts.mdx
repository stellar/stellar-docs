...
There are two types of accounts on Stellar: Stellar accounts (`G...` addresses) and [contract accounts (`C...` addresses)](https://developers.stellar.org/docs/build/smart-contracts/example-contracts/custom-account). This section focuses on Stellar `G...` accounts.
