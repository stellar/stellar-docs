...
The Asset parameter is of the `TrustLineAsset` type. If you are modifying a trustline to a regular asset (i.e. one in a `Code:Issuer` format), this is equivalent to the Asset type. If you are modifying a trustline to a pool share, however, this is composed of the liquidity pool's [unique ID](../../learn/fundamentals/liquidity-on-stellar-sdex-liquidity-pools.mdx#TODO).
