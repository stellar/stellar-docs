...
All these operations have an optional source account paramter. If the source account parameter is omitted, the source account of the transaction is considered as the source account of the operation
...
This operation can only be performed by the asset issuer

:::

Updates the authorized flag of an existing trustline This operation is deprecated as of Protocol 17- prefer SetTrustlineFlags instead
...
Burns an amount in a specific asset from an account

:::note

Only the issuing account for the asset can perform this operation
