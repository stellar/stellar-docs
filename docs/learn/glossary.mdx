...
An exchange in which every asset transfer succeeds together or none is applied. On Stellar, atomic swaps are the default behavior of the protocol-level decentralized exchange (SDEX), not an additional service: when an offer crosses the order book, the assets change hands as a single, indivisible ledger operation. Stellar transactions are also atomic, so if any operation fails, all earlier operations are rolled back and the transaction is not applied.

This lets accounts trade assets without relying on a trusted intermediary to hold either side of the exchange. The SDEX reserves liabilities for open offers so a matched offer can be executed, and path payments can atomically route a conversion across order books and liquidity pools.

Learn more about [liquidity on the SDEX](./fundamentals/liquidity-on-stellar-sdex-liquidity-pools.mdx) and [atomic transactions](./fundamentals/transactions/operations-and-transactions.mdx). For a smart contract approach that matches several authorized participants, see the [Batched Atomic Swaps example](../build/smart-contracts/example-contracts/atomic-multi-swap.mdx).
...
The maximum number of operations per ledger, as determined by validator vote. As of July 2026, the limit is 1,000 operations for the mainnet and 200 operations for the testnet; check current values on Stellar Lab's [Network Limits](https://lab.stellar.org/network-limits) page.
...
Short for Scalable, Parallelizable, and Economically Efficient Distributed EXchange, SPEEDEX is a proposed fully on-chain decentralized exchange design that processes trades in batches at shared exchange rates. This approach makes trades parallelizable, eliminates internal arbitrage opportunities, and prevents certain risk-free front-running attacks. SPEEDEX has been prototyped for Stellar but is not currently deployed on the network.

Learn more in the [SPEEDEX research paper](https://arxiv.org/abs/2111.02719), [SDF blog post](https://stellar.org/blog/developers/building-speedex-a-novel-design-for-decentralized-exchanges), and [CAP-44 configuration proposal](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0044.md).
