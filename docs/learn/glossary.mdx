...
Learn more in the [Accounts section](./fundamentals/stellar-data-structures/accounts.mdx) to learn more.
...
Learn more in the [Anchor Encyclopedia Entry](./fundamentals/anchors.mdx).
...
2. an issuer claws back a clawback-enabled asset from a holder's account, or
3. an issuer aquires their own asset through the SDEX.
...
todo https://stellar.org/blog/developers/composable-data-platform from D:\GitHub\stellar-docs\docs\build\apps\ingest-sdk
...
A fully-qualified domain name linked to a Stellar account. Home domains let [apps](#app) generate on-chain links to a [Stellar Info File](#stellar-toml), which holds off-chain metadata. 
...
LedgerKey holds information to identify a specific ledgerEntry. It is a union that can be any one of the LedgerEntryTypes ([`ACCOUNT`](#account), [`TRUSTLINE`](#trustline), [`OFFER`](#offer), [`DATA`](#manage-data), `CLAIMABLE_BALANCE`, Liquidity Pool, Contract Data, Contract Code, Config Setting, or [`TTL`](#ttl)).
...
### OperationID {#todo-see-Account-ID}
...
The ratio of the quote asset and the base asset in an [order](#offer).
...
Stellar’s a layer-2 protocol that allows for bi-directional payment channels.
...
You can learn more on their [website](https://stellar.org) or view their / fonuding tbd [Articles Certificate](/assets/sdf/start-filing.pdf) todo https://icis.corp.delaware.gov/ecorp2/services/e-filing for 5507594 from https://web.archive.org/web/20140801062313/https://www.stellar.org/wp-content/uploads/2014/07/FILED-STELLAR-DEVELOPMENT-FOUNDATION-00594674.pdf
...
A formatted configuration file containing published information about a node and an organization. For more, see the [Stellar Info File SEP](https://stellar.org/protocol/sep-1).
...
Read more about the [Lumen Supply Metrics](./fundamentals/lumens.mdx#lumen-supply-metrics). nd manage their assets.
