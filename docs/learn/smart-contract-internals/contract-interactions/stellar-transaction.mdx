...
<head>
  ...
  <link
    rel="canonical"
    href="https://soroban.stellar.org/docs/soroban-internals/contract-interactions/stellar-transaction"
  />
</head>
...
- Deploy new contracts using the uploaded Wasm or built-in implementations (this currently includes only the [token contract](https://soroban.stellar.org/docs/tokens/stellar-asset-contract)).
...
Every Soroban transaction has to have a `SorobanTransactionData` transaction [extension] populated. This is needed to compute the [Soroban resource fee](../fees-and-metering.mdx).
