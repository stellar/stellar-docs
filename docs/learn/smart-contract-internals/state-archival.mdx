...
For each entry type, there is a minimum TTL that the entry must have when being created or restored. This TTL minimum is enforced automatically at the protocol level. This minimum is a network parameter and defaults to 16 ledgers for `Temporary` entries and 4,096 ledgers for `Persistent` and `Instance` entries.
