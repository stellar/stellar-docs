...
- **SDP Core Service**: the core backend service that performs several functions:
  ...
  - **Wallet Registration UI**: a web application that collects and verifies the recipient’s OTP code and verification information via Stellar’s SEP-24 protocol
...
- **Stellar Accounts**:
  ...
  - SEP-10 Auth Account: the SDP requires a Stellar account for the mutual authentication protocol (SEP-10) used to connect to wallet applications
