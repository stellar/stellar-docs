...
In this guide, we'll help you set up your local workstation with a complete end-end development environment using docker-compose, Stellar demo-wallet, USDC on Stellar, and Stellar Testnet. Let’s bring up all the services. Disclaimer: USDC and the Demo Wallet are used in the following walk-through demo as an example asset and wallet, though they are not the only options.
...
- Create the Stellar account used for SEP-10 authentication and configure the following values in docker-compose-sdp-anchor.yml
...
- Create a Stellar account and fund it with the digital asset you will send in your disbursement (ex: USDC). This account will be used as the Distribution Account. Configure the following values in docker-compose-sdp-anchor.yml
...
- **Optional for testnet but must be done in production**: Get a Google Recaptcha site key/secret and configure the following values in docker-compose-sdp-anchor.yml
