...
The link format supported by the SDP supports a simple format like `https://<host-with-optional-path>`. If your deep linking platform requires anything more than that, this custom logic will need to be handled by a server owned by the Wallet that's capable of receiving the deep link, parsing it, and redirecting the user to the appropriate location.

### Understanding the Deep Link Format

Once the user has installed the wallet application, the wallet should use the information included in the [deep link] to kick off the [Wallet Registration Procedure](#wallet-registration-procedure). The deep link format supported by the SDP is as follows:
...
When opening registration [deep link], these are the steps the wallet should follow in order to enforce the security and pricacy measures expected in this flow, and to allow the user to input their information dicectly with the SDP:
