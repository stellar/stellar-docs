...
| --- | --- |
| Soroban Txn per ledger | 125 |
| Max CPU Instructions per Txn | 500 million |
| Memory limit per Txn | 42 MB |
| Ledger entry size (including Wasm entries) per Txn | 66 KB |
| Read/Write Ledger entries per Txn | 40 read; 25 write |
| Read/Write bytes per Txn | 133 KB read; 66 KB write |
| Transaction size | 70 KB |
| Persistent entry minimal/initial lifetime | 120 days |
| Temporary entry minimal/initial lifetime | 1 day |
| Max ledger entry expiration bump | N/A |
| Events+return value size bytes | N/A |
