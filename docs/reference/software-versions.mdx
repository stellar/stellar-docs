...
This release comes with a revamp of authorization approach that is breaking for most of the contracts that did any sort of auth logic or used tokens. [example](https://developers.stellar.org/docs/tutorials/auth.mdx) and [authorization overview](../learn/soroban-internals/authorization.mdx) for more details.
