...
- **[MPP Router](https://www.mpprouter.dev)** by ROZO: A Cloudflare Worker exposing a stable public API for paid services, paid per request with MPP on Stellar. It accepts USDC through the `stellar.charge` intent and Stellar payment channels, and refunds the full USDC amount to the original payer on a timeout, upstream 5xx, or empty response. Services include Stellar indexed data not available from Horizon or RPC: Mercury by Xycloo Labs (full-history Soroban events and transactions) and Stellar Indexer by Creit Tech (classic liquidity pools and live Soroban contract storage). The payment channel contracts were audited by HackenProof, with all findings resolved ([report](https://github.com/mpprouter/rozo-mpprouter/releases/download/v0.2.2/HackenProof.Audit.Report.for.MPP.Router.ROZO.pdf)). [Service catalog](https://apiserver.mpprouter.dev/v1/services/catalog). [View on GitHub](https://github.com/mpprouter/rozo-mpprouter)
