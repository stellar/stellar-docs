...
Third-party services built on MPP on Stellar.

### [MPP Router](https://www.mpprouter.dev) by ROZO

An agentic payments gateway: one API for hundreds of paid services, paid per request in USDC with MPP on Stellar, through either the `stellar.charge` intent or payment channels. Requests that are not delivered are refunded automatically.

- **Stellar infrastructure services**: [Mercury Data](https://www.mpprouter.dev/services/mercury) by Xycloo Labs (full-history Soroban events and transactions) and [Stellar Indexer](https://www.mpprouter.dev/providers/stellar-indexer) by Creit Tech (classic liquidity pools and live Soroban contract storage), indexed data that Horizon and RPC do not provide.
- **AI models**: [Anthropic Claude](https://www.mpprouter.dev/services/anthropic) and [OpenAI](https://www.mpprouter.dev/services/openai) APIs routed through Tempo MPP services.
- **Security**: payment channel contracts audited by HackenProof, all findings resolved ([report](https://github.com/mpprouter/rozo-mpprouter/releases/download/v0.2.2/HackenProof.Audit.Report.for.MPP.Router.ROZO.pdf)).
- **Links**: [Service catalog](https://apiserver.mpprouter.dev/v1/services/catalog), [GitHub](https://github.com/mpprouter/rozo-mpprouter)
