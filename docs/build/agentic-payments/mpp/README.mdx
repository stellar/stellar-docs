...
- **[MPP Router](https://www.mpprouter.dev)**: A Cloudflare Worker exposing a stable public API for paid services, paid per request with MPP on Stellar. It accepts USDC through the `stellar.charge` intent and Stellar payment channels. On a timeout, upstream 5xx, or empty response, the Router refunds the full USDC amount to the original payer on Stellar. [Service catalog](https://apiserver.mpprouter.dev/v1/services/catalog). [View on GitHub](https://github.com/mpprouter/rozo-mpprouter)
