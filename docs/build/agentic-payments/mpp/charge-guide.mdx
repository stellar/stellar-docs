...
To use push mode, set `mode: "push"` in the client configuration and provide the keypair of the `from` account (the account funding the transfer). The client will then sign a `signedHash` credential containing the transaction hash:
...
On the server, enable push mode by setting `mode: "push"` in the charge configuration. As of v0.7, push-mode credentials must be payer-authenticated (`signedHash`) by default — the server automatically rejects the legacy unsigned `{ type: 'hash' }` format, so no extra option is needed:
...
Teams migrating from earlier SDK versions can temporarily re-enable the legacy unsigned `{ type: 'hash' }` format with `allowUnsignedPush: true`. This logs each acceptance for migration tracking and should be removed once all clients send `signedHash` credentials:

```js title="server.js (temporary migration)"
    stellar.charge({
      recipient: RECIPIENT,
      currency: USDC_SAC_TESTNET,
      network: "stellar:testnet",
      mode: "push",
      allowUnsignedPush: true, // TEMPORARY: accept legacy unsigned push during migration
    }),
```
...
3. Signing this string using the keypair of the `from` account
...
:::note As of v0.7, the legacy `{ type: 'hash' }` credential format (unsigned hash only) is **rejected by default** — this is a breaking change. Operators who need a temporary migration window can opt in with `allowUnsignedPush: true`, which accepts legacy push credentials and logs each acceptance for tracking. New implementations must use the `signedHash` credential format. :::
