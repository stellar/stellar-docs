...
:::note
The legacy `{ type: 'hash' }` credential format (unsigned hash only) is accepted in v0.7 with a deprecation warning. It will be rejected by default in v0.8 unless the server explicitly allows it via `rejectUnsignedPush: false`. New implementations should use the signed hash credential format.
:::
