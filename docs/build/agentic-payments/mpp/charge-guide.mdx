...
import { Mppx, AtomicStore } from "mppx/server";
...
const mppx = Mppx.create({
  ...
  store: new AtomicStore(), // Required by v0.7 for compare-and-set operations
  methods: [
    stellar.charge({
      recipient: RECIPIENT,
...
The examples above use **pull mode** (default), where the server broadcasts the signed transaction. In **push mode**, the client broadcasts the transaction directly and sends the signed transaction hash to the server for verification.

### Push mode client

To use push mode, set `mode: "push"` in the client configuration and provide the keypair of the account sending the transaction on-chain. The client will then sign a `signedHash` credential containing the transaction hash:
...
On the server, enable push mode by setting `mode: "push"` in the charge configuration. You can optionally set `rejectUnsignedPush: true` to strictly reject any push-mode credentials that use the deprecated `{ type: 'hash' }` format (legacy) instead of the new `signedHash` format:
...
const mppx = Mppx.create({
  ...
  methods: [
    stellar.charge({
      ...
      network: "stellar:testnet",
      mode: "push",
      rejectUnsignedPush: true, // reject legacy unsigned hash credentials
    }),
  ],
});
...
In push mode, the client generates a `signedHash` credential by:

1. Computing a hash of the transaction envelope
2. Creating the string `"{challenge.id}:{hash}"` where `challenge.id` is the MPP challenge identifier
3. Signing this string using the keypair of the account that sends the transaction on-chain

The server verifies the signature against the public key of the account that will broadcast the transaction, ensuring the client authorized the specific hash.

:::note The legacy `{ type: 'hash' }` credential format (unsigned hash only) is accepted in v0.7 with a deprecation warning. It will be rejected by default in v0.8 unless the server explicitly allows it via `rejectUnsignedPush: false`. New implementations should use the signed hash credential format. :::
