...
| --- | --- | --- |
| `insufficient_balance` | Client lacks USDC | Get USDC from [Circle faucet](https://faucet.circle.com/) |
| `missing_trustline` | No USDC trustline | Add trustline (see Step 3 above) |
| `invalid_signature` | Wrong key used | Verify private key matches public key |
| `simulation_failed` | Transaction would fail | Check balances and trustlines |
...
```bash
curl -i http://localhost:3001/my-service
```
