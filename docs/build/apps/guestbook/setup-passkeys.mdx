...
The `PUBLIC_WALLET_WASM_HASH` is the wasm hash generated on `stellar contract deploy...` and `PUBLIC_NATIVE_CONTRACT_ADDRESS` is the native contract address. The native contract address can be obtained from stellar-cli with the command `stellar contract id asset --asset native`.
