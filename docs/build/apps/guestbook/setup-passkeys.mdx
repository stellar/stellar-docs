...
The `PUBLIC_WALLET_WASM_HASH` is the wasm hash generated on `stellar contract deploy...` and `PUBLIC_NATIVE_CONTRACT_ADDRESS` is the native contract address. The Wasm hash identifies the executable code that will be deployed for new smart wallets.
...
- A SAC client to interact with the native XLM asset contract. The native contract address can be obtained from stellar-cli with the command `stellar contract id asset --asset native`. We're making an assumption that native lumens is a "good enough" asset interaction to get the tutorial working, and for playing on Testnet. You could easily export _another_ SAC client to interact with USDC, for example.
