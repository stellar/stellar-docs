...
- **A way to interact with a user's account:** we're foregoing traditional wallets here, and using [Smart Account Kit](https://github.com/stellar/smart-account-kit) to give each user a smart account they unlock with a passkey (Touch ID, Face ID, a hardware key, etc.). Smart Account Kit is the successor to `passkey-kit`; it targets the [OpenZeppelin Smart Account](https://docs.openzeppelin.com/stellar-contracts/accounts/smart-account) contracts and integrates natively with the OpenZeppelin Relayer for fee sponsorship.
- **A relayer for fee sponsorship:** the [OpenZeppelin Relayer](https://docs.openzeppelin.com/relayer/guides/stellar-channels-guide) (Stellar Channels plugin) submits our users' transactions on-chain without exposing them to fees or sequence-number management.
...
- The smart contract has a proper test suite, which you can read in `contracts/ye_olde_guestbook/src/test.rs`. The frontend has **no** automated testing at all. You'll want some for your own application.
...
- [Smart Account Kit](https://github.com/stellar/smart-account-kit): TypeScript SDK for creating and managing Stellar smart accounts.
- [OpenZeppelin Smart Account](https://docs.openzeppelin.com/stellar-contracts/accounts/smart-account): the contract framework each user's smart account is deployed from.
