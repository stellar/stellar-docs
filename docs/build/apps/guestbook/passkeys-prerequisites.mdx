...
We do have Mainnet Launchtube tokens available! You can request a token in the `#launchtube` channel on our Stellar Developers Discord server. In particular, pinging `@kalepail`, `@ElliotFriend`, `@carsten.xlm`, or `@Chris.anatalio.xlm` should get you on your way pretty quickly.
...
2. Get an authentication token from the Mercury website. You can login to the Testnet dashboard here. Click on the **Get access token** button under the "Active subscriptions" section. You'll be given a JWT which will be valid for the next 7 days.
...
3. **Check the Mercury documentation.** It's quite good and can help you get past a lot of the hurdles you might face.

In any case, feel free to ask questions or drop a chat in the `#passkeys` and `#launchtube` channels in the Stellar Developers Discord server. There's usually somebody around who's ready and willing to help out!
