...
- **A means of writing messages.** Users can invoke this function to leave a message for the site maintainer. They will have to authenticate this function, and it must contain a `title` and `text` field (both `String`s). The function will return the `u32` ID number of the message, which increments sequentially.
...
We'll be diving into each of the main functions below, but if you want to see the whole smart contract uninterrupted, it can be found [on Github](https://github.com/ElliotFriend/ye-olde-guestbook/blob/main/contracts/ye_olde_guestbook/src/lib.rs)
...
We're abstracting away the method we're using to write a message to the contract storage because it's used in two places: the `__constructor` and `write_message` functions. We want both to store messages in the same manner, so we're enforcing that by using this utility function.
...
This is a struct that's used elsewhere in the contract to define the keys for the various storage entries the contract will hold. Nothing groundbreaking or remarkable here, to be honest, but it's still worth showing. The `Message(u32)` variant will be used as the key to store a `Message` struct on-chain as the corresponding value.
