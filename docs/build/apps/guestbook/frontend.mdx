...
Two details in there are worth dwelling on, because they're the kind of thing that's obvious in hindsight and baffling in the moment.

The first is that `createWallet` reports **two** outcomes, and they don't deserve equal treatment. If `submitResult` failed, the wallet was never deployed and there's nothing to log in to, so we throw. If `fundResult` failed, the user has a perfectly good smart account that just happens to be empty, and there's a "Fund Wallet" button in the settings menu they can hit later. Failing signup over that would be rude, so we log a warning and carry on.

The second is `userDismissedPasskey`. A user who opens the passkey prompt and thinks better of it has not encountered an error, and telling them "something went wrong" is a small lie that makes your app feel broken. Checking for the dismissal first (and every passkey flow in this app does) is the difference between a gentle "Cancelled" toast and a scary red one.
...
Now the main event: writing a guestbook entry. The page has a simple form with title and message inputs:
...
The `signGuestbook` function is where the contract call happens:
...
async function signGuestbook() {
  ...
  try {
    ...
    toaster.error({
      title: "Error",
      description:
        "Something went wrong signing the guestbook. Please try again later.",
    });
  } finally {
    isLoading = false;
  }
...
The page component then displays the message using a shared `GuestbookMessage` component:
...
const submitEdit = async () => {
  ...
  try {
    ...
    toaster.error({
      title: "Error",
      description:
        "Something went wrong editing your message. Please try again later.",
    });
  } finally {
    isEditing = false;
    isLoading = false;
