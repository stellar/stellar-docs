...
`userDismissedPasskey` earns its keep for a similar reason. A user who opens the passkey prompt and thinks better of it has not encountered an error, and telling them "something went wrong" is a small lie that makes your app feel broken. The signup and login flows check for the dismissal first.
...
<script lang="ts">
    ...
    async function login() {
        try {
            ...
            // ...omitted: the generic "something went wrong" fallback toast
        }
    }
</script>
...
There's more happening in that one call than it lets on. The credential-to-contract lookup has to take place somewhere, and the kit handles it: it checks its local IndexedDB index first, then falls back to a hosted indexer. Have a look back at the [prerequisites](./passkeys-prerequisites.mdx) if you skipped that part, because it's the one place where "I didn't configure it" is easy to mistake for "it isn't happening."

The lookup is only half of it, though. Before the kit connects, it also verifies how that smart account was created, what contract code it's running now, and that the passkey the user just presented is a live signer on it. A failed check means no connection at all, which is why the `catch` block runs the error past `describeConnectionError` from the [setup page](./setup-passkeys.mdx#a-few-helpers) before falling back to a generic message. "Something went wrong, please try again later" is actively misleading for a passkey that is never going to work on this account.
...
This one's already done. We wrote `getNativeBalance` back on the [setup page](./setup-passkeys.mdx#a-few-helpers), and it simulates a `balance` call against the native Stellar Asset Contract. Simulation costs nothing and needs no signature, so reading a balance is about as cheap as an interaction gets.
...
For the "list all entries" page, we keep the query logic server-side in `src/lib/server/getLedgerEntries.ts`. Rather than invoking the contract once per message, it reads the ledger directly: one `rpc.getLedgerEntries()` call against the contract's instance storage to find the message count, then batched `getLedgerEntries` calls of at most 200 keys (the Stellar RPC cap).
...
export async function getAllMessages(): Promise<MessageWithIndex[]> {
  ...
  for (
    let offset = 0;
    offset < ledgerKeysArray.length;
    offset += MAX_LEDGER_KEYS
  ) {
...
This is a genuinely useful trick to have in your pocket. Because we know how the contract lays out its storage keys (a `Message(u32)` variant of the `DataKey` enum, remember), we can construct those ledger keys ourselves and ask RPC for them in batches of at most 200. See the [key-building helper and message-count read](https://github.com/ElliotFriend/ye-olde-guestbook/blob/main/src/lib/server/getLedgerEntries.ts). That file currently sends all message keys in one request; use the batching shown above for lists longer than 200 keys.
