---
title: MPP Quickstart Guide
description: Build a paid API with Node.js and Stellar settlement using the Machine Payments Protocol (MPP).
sidebar_position: 20
---

This tutorial shows how to build the simplest possible paid API with Node.js and Express using `stellar-mpp-sdk`, with settlement directly on the Stellar network — no external facilitator required.
...
```bash
npm install express stellar-mpp-sdk mppx @stellar/stellar-sdk
```
...
import { stellar } from 'stellar-mpp-sdk/server'
...
By default, the client pays Stellar network fees. To have the server pay fees on behalf of the client, configure `feePayer` on the server:
...
if (!MPP_SECRET_KEY) {
  throw new Error('Set MPP_SECRET_KEY to a strong secret for MPP credential verification')
}
...
const mppx = Mppx.create({
  ...
  methods: [
    stellar.charge({
      ...
      currency: USDC_SAC_TESTNET,
      network: 'testnet',
      feePayer: Keypair.fromSecret(FEE_PAYER_SECRET), // pays tx fees
    }),
  ],
})
```
...
When `feePayer` is set, the client automatically detects this from the challenge and signs only the Soroban auth entries (not the full transaction envelope). The server rebuilds the transaction with the fee payer's account as source and broadcasts it.

## Additional documentation

- [stellar-mpp-sdk on GitHub](https://github.com/stellar/stellar-mpp-sdk) — Full API reference, channel mode, and examples
...
- [Channel payment guide](./channel-guide.mdx) — Set up off-chain payment channels for high-frequency payments
