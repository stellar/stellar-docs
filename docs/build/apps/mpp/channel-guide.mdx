...
if (!MPP_SECRET_KEY) {
  console.error('Set MPP_SECRET_KEY to a strong secret for MPP credential verification')
  process.exit(1)
}
  ...
  methods: [
    stellar.channel({
      ...
      onProgress(event) {
        switch (event.type) {
          case 'challenge':
            console.log(`Challenge: ${event.amount} base units via channel ${event.channel.slice(0, 12)}...`)
            break
          case 'signed':
            console.log(`Commitment signed (cumulative: ${event.cumulativeAmount} base units)`)
            break
        }
      },
    }),
