...
This is a research prototype that has not been audited. Do not use it in production with real assets.
...
Confidential Tokens let users keep token balances and transaction amounts private while keeping the sender's and receiver's addresses publicly visible onchain. Confidential tokens are designed for contexts where the parties to a transaction are known, but the amounts should not be.
...
- **Efficient BN254 ZK Operations** ([CAP-0080](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0080.md)): adds BN254 G1 multi-scalar multiplication; scalar-field modular addition, subtraction, multiplication, exponentiation, and inversion; and curve-membership checks for BN254 G1 and BLS12-381 G1/G2 points.
