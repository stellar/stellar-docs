...
The [Confidential Token Association](https://www.confidentialtoken.org/) — whose members include the Stellar Development Foundation, Nethermind, OpenZeppelin, and Zama — is developing an open standard for encryption-based on-chain confidentiality compatible with existing token interfaces. Implementation on Soroban is in progress.
