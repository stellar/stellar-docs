...
Smart wallets on Stellar are currently under active development. This page is where that work is being captured.
...
If you're building or experimenting with passkeys on Stellar, some essential resources are:
...
An API service where developers can send their smart contract transactions to get them funded and submitted to the Stellar network. No classic Stellar G addresses needed!
...
- Ask in the `#launchtube` channel on [Discord](https://discord.gg/stellardev) for an access token
