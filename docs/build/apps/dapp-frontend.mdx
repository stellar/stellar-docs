...
Even doe we're using Freighter to test our app, there are more wallets that support signing soroban transactions and to make their integration easier we are using the `StellarWalletsKit` library which allows us supporting all Stellar Wallets with a single library.
...
In the code above we created an instance of the kit and two simple functions that will take care of "setting" and "loading" the public key of the user in case we want to use it elsewhere in our code. The kit is started with Freighter as the default wallet and the Testnet network as the default network, you can check all the details about how the kit works at [https://stellarwalletskit.dev/](https://stellarwalletskit.dev/)

Now we're going to add a "Connect" button to the page that opens the kit's built in modal and prompts the user to use their preferred wallet. Once the user picks its preferred wallet and grants permission to accept requests from the website, we will fetch the public key and the "Connect" button will be replaced with a message saying, "Signed in as [their public key]".
...
The code itself here is pretty self-explanatory. We import the kit from the file we created before, then when the user clicks on the button we launch the built-in modal and once the user picks their preferred wallet, we set it as the default one before requesting and saving the address.
...
- Check out a more full-featured [example app](https://github.com/stellar/soroban-example-dapp), which uses React rather than vanilla JavaScript and Next.js rather than Astro. This app also has a more complex setup & initialization process, with the option of using a [locally-hosted RPC node](../../data/rpc/admin-guide.mdx).
