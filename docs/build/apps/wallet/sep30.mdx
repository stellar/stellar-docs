...
Note that the account created above will match the first criteria. If a 2-3 schema were used, then the second criteria would match. (In a 2-3 schema, 3 servers are used and 2 of them are enough to recover the key. This is a recommended approach.)

:::note

You can also use more low-level `signWithRecoveryServers` functions to sign arbitrary transaction.
