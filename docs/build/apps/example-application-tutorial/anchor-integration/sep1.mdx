...
Soroban tokens do not require .toml files; .toml files often point to the icon for the token, so if you want your Soroban token to have an icon in Freighter or other wallets, you should [submit your token to an asset list](https://stellar.expert/directory/add).
