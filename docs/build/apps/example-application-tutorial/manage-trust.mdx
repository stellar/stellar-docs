...
<script>
  ...
  import { Server, Api, LinearSleepStrategy } from "@stellar/stellar-sdk/rpc";
  ...
  const submit = async (transaction) => {
    try {
      ...
      // Poll for the final transaction status using LinearSleepStrategy
      const finalResponse = await server.pollTransaction(sendTransactionResponse.hash, {
        sleepStrategy: new LinearSleepStrategy(500),
        attempts: 10,
      });
      ...
      switch (finalResponse.status) {
        ...
        case Api.GetTransactionStatus.NOT_FOUND:
          ...
          if (finalResponse.resultXdr) {
            try {
              const decodedResult = xdr.TransactionResult.fromXDR(finalResponse.resultXdr, 'base64');
              errorDetails += `\nResult XDR: ${JSON.stringify(decodedResult, null, 2)}`;
            } catch (xdrError) {
        ...
        default:
          throw new Error(`Unexpected transaction status: ${finalResponse.status}`);
      }
    } catch (err) {
      throw error(400, {
