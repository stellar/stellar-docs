...
This tutorial walks through how to build an application with the [`js-stellar-sdk`](https://github.com/stellar/js-stellar-sdk), to build with the Wallet SDK, please follow the [Build a Wallet tutorial](../wallet/overview.mdx). To build with smart contracts, navigate to the [Smart Contracts section](../../../build/smart-contracts/getting-started/setup.mdx).
...
- Application framework: we're using [SvelteKit](https://kit.svelte.dev) opting for JavaScript with JSDoc typing. SvelteKit is quite flexible and could be used for a lot, but we are mainly using it for its routing capabilities to minimize this being a "SvelteKit tutorial".
- Frontend framework: we're using [DaisyUI](https://daisyui.com) to simplify the use of [Tailwind CSS](https://tailwindcss.com).
- A way to interact with the Stellar network: we're using the [`js-stellar-sdk`](https://github.com/stellar/js-stellar-sdk), but you could use traditional fetch requests. The [`js-stellar-sdk`](https://github.com/stellar/js-stellar-sdk) is also used for building transactions to submit to the Stellar network.
- A way to interact with the user's keypair: we're using the [`js-stellar-wallets` SDK](https://github.com/stellar/js-stellar-wallets), but you can opt to use an existing wallet.
...
- We're deploying this as a static "single-page application" with [Cloudflare Pages](https://pages.cloudflare.com). Your own deployment decisions will have an impact on your configuration and build process.
...
- [Testnet toml file](https://testanchor.stellar.org/.well-known/stellar.toml): an example `stellar.toml` file that demonstrates what information an anchor might publish
- [BasicPay dev helpers](https://basicpay.pages.dev/dashboard/settings/dev): if you're _using_ the BasicPay application, we've created a few helpful tools to help you explore its functionality

## Getting started

Here are the steps we've taken to start building BasicPay. Feel free to be inspired and customize these directions as you see fit. The entire [BasicPay codebase](https://github.com/stellar/basic-payment-app) is freely open and available on GitHub for reference.
