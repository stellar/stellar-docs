...
## Implement secure architecture and secure by design principles

[Insecure design](https://owasp.org/Top10/A04_2021-Insecure_Design) is a category of web application security risks, which calls for more use of threat modeling, secure design patterns, and reference architectures. Developers should move beyond "shift-left" in the coding space to pre-code activities that are critical for the principles of Secure by Design and [secure architecture](https://top10proactive.owasp.org/the-top-10/c4-secure-architecture). For more information on this great modeling, see the [relevant Stellar docs](https://developers.stellar.org/docs/build/security-docs/threat-modeling).

## SSL/TLS

Ensure that the up-to-date version of Transport Layer Security (TLS) is enabled. Redirect HTTP to HTTPS where necessary to ensure that Man in the Middle (MITM) attacks can’t occur and sensitive data is securely transferred between the client and browser. Enable TLS and get an SSL certificate for free at [LetsEncrypt](https://letsencrypt.org/getting-started/).

If you don’t have SSL/TLS enabled, stop everything and do this first. You can also use a tool like [SSL Labs SSL Server Test](https://www.ssllabs.com/ssltest) to check that your TLS version is up-to-date.
...
We strongly prefer 2FA and require U2F or [Time-Based One-Time Password Algorithm (TOTP)](https://tools.ietf.org/html/rfc6238) 2FA for sensitive actions. 2FA is important as email accounts are usually not very secure. Having a second factor of authentication ensures that users who accidentally stay logged on or have their password guessed are still protected.
...
## Protection against phishing attacks and social engineering
...
Bad actors may also use phishing, and other types of social engineering techniques, to target developers or others who are involved in blockchain development.
...
A web application injection attack occurs when an attacker supplies untrusted input that the application mistakenly treats as code or commands, causing the app to execute unintended actions. This can let the attacker alter program behavior, access or corrupt data, or perform unauthorized operations.

Injection is listed as [third category on the 2021 Open Worldwide Application Security Project (OWASP) Top 10 web application security risks](https://owasp.org/Top10/A03_2021-Injection).

Here’s a breakdown of each web application injection vulnerability, along with recommended mitigations to help safeguard your systems.

### Cross-site Scripting (XSS)

**Description:** XSS occurs when an attacker injects malicious scripts into content that is then rendered by a user’s browser. These scripts can steal session cookies, redirect users, or manipulate page content. It typically exploits the lack of proper input/output encoding in web applications.

**Types:**

- **Stored XSS:** Malicious script is permanently stored on the server (e.g., in a database).
- **Reflected XSS:** Script is reflected off a web server (e.g., via a URL parameter).
- **DOM-based XSS:** Script is executed due to client-side JavaScript manipulation of the DOM.

**Mitigations:**

- Use context-aware output encoding (e.g., HTML, JavaScript, URL encoding).
- Implement Content Security Policy (CSP) to restrict script execution.
- Sanitize user input using trusted libraries (e.g., [DOMPurify](https://www.npmjs.com/package/dompurify)).
- Avoid inserting untrusted data directly into the DOM.
- Use frameworks that auto-escape output (e.g., React, Angular).

### SQL Injection

**Description:** SQL Injection allows attackers to manipulate SQL queries by injecting malicious input into form fields, URLs, or headers. This can lead to unauthorized data access, data modification, or even full system compromise.

**Mitigations:**

- Use parameterized queries or prepared statements. For example, this can be done with PHP Data Objects (PDO), Object-Relational Mapping (ORM) frameworks).
- Avoid dynamic SQL construction using string concatenation.
- Validate and sanitize input (e.g., enforce data types, length).
- Implement least privilege for database accounts.
- Monitor and log database activity for anomalies.

### External Control of File Name or Path

**Description:** This vulnerability arises when user input is used to construct file paths or names without proper validation. Attackers can exploit this to access unauthorized files (e.g., via path traversal like `../../etc/passwd`) or overwrite critical files.

**Mitigations:**

- Never trust user input for file paths or names.
- Use whitelisting to restrict allowed file names or directories.
- Normalize and validate paths using secure APIs (e.g., `realpath()`).
- Restrict file access to specific directories using chroot or sandboxing.
- Implement access controls and audit file operations.
...
Most modern web and mobile frameworks handle CSRF protection. Ensure CSRF protection is enabled. You can reference this [cheat sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html) for further guidance on CSRF prevention.
