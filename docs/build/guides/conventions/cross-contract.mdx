...
All public functions of a contract can be called. Using a network explorer can be helpful as some propose to see the Rust interphase of a contract. Bindings can also be generated using the CLI:
...
The Wasm can be fetched by using the [Stellar CLI](../../../tools/developer-tools/cli). This can serve as a quick way to (for example) automate a hash check in a continuous integration system. However, such a check would not provide strong on-chain guarantees, but one could build a contract for that!
