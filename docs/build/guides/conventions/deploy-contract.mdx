---
title: Contract Deployment from Installed Wasm Bytecode
...
- `deploy_contract(ContractExecutable::Wasm(wasm_hash), constructor_args)` deploys the contract using the provided Wasm bytecode hash, invokes its constructor with `constructor_args`, and returns the address of the newly deployed contract.
