...
We'll start off by installing the React library and then walk through creating the initialization script and finally wrap up with using the template to build, deploy, and generate contract bindings.
...
After running the initialization script, there should be new `packages` directory which confirms successful bindings generation. Navigate to the `packages` directory and there should be a package for the deployed contract(s).
This package will form the last piece in solving the puzzle of interacting with soroban contracts from frontend appications which will be discussed briefly.
...
Navigate to the packages folder to find the hello wold package and observe the directory contents. If the directory does not contain a `dist` folder, you must build the hello world typescript bindings for the package to work in our react app.

```bash
npm run build
```

After running the `build` command, there should now be a `dist` directory containing `index.js` and `index.ts` files. These will serve as an entry point to the package when it is eventually imported.
...
Now we instantiate a the `Client` class and pass an object containing the RPC url and a copy of the `networks.testnet` object by using the spread operator.
This technically means that the `Client` class takes in an object of the network configuration along with the contract ID but we make it easier by using the `networks` object.
...
To put it all together, modify the `App` component in the `src` directory by replacing its content with the code below.
...
Alot has been covered from creating template files to building, deploying, generating bindings, and interacting with smart contracts. Now go ahead and play witht the code and try applying the procedures to other contracts.
