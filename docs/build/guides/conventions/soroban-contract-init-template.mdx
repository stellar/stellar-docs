...
```bash
stellar contract init hello_contract -w hello -f https://github.com/CaptainPrinz/soroban-react-template
```
...
Navigate to the packages folder to find the hello world package and observe the directory contents. If the directory does not contain a `dist` folder, you must build the hello world typescript bindings for the package to work in our react app.

To build the typescript bindings for the package, navigate to the `packages` directory, then enter `hello_world` package directory and run the command below.
