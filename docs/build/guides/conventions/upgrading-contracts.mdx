...
Navigate to to `upgradeable_contract/new_contract` and run the following command:
...
You also need to call the `init` method so the `admin` is set. This requires us to setup some [identities].

[identities]: ../../../smart-contracts/getting-started/setup#configure-an-identity
