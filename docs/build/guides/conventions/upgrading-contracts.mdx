...
When upgrading a contract, the key function used is `env.deployer().update_current_contract`, which takes a `ContractExecutable` (wrap the new contract's Wasm hash in `ContractExecutable::Wasm(...)`) as a parameter. Here's a step-by-step breakdown of how this process works:
