...
This process uploads the contract code to the testnet in a transaction, the uploaded Wasm blob is a [contract source](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0046-02.md#contract-source), which can be thought of as a 'class' of a contract. Multiple [instances of a contract can be deployed](./deploy-contract.mdx) which share the same source, but have their own state.
...
Different programming languages can be used to upload the `.wasm` file, its SHA256 hash is used for deploying contract.

<Tabs>

<TabItem value="js" label="Javascript">
This guide will walk you through installing the wasm of the contract using the [js-stellar-sdk](https://github.com/stellar/js-stellar-sdk) Javascript SDK.
...
This guide will walk you through installing the wasm of the contract using the [py-stellar-base](https://stellar-sdk.readthedocs.io/en/soroban/) Python SDK.

Create a new python script, and install necessary dependencies:
...
Ensure that you handle secrets and private keys securely in production environments and never expose them in your code repositories.
