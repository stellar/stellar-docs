...
This process uploads the contract code to the testnet in a transaction, which is needed before [deploying the contract](./deploy-contract.mdx).
...
We can use one of the SDKs:

1. [Node.js](https://nodejs.org/en) and npm (for running JavaScript install scripts)
2. [Python](https://www.python.org/downloads/) and [pip](https://pip.pypa.io/en/stable/installation/)
...
Different programming languages can be used to upload the `.wasm` file, its SHA256 hash can be used in deploying contract.
...
This guide will walk you through installing the wasm of the contract using [py-stellar-base](https://stellar-sdk.readthedocs.io/en/soroban/) Python SDK.
