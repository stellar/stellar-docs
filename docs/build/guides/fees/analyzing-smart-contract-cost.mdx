...
## How to optimize smart contract cost: {#how-to-optimize-smart-contract-cost}
...
### 1. Efficient loop and storage calls usage {#1-efficient-loop-and-storage-calls-usage}
...
#### Not Optimal Contract ❎ {#not-optimal-contract-}
...
### 2. Proper use of batch operations {#2-proper-use-of-batch-operations}
...
#### Not Optimal Contract ❎ {#not-optimal-contract--1}
...
#### Optimal Contract ✅ {#optimal-contract--1}
...
### 3. Use of events over storage {#3-use-of-events-over-storage}
...
#### Optimized Using Events {#optimized-using-events}
...
### 4. Use of efficient data structures {#4-use-of-efficient-data-structures}
...
##### Example (Heap Allocated Array) ❎ {#example-heap-allocated-array-}

```rust
let mut v1 = alloc::vec![];
```

##### Example (Fixed-Sized Array) ✅ {#example-fixed-sized-array-}
...
##### Example (Inefficient Data Storage) ❎ {#example-inefficient-data-storage-}
...
##### Alternative: using keyed vecs ✅ {#alternative-using-keyed-vecs-}
...
### 5. Use of appropriate `env.storage` mechanisms {#5-use-of-appropriate-envstorage-mechanisms}

There are three types of storage mechanisms in Stellar:

#### [`env.storage().persistent()`](https://docs.rs/soroban-sdk/latest/soroban_sdk/storage/struct.Storage.html#method.persistent) {#envstoragepersistent}
...
### [`env.storage().temporary()`](https://docs.rs/soroban-sdk/latest/soroban_sdk/storage/struct.Storage.html#method.temporary) {#envstoragetemporary}
...
### [`env.storage().instance()`](https://docs.rs/soroban-sdk/latest/soroban_sdk/storage/struct.Storage.html#method.instance) {#envstorageinstance}
...
### 5. Contract size {#5-contract-size}
...
## Other tips to optimize smart contract cost: {#other-tips-to-optimize-smart-contract-cost}

### 1. Use of built-in tools {#1-use-of-built-in-tools}
...
### 2. Manual code review {#2-manual-code-review}
...
### 3. Unit testing with gas measurements {#3-unit-testing-with-gas-measurements}
...
### 5. Reconsidering storage locations {#5-reconsidering-storage-locations}
