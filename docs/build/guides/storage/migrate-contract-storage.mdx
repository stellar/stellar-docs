...
Suppose a contract stores `DataV2` entries and is upgraded to use `DataV1`, dropping field `c`:
...
Built with `soroban-sdk` v28 or newer, reading an old `DataV2` entry directly as `DataV1` now succeeds: the `c` key in the stored map is not a field of `DataV1`, so it's ignored.

```rust
pub fn read_data(e: Env, id: u32) -> Option<DataV1> {
    e.storage().persistent().get(&DataKey::Data(id))
}
```
...
The discarded field isn't remembered. If the value is later packed and written back to storage, the write includes only the fields `DataV1` has, and the entry's `c` value is permanently lost. A routine read followed by a write - not just an explicit migration - discards it.
