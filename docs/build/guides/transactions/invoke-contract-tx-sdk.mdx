...
/**
 * Generate a random keypair and fund it on Testnet
 */
async function generateFundedKeypair() {
  const keypair = Keypair.random();
  await fetch(`https://friendbot.stellar.org/?addr=${
    keypair.publicKey()
  }`)
  return keypair
}
