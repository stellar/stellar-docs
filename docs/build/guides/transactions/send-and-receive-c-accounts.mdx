---
title: "Send to and receive payments from C Accounts"
description: "Learn to send payments to and receive payments from C Accounts on the Stellar network."
sidebar_position: 7
---
...
Smart Wallets use C addresses on Stellar. Payments between C and G addresses are supported on a protocol level, but there are differences in the implementations that are required to support them. This guide will cover implementations for applications that use classic G addresses and want to support transacting with C addresses.

## Receiving payments from C-accounts (C to G)

If a sender is using a C address, and sending an [SAC token](https://developers.stellar.org/docs/tokens/stellar-asset-contract) (such as XLM, USDC, etc.), the transaction will be received by the G address normally since it is supported on a protocol level.

### Receiving payments from C-accounts that require a memo
...
It is important to note the 'asset_balance_changes' section only supports Stellar Assets and transactions that make use of the `transfer` function in the [Stellar Asset Contract (SAC)](https://developers.stellar.org/docs/tokens/stellar-asset-contract).

**Example response from a C address:**
...
To monitor these payments, the method '[getEvents](https://developers.stellar.org/docs/data/apis/rpc/api-reference/methods/getEvents)' of the Stellar RPC is used to stream events emitted by the Stellar Asset Contract. A filter can be used to specify the target asset (SAC contract id) and accounts.
...
## Sending payments to C accounts (G to C)

### Sending a ‘transfer’ via Stellar RPC {#sending-a-‘transfer’-via-stellar-rpc}

This approach is simpler but requires the use of a RPC and involves a slightly different transaction workflow than a traditional native operation. For a detailed code example, see the section ‘_Sending Payments through the SAC \-\> Using the Stellar RPC_’ of [this repository's examples](https://github.com/fazzatti/c-address-payment-examples#sending-payments-through-the-sac).
...
When the ‘source’ account of the transaction differs from the ‘sender’ account, an additional signed entry will be necessary to authorize the ‘transfer’ invocation explicitly. Refer to the [Soroban Authorization Framework documentation](https://developers.stellar.org/docs/learn/fundamentals/contract-development/authorization#soroban-authorization-framework) for further details.
