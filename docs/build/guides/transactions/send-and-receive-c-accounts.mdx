...
If a sender is using a Contract Account, and sending an [SAC token](https://developers.stellar.org/docs/tokens/stellar-asset-contract) (such as XLM, USDC, etc.), the transaction will be received by the G account normally since it is supported on a protocol level.
