...
```python
from stellar_sdk import Account, Asset, Keypair, Network, SorobanServer, TransactionBuilder
from stellar_sdk.exceptions import AccountNotFoundException, SorobanRpcErrorResponse

rpc = SorobanServer("https://soroban-testnet.stellar.org")
source_key = Keypair.from_secret("SBRS7YP7CNB55BHU5VRLGNCWC7PC5GCTA2TCFDY7ZWG3CHATMFN4APBF") # GAKKNB3W22SU4IZL6A2XHXMXY2KQH2G35OTOPURESA7BSQO63JQOXOWH
destination_id = "GBQJCVIBBAKRXHRID5UGD3T3VF3MAAAEPNIYAB6RE3FKWA5BR3Z2BDRH"

# First, check to make sure that the destination account exists.
# You could skip this, but if the account does not exist, you will be charged
# the transaction fee when the transaction fails.
try:
    rpc.load_account(destination_id)
...
try:
    source_account = rpc.load_account(source_key.public_key)
except AccountNotFoundException:
    # If the account is not found, surface an error message for logging.
    raise Exception("The source account does not exist!")


# Let's fetch the mode (most common) inclusion fee from the network
mode_fee = rpc.get_fee_stats().inclusion_fee.mode

# The next step is to parametrize and build the transaction object:
# - Using the source account we just loaded we begin to assemble the transaction.
# - We set the network passphrase to TESTNET.
# - We also set the fee to the mode fee we just got from the server.
transaction = (
    TransactionBuilder(
    ...
    # We then add a payment operation to the transaction oject. This operation
    # will send 10 native XLM to the destination account.
    .append_payment_op(destination=destination_id, asset=Asset.native(), amount="10")
    # A memo allows you to add your own metadata to a transaction. It's optional
    # and does not affect how Stellar treats the transaction. These memos are
    # oftentimes used to identify the transaction when working with pooled
    # accounts or to facilitate reconciliation.
    .add_text_memo("Test Transaction")
    # Finally, we set a timeout for the transaction. This means that the
    # transaction will not be valid anymore after 180 seconds.
    .set_timeout(180)
    .build()
)
...
# And finally, send it off to the Stellar network!
try:
    # The send_transaction method immediately returns a reply with the
    # transaction hash and the status "PENDING". This means the transaction was
    # received and is being processed.
    send_tx_response = rpc.send_transaction(transaction)

    # Here we check the status of the transaction as there are a possible
    # outcomes after sending a transaction that would have to be handled
    # accordingly, such as "DUPLICATE" or "TRY_AGAIN_LATER".
    if send_tx_response.status.value != "PENDING":
        raise Exception(
            f"Failed to send transaction, status: {send_tx_response.status.value}"
        )

    # The poll_transaction method will return the final status of the
    # transaction after the specified number of attempts or when the transaction
    # is finalized. We then check the final status of the transaction and handle
    # it accordingly.
    poll_tx_response = rpc.poll_transaction(
        transaction_hash=send_tx_response.hash,
        max_attempts=5, # defaults to 30
        sleep_strategy=lambda _x: 2, # defaults to 1 second between polling
    )

    print(f"Status: {poll_tx_response.status.value}, Hash: {poll_tx_response.transaction_hash}")
except (SorobanRpcErrorResponse) as err:
...
try:
    rpc.load_account(destination_id)
    # validate the account, if needed
except AccountNotFoundException:
    raise Exception("The destination account does not exist!")
...
try:
    ...
    raise Exception("The source account does not exist!")
# use source_account to build the transaction
...
try:
    ...
    # the poll_transaction function comes with sensible defaults for number of
    # attempts and sleep strategy.
    poll_tx_response = rpc.poll_transaction(send_tx_response.hash)
...
E.g.: Generating the bindings for the `sac` contract of a given asset:
...
# TypeScript
 ...
 # Python (https://github.com/lightsail-network/stellar-contract-bindings)
 stellar-contract-bindings python --contract-id CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC --rpc-url https://soroban-testnet.stellar.org --output ./sac_bindings
```
...
```python
from stellar_sdk import Account, Asset, Keypair, Network, SorobanServer
from stellar_sdk.exceptions import AccountNotFoundException
from sac_bindings.bindings import Client
...
# Now we initialize the Stellar Asset Contract (SAC) client. The client needs
# the RPC endpoint, network passphrase, contract ID for the asset, and your
# account's public key. A signing function can also be provided to automatically
# handle transaction signing when invoking contract methods with this client.
#
# In this case, we are using the native asset contract ID for the testnet. The
# contract ID is derived from the native asset, which is represented by
# `Asset.native()`. The `contractId` method will return the contract ID for the
# native asset on the specified network.
xlm_client = Client(
    Asset.native().contract_id(Network.TESTNET_NETWORK_PASSPHRASE),
    rpc_url="https://soroban-testnet.stellar.org",
    network_passphrase=Network.TESTNET_NETWORK_PASSPHRASE,
)

# Now, using the client, we assemble a soroban transaction to invoke the
# transfer function of the native asset contract. The client will automatically
# bundle the operation and simulate the transaction before providing us with an
# assembled transaction object that we can sign and send.
# assembled_tx: AssembledTransaction
try:
    assembled_tx = xlm_client.transfer(
        source=source_key.public_key,
        from_=source_key.public_key,
        to=destination_id,
        amount=10 * 10_000_000, # amount in stroops (1 XLM = 10,000,000 stroops)
    )
except:
    raise Exception(f"Error assembling transaction")

# The assembled transaction is ready to be signed and sent. It already includes
# a function that allows us to perform the signing and sending in one step. It
# will use the signTransaction function we provided to the client to sign the
# transaction or alternatively, receive a custom one just for this transaction.
try:
    # the sign and send function adds a response to the assembled transaction
    # object, so we don't need to assign the response to anything.
    assembled_tx.sign_and_submit(source_key)
    print(f"Transaction successful: {assembled_tx.send_transaction_response.hash}")
except:
    raise Exception("Error during transaction submission")
```
...
In the python bindings, the signer, public key, and some other options are provided when the contract's function is invoked. Not when the client is instantiated, as is the case in TypeScript.
...
```js
import { Server } from "@stellar/stellar-sdk/rpc";
import { xdr, Asset, Networks, Address } from "@stellar/stellar-sdk";
...
from stellar_sdk import scval, Address, Asset, Network, SorobanServer
...
# Get the native XLM contract ID for testnet
contract_id = Asset.native().contract_id(Network.TESTNET_NETWORK_PASSPHRASE)

# The address we want to monitor for incoming payments
monitored_address = Address("GBQJCVIBBAKRXHRID5UGD3T3VF3MAAAEPNIYAB6RE3FKWA5BR3Z2BDRH")

# Paging state for event polling
last_ledger_start: int | None = None
paging_token: str | None = None

def poll_for_transfers():
    global last_ledger_start
    global paging_token

    # Set starting ledger if not set (get the latest ledger as starting point)
    if last_ledger_start is None:
        latest_ledger = rpc.get_latest_ledger()
        last_ledger_start = latest_ledger.sequence
    print(f"> Monitoring transfers for ledger: {last_ledger_start}")

    # Get events for "transfer" topic from the native asset contract
    response = rpc.get_events(
        start_ledger=last_ledger_start if not paging_token else None,
        cursor=paging_token,
        filters=[
            EventFilter(
                ...
                # Filter for transfer events to the monitored address
                # Using wildcards (*) to match any sender and asset
                # Event structure: ["transfer", from_address, to_address, asset_name]
                topics=[
                    [
                        scval.to_symbol("transfer").to_xdr(),
                        "*",
                        monitored_address.to_xdr_sc_val().to_xdr(),
                        "*",
                    ],
                ],
    ...
    # Update paging tokens for next poll
    paging_token = None
    if response.latest_ledger:
        last_ledger_start = response.latest_ledger

    # Process events and check for payments to our monitored address
    for event in response.events:
        try:
            topics = event.topic
            print(f"Processing event: {event.transaction_hash} at ledger {event.ledger}")

            if topics and len(topics):
                # extract the recipient address from event topics
                to_address = scval.from_address(topics[2]).address

                # check if the payment is to our monitored address
                if (to_address == monitored_address.address):
                    print("Payment Received!")
                    print(f"\tTransaction: {event.transaction_hash}")
                    print(f"\tLedger: {event.ledger}")
                    print(f"\tSender: {scval.from_address(topics[2]).address}")
                    print(f"\tAmount: {scval.from_int128(event.value)}")
        except Exception as err:
            print(f"Error processing event: {err}")

        # Update paging token for next poll
        paging_token = event.id
...
# Start monitoring for payment events
print(f"Starting payment monitor for {monitored_address.address}")
poll_for_transfers()
```
...
response = rpc.get_events(
    ...
    filters=[
        EventFilter(
            contractIds=[contract_id],
            # Event structure: ["transfer", from_address, to_address, asset_name]
            # Using wildcards (*) allows mathing any value in that position
            topics=[
                [
                    scval.to_symbol("transfer").to_xdr(),
                    "*",
