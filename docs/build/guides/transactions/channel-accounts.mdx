...
Submitting transactions quickly [often requires](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0006.md) hot signing keys. It is best practice to extensively test your systems on [Testnet](../../../networks/README.mdx#network-comparison) before deploying in high-speed production. Moreover, you may consider using hardware security modules or [multisignature wallets](../../../learn/fundamentals/transactions/signatures-multisig.mdx#thresholds) to secure your accounts.
...
Stellar [validators](../../../validators/README.mdx) are spread across the globe, so transactions from one source can lag and arrive out of order. Even if you sequence and propose a transaction set yourself, consensus [shuffles](../../../learn/fundamentals/transactions/transaction-lifecycle.mdx#8-transaction-apply-order-is-determined-validator-network) the final joined order. Accordingly, channel accounts use independent parallel streams to guarantee layer-1 settlement for many consecutive transactions.
...
sourceKeypair = Keypair.from_secret(secretKey)
sourceAccount = server.load_account(sourceKeypair.public_key)
...
// Attempting to send 200 operations at once from a single source
for (let n = 0; n < 100; n++) {
  transaction1.addOperation(Operation.payment({
    destination: first100Users[n],
    asset: Asset.native(),
    amount: "10"
  }));
  transaction2.addOperation(Operation.payment({
    destination: second100Users[n],
    asset: Asset.native(),
    amount: "10"
  }));
...
public static void main(String[] args) {
  try {
    ...
    TransactionBuilder transactionBuilder1 = new TransactionBuilder(sourceAccount, Network.TESTNET)
      .setBaseFee(100)
      .setTimeout(3600);

    TransactionBuilder transactionBuilder2 = new TransactionBuilder(sourceAccount, Network.TESTNET)
      .setBaseFee(100)
      .setTimeout(3600);
    ...
    for (int n = 0; n < 100; n++) {
      transactionBuilder1.addOperation(
        PaymentOperation.builder()
          .destination(first100Users.get(n))
      ...
      transactionBuilder2.addOperation(
        PaymentOperation.builder()
          .destination(second100Users.get(n))
          .asset(new AssetTypeNative())
          .amount(new BigDecimal("10"))
          .build()
      );
...
func main() {
  ...
  txOps1 := []txnbuild.Operation{}
  txOps2 := []txnbuild.Operation{}
  ...
  for n := 0; n < 100; n++ {
    ...
    txOps1 = append(txOps1, &paymentOp1)
    ...
    txOps2 = append(txOps2, &paymentOp2)
  }

  txParams.Operations = txOps1
  transaction1, _ := txnbuild.NewTransaction(txParams)
  txParams.Operations = txOps2
  transaction2, _ := txnbuild.NewTransaction(txParams)

  tx1, _ := transaction1.Sign(network.TestNetworkPassphrase, sourceKeypair)
  tx2, _ := transaction2.Sign(network.TestNetworkPassphrase, sourceKeypair)
...
The simple solution of sending all 500 payments from $A_P$ would be rate-limited and prone to sequence number errors. Accordingly, we can split the operations up between five channel accounts. Each channel account submits transactions containing 100 payment operations.

This approach ensures that only the channel accounts submit to the network. The primary account retains secure custody over the assets, and it could preauthorize the transfers offline. With this approach, you can safely thread all channel accounts to submit transactions concurrently in a hot app.

:::info

For our example, we'll assume the goal is high throughput rather than operational delegation. In the [delegation examples](../../../learn/fundamentals/transactions/signatures-multisig.mdx#example-3-expense-accounts) only a single hot signer may be necessary. Accordingly, let us use an [abnormally-high](https://horizon.stellar.org/fee_stats) base fee of `640` for inclusion in an example ledger.
...
const main = async () => {
  ...
  for (const transaction of txOutput) {
    try {
...
for (int i = 0; i < channelAccounts.size(); i++) {
  ...
  TransactionBuilder transactionBuilder = new TransactionBuilder(channelAccount, Network.TESTNET)
    .setBaseFee(Transaction.MIN_BASE_FEE);

  for (String recipient : allRecipients.get(i)) {
    transactionBuilder.addOperation(
      PaymentOperation.builder()
        .sourceAccount(primaryKeypair.getAccountId())
...
func main() {
  ...
  for channelIndex, channelAccount := range channelAccounts {
    ...
    _, err = server.SubmitTransaction(tx)
    check(err)
  }
}
```
...
The above example assumes that all accounts are all properly configured on the network. Unless of course the payments themselves are [XLM](../../../learn/glossary.mdx#lumen), $A_P$ needs the least lumens to cover transaction fees, but you still need to fund the account with minimal base reserves. In contrast, $A_{C_{1\text{--}5}}$ need the most lumens since they pay for all transactional costs.
...
while recipientIndex < len(allRecipients):
  ...
  for channelData in channelAccountsTracker:
    ...
    if channelData["state"] == "available":
      ...
      txOutput.append((txBundle, channelData))
    else:
  ...
  for bundle, channelData in txOutput:
    try:
      ...
      for op in bundle.transaction.operations:  # Assuming only payments
...
await Promise.all(
  txOutput.map(async ({transaction, channelData}) => {
    try {
      const response = await server.submitTransaction(transaction);
      console.log(`Transaction succeeded with hash: ${response.hash}`);
    } catch (error) {
...
public static void main(String[] args) {
  // Dynamic recipients' public keys
  List<String> allRecipients = List.of("GD72...B2D", "GDX7...G7M", "GBI3...Q4V", "GBH2...N6E", "GCJY...OJ2");

  // index i = 0-4 in our example
  List<ChannelAccount> channelAccountsTracker = new ArrayList<>();
  for (int i = 0; i < channelAccounts.size(); i++) {
    channelAccountsTracker.add(new ChannelAccount(channelAccounts.get(i), channelKeypairs.get(i), "available"));
  }

  List<TransactionBundle> txOutput = new ArrayList<>();
  int recipientIndex = 0;
  while (recipientIndex < allRecipients.size()) {
    for (ChannelAccount channelData : channelAccountsTracker) {
      if (recipientIndex >= allRecipients.size()) break;
      if (channelData.state.equals("available")) {
        channelData.state = "active";
        TransactionBuilder txBuilder =
          new TransactionBuilder(channelData.accountResp, Network.TESTNET).setBaseFee(640);
        ...
        for (int i = 0; i < 100; i++) {
          ...
          txBuilder.addOperation(
            PaymentOperation.builder()
              .sourceAccount(primaryKeypair.getAccountId())
              .destination(allRecipients.get(recipientIndex))
              .asset(new AssetTypeNative())
              .amount(new BigDecimal("10"))
              .build()
          );
...
static class ChannelAccount {
  ...
  ChannelAccount(AccountResponse accountResp, KeyPair keypair, String state) {
    this.accountResp = accountResp;
...
If you fill a ledger with your own transactions, you can expect to pay [much higher fees](../../../learn/fundamentals/fees-resource-limits-metering.mdx#surge-pricing) than with dynamic bundle sizes that spread operations over time. This chiefly depends on the rate at which you submit transactions. As long as you aren't consistently above 100 operations per ledger, each channel can just submit a transaction each ledger.

![Filling Ledger](/assets/channel-accounts/congesting.png)

:::note[Block Size]

You can create as many channel accounts as needed to maintain your desired transaction rate. However, one Mainnet ledger [can only fit](../../../networks/README.mdx#network-comparison) 1,000 operations from all peers. To keep [fees](../../../learn/fundamentals/fees-resource-limits-metering.mdx#inclusion-fee) within reason and minimize congestion, it is best practice to limit yourself to 300 operations per ledger. For higher throughput in non-emergency applications, consider using other scaling solutions like [Starlight](../../../learn/glossary.mdx#starlight).
...
While we've discussed sending lumens directly to the channel accounts, you can instead have the primary account sponsor $A_C$ transaction fees. In a custodial solution, you may prefer that channel accounts hold no assets at all, maintaining trustlines with [sponsored reserves](./sponsored-reserves.mdx), for example. It is best practice to simply leave channels with adequate funding, but you can use payments from $A_P$ to $A_C$ to cover fees each transaction.

One less direct way is to use [fee-bump transactions](./fee-bump-transactions.mdx) for each batch. Instead of funding channels, the primary account can wrap each channel transaction with a signed outer envelope and act as its fee source. The inner transaction still uses and increments its channel account's sequence number, even when $A_P$ is also the source of the payment operations, because operation sources do not supply the transaction sequence number.
