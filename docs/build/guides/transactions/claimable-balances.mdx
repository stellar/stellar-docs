...
- **`ClaimableBalanceID`**: ClaimableBalanceID is a union with one possible type (`CLAIMABLE_BALANCE_ID_TYPE_V0`). It contains an SHA-256 hash of the [OperationID](https://github.com/stellar/stellar-protocol/blob/75703bbad9700e2691d89817f0a11f0339d5a6ee/core/cap-0023.md?plain=1#L356) for claimable balances.
- **`ClientBalanceID`**: A client-facing string form of `ClaimableBalanceID` (what you pass into claim/clawback operations). It's commonly represented as the 72-character "API format" hex string (type prefix like `00000000` + 64 hex chars, left-padded with zeros to 72 characters); some tools/SDKs also accept a StrKey (base32check) representation that starts with `B` (e.g. `BA...`). See [SEP-0023](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0023.md) for StrKey encoding details, and use your SDK's helpers (for example `StrKey.encodeClaimableBalance()` / `StrKey.decodeClaimableBalance()` in `@stellar/stellar-sdk`). The `ClaimClaimableBalance` operation uses this to claim the `ClaimableBalanceEntry`.
...
<Details summary="Go Helper Functions">
