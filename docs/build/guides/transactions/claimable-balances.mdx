...
- **`ClaimableBalanceID`**: ClaimableBalanceID is a union with one possible type (`CLAIMABLE_BALANCE_ID_TYPE_V0`). It contains an SHA-256 hash of the [OperationID](https://github.com/stellar/stellar-protocol/blob/75703bbad9700e2691d89817f0a11f0339d5a6ee/core/cap-0023.md?plain=1#L356) for claimable balances.
  - **String representations**:
    - **Hex ("API format")**: 72 hex characters: a 4-byte type prefix (usually `00000000` for v0) plus the 32-byte hash (64 hex chars).
    - **StrKey (`B...`)**: base32check encoding of the same underlying value, starting with `B` (e.g. `BA...`). See [SEP-0023](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0023.md) for details.
- **`ClientBalanceID`**: The client-facing identifier you pass into claim/clawback operations. Depending on the API/tooling, it may be provided/accepted in either of the string forms above; in this guide we show the hex form.
