...
| --- | --- | --- |
| Transaction signing | G-accounts | G-accounts who fully trust and know how to thoroughly verify the authenticity and security of the transactions. <br />📌 **Note:** This is the only method supported by Stellar Classic. |
...
7. **Fee-payer** simulates (Enforcing Mode) to catch errors before paying fees
8. **Fee-payer** signs the transaction envelope and submits
...
The **fee-payer** must simulate in Enforcing Mode, and the **client** is strongly recommended to simulate as well:
...
Mor info on fee bump transactions can be found in the [Fee-bump transactions](./fee-bump-transactions.mdx) guide.
