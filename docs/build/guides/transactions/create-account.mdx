...
public final class CreateAccountExample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String friendbotUrl = networkInfo != null ? networkInfo.getFriendbotUrl() : null;
    check(friendbotUrl != null && !friendbotUrl.isBlank(), "Friendbot URL not provided by network");
    requestFunds(friendbotUrl, parent.getAccountId());
  ...
  private static SendTransactionResponse sendTransaction(Transaction transaction)
      throws IOException {
    SendTransactionResponse response = SERVER.sendTransaction(transaction);
    check(response.getStatus() == SendTransactionStatus.PENDING,
        "Transaction submission failed: " + response.getStatus());
    return response;
  }
