...
```python
# # The SDK does not have tools for creating test accounts, so you'll have to
# make your own HTTP request. In Python, import the `requests` library for this.
import requests
...
```python
# Querying data from an RPC server often makes use of the `getLedgerEntries` RPC
# method. Creating the keys for those ledger entries, is done with XDR objects.
from stellar_sdk import SorobanServer, xdr

server = SorobanServer("https://horizon-testnet.stellar.org")
public_key = "GD4NB2FLQAN5JO7PKPGZJMNBDYQXVSNVC7DEIZMOL5WSNSBLEBUTEF5Q"

# You can use `load_account` to get just the account ID and sequence number,
# suitable for creating and submitting transactions.
account = server.load_account(public_key)
print(f"Account ID: {account.account.account_id}, Sequence: {account.sequence}")

# Additional information, such as extra signers, balances (XLM only), etc. can
# be retrieved using the account's ledger entry itself.
...
response = server.get_ledger_entries([account_ledger_key])
entry = xdr.LedgerEntryData.from_xdr(response.entries[0].xdr).account

# Note there is additional "drilling" that must be done due to the types
# returned from the RPC server.
print(f"XLM Balance: {entry.balance.int64}, Sequence: {entry.seq_num.sequence_number.int64}, Signers: {len(entry.signers)}")
```
