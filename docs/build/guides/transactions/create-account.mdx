...
```python
# The SDK does not have tools for creating test accounts, so you'll have to
# make your own HTTP request. In Python, import the `requests` library.
import requests
...
```python
# Retrieving data from an RPC server often makes use of the `getLedgerEntries`
# RPC method. Creating the keys which are used to query those ledger entries is
# done with XDR objects.
...
# which is enough information for creating and submitting transactions.
...
print(f"XLM Balance: {account.balance.int64}, Sequence: {account.seq_num.sequence_number.int64}, Signers: {len(account.signers)}")
```
