...
(async function main() {
  try {
    const response = await server.requestFunds(pair.publicKey());
...
public static void main(String[] args) throws Exception {
  ...
  String friendbotResponse = requestAirdrop(parent.getAccountId());
  ...
  GetTransactionResponse txResp = pollTransaction(sendResp.getHash());
  ...
  BigInteger sacAmount = getSacBalance(
    TESTNET_USDC_CONTRACT,
    new AssetTypeNative(),
    Network.TESTNET.getNetworkPassphrase());
  log("XLM in USDC contract (raw): %s", sacAmount.toString());
}

private static String requestAirdrop(String accountId) throws IOException, InterruptedException {
...
```java
private static BigInteger getSacBalance(
  String contractId, org.stellar.sdk.Asset asset, String networkPassphrase
) throws IOException {
  byte[] targetContract = StrKey.decodeContractId(contractId);
  byte[] sacContractId = StrKey.decodeContractId(asset.contractId(networkPassphrase));

  LedgerKey key = new LedgerKey();
  key.setDiscriminant(LedgerEntryType.CONTRACT_DATA);
  LedgerKeyContractData contractDataKey = new LedgerKeyContractData();
  contractDataKey.setContract(new SCAddress(SCAddressType.SC_ADDRESS_TYPE_CONTRACT, sacContractId));
  contractDataKey.setKey(buildSacBalanceKey(targetContract));
  contractDataKey.setDurability(ContractDataDurability.PERSISTENT);
  key.setContractData(contractDataKey);
  ...
  LedgerEntryData data = LedgerEntryData.fromXdr(response.getEntries().get(0).getXdr());
  if (data.getContractData() == null || data.getContractData().getVal().getMap() == null) {
    return BigInteger.ZERO;
  }
  for (SCMapEntry entry : data.getContractData().getVal().getMap().getEntries()) {
    boolean isBalanceKey = entry.getKey().getDiscriminant() == SCValType.SCV_SYMBOL
        && "Balance".equals(new String(entry.getKey().getSym().getBytes()));
    if (isBalanceKey) {
      return scValToBigInteger(entry.getVal());
    }
  }
  return BigInteger.ZERO;
}

private static SCVal buildSacBalanceKey(byte[] holderContractId) {
  SCVal symbol = new SCVal();
  symbol.setDiscriminant(SCValType.SCV_SYMBOL);
  symbol.setSym(new SCSymbol("Balance".getBytes()));

  SCVal holder = new SCVal();
  holder.setDiscriminant(SCValType.SCV_ADDRESS);
  holder.setAddress(new SCAddress(SCAddressType.SC_ADDRESS_TYPE_CONTRACT, holderContractId));

  SCVec vec = new SCVec();
  vec.setScVec(new SCVal[]{symbol, holder});

  SCVal key = new SCVal();
  key.setDiscriminant(SCValType.SCV_VEC);
  key.setVec(vec);
  return key;
}

private static BigInteger scValToBigInteger(SCVal val) {
  if (val.getDiscriminant() == SCValType.SCV_I128) {
    Int128Parts parts = val.getI128();
    BigInteger hi = BigInteger.valueOf(parts.getHi().getInt64());
    BigInteger lo = unsignedLongToBigInteger(parts.getLo().getUint64());
