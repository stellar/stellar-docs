...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let public_key = "GBCEKZ6HJA636XRDWT2Y5TSX3AN62MCFDXKJAX2BNGVZADO63R427ANG";

    // This SDK only uses the RPC (not horizon), we need to get the data using ledger keys.
    let account_id = Keypair::from_public_key(public_key)?.xdr_account_id();
    ...
    if let xdr::LedgerEntryData::Account(account) = response.entries.unwrap()[0].to_data() {
        // The balance is in stroops, convert to XLM by dividing by operation::ONE (10000000)
        let balance = account.balance / operation::ONE;
        println!("{public_key} holds {balance} XLM");
    }

    // To fetch other balances, you need to fetch them directly.
    // For example here with ABC:GISSUER...
    let code = "ABC";
    let issuer = public_key;
    let asset = Asset::new(code, Some(issuer))?.into();
    let account_id = Keypair::from_public_key(public_key)?.xdr_account_id();
    ...
    if let Some(entries) = response.entries {
        if !entries.is_empty() {
            if let xdr::LedgerEntryData::Trustline(trustline) = entries[0].to_data() {
                // The balance is in stroops, convert to XLM by dividing by operation::ONE (10000000)
                let balance = trustline.balance / operation::ONE;
                println!("{public_key} holds {balance} {code}:{issuer}");
            }
        } else {
            println!("{public_key} does not have a trustline for {code}:{issuer}");
        }
    }
