...
(async function main() {
  ...
  try {
    const response = await s.requestAirdrop(pair.publicKey());
  ...
  try {
    //make sure the parent account exists on ledger
    var parentAccount = await server.getAccount(pair.publicKey());

    //generate a random account to create create a transacion object.
...
// the JS SDK uses promises for most actions, such as retrieving account data
