...
Once sponsorships are introduced, the minimum balance calculation is: (2 base reserves + `numSubEntries` + `numSponsoring` - `numSponsored`) \* `baseReserve`.

Selling liabilities are not part of the minimum balance. They reduce what an account can spend above it: `available balance` = `balance` - `minimum balance` - `liabilities.selling`.
