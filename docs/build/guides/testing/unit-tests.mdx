...
Test utilities such as `Address::generate`, `env.auths()`, `env.set_config()`, `env.to_snapshot()`, `env.to_ledger_snapshot()`, and `env.register()` are only usable on the `Env` created by the test itself. As of soroban-sdk v27.0.7, calling them from inside a contract's own function body panics, because the `Env` the host passes into a contract invocation carries none of the test's state. Prior versions did not panic and instead silently operated on empty or default state, which could produce confusing results.
