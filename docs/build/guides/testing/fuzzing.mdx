...
The following steps can be used in any Stellar contract workspace. If experimenting, try them in the [increment example](https://github.com/stellar/soroban-examples/blob/main/increment/src/lib.rs). The contract has an `increment` function that increases a counter value by one on every invocation.
...
7. Fill out the `fuzz_target!` call with test setup and assertions. For example, for the [increment example](https://github.com/stellar/soroban-examples/blob/main/increment/src/lib.rs):
...
This test uses the same patterns used in [unit tests](./unit-tests.mdx) and [integration tests](./integration-tests.mdx):
...
For a full detailed example, see the [fuzzing example](../../smart-contracts/example-contracts/fuzzing.mdx).

:::

:::info

There is another tool for fuzzing Rust code, `cargo-afl`. See the [Rust Fuzz book](https://rust-fuzz.github.io/book) for a tutorial for how to use it.
...
4. Run the llvm-cov command to convert the profdata file to an lcov file.
   ...
   Load the `lcov.info` file into your IDE using its coverage feature. In VSCode this can be done by installing the [Coverage Gutters](https://marketplace.visualstudio.com/items?itemName=ryanluker.vscode-coverage-gutters) extension and executing the `Coverage Gutters: Watch` command.

:::tip

To measure code coverage of regular Rust tests, see [Code Coverage](code-coverage.mdx).
