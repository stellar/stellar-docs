...
2. `proptest` generates values from `Strategy`s, not from `Arbitrary` implementations, so it cannot directly consume the `SorobanArbitrary::Prototype` types used for fuzzing above. The [proptest-arbitrary-interop] crate's `arb` function bridges the gap, turning any `Arbitrary` type, including every `SorobanArbitrary::Prototype`, into a `Strategy`.
