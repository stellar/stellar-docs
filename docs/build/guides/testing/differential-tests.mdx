...
All contracts built with the Rust Soroban SDK have a form of differential testing built-in and enabled by default. See [Differential Testing with Test Snapshots](./differential-tests-with-test-snapshots.mdx).
...
1. Use the [stellar contract fetch](../../../tools/cli/stellar-cli.mdx#stellar-contract-fetch) command to fetch the contract that's already deployed. The contract already deployed will be used as a baseline that the local copy is expected to behave like.
...
3. Write a test that runs the same logic for the deployed contract and the local contract, comparing the result. Assuming the [increment example](https://github.com/stellar/soroban-examples/blob/main/increment/src/lib.rs) is in use, the test would look something like the following.
