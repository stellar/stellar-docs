...
The [Soroban Rust SDK] makes it just as easy to test against a real contract as it does to test against a mock of a contract. In some ecosystems integration tests are avoided. Not in the Stellar ecosystem. Learn more in [Integration Tests].
...
Learn more in [Integration Tests] for how to test with real dependencies.

The [Soroban Rust SDK] handles contract calls defensively so that any unexpected error or unexpected type returned from the called contract will cause execution to stop. The SDK also provides methods to make these calls and to intercept error situations. Learn more in [Making Cross-Contract Calls].
