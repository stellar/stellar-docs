...
The [Soroban Rust SDK](../../../tools/sdks/contract-sdks.mdx#soroban-rust-sdk) makes it just as easy to test against a real contract as it does to test against a mock of a contract. In some ecosystems integration tests are avoided. Not in the Stellar ecosystem. See [Integration Tests](./integration-tests.mdx).

:::

## How to Write Tests with Mocks

The following is an example of a test that uses a mock, written to test the [increment-with-pause contract](https://github.com/stellar/soroban-examples/blob/main/increment_with_pause/src/lib.rs). The contract has an `increment` function that increases a counter value by one on every invocation. The contract depends on another contract that controls whether the increment functionality is paused.
...
See [Integration Tests](./integration-tests.mdx) for how to test with real dependencies.

The [Soroban Rust SDK](../../../tools/sdks/contract-sdks.mdx#soroban-rust-sdk) handles contract calls defensively so that any unexpected error or unexpected type returned from the called contract will cause execution to stop. The SDK also provides methods to make these calls and to intercept error situations. See [Making Cross-Contract Calls](../conventions/cross-contract.mdx) for more details.
