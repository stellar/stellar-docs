...
Tests should can be used as assert on any auths that are expected to be required. A simple example can be found in the [auth example contract](../../smart-contracts/example-contracts/auth.mdx). (In the following code block, some code has been omitted for brevity.)
