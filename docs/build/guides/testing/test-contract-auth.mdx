...
`env.auths()` must be called from the test function, not from inside a contract's own function body. As of soroban-sdk v27.0.7, calling it inside a contract function panics. See [Unit Tests](./unit-tests.mdx) for details.
