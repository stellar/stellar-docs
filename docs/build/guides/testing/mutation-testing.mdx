...
Mutation testing is similar to measuring [code coverage], sharing the same goal to identify code not covered by tests. But where code coverage focuses on checking if a line of code is executed during a test, mutation testing will actually check that the line is tested because a test fails when the line is replaced with something else. A line of code can look like it is covered by tests, but the outcomes and side-effects themselves such as contract function return value, or events published, may not be asserted on in the tests.
