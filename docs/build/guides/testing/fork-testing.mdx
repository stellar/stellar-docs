...
Fork testing is another form of [integration test](./integration-tests.mdx), where not only mainnet contracts can be used, but also their data on mainnet. A snapshot of the ledger is taken and used to create an environment for testing. The test operates as a fork of that initial state.
...
The [Soroban Rust SDK](../../../tools/sdks/contract-sdks.mdx#soroban-rust-sdk) and [Stellar CLI](../../../tools/cli/README.mdx) come together to make possible testing with mainnet data.

## How to Write Tests with Mainnet Data

The following is an example of a test that includes a dependency contract into the test, rather than mock it. The test is written to test the [increment-with-pause contract](https://github.com/stellar/soroban-examples/blob/main/increment_with_pause/src/lib.rs) and the [pause contract](https://github.com/stellar/soroban-examples/blob/main/pause/src/lib.rs). The contract has an `increment` function that increases a counter value by one on every invocation. The contract depends on the pause contract to control whether the increment functionality is paused.
