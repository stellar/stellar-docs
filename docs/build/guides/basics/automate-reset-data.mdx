...
3. `createLiquidityPool(server, accountKeypair, nativeAsset, customAsset, networkParaphrase)`:
...
4. `createLiquidityPool(server, accountKeypair, nativeAsset, customAsset, networkPassphrase)`:

Creates a liquidity pool asset, Sets up a trust line for the pool and Deposits initial liquidity into the pool.

5. `deployAndInvokeContract(server, deployer, networkPassphrase, contractWasmFilePath, salt)`:
