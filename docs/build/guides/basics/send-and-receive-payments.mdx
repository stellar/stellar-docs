...
Most of the time, you’ll be sending money to someone else who has their own account. For this tutorial, however, you'll need a second account to transact with. So before proceeding, follow the steps outlined in [Create an Account](./create-account.mdx) to make _two_ accounts: one for sending and one for receiving. We'll also be using both native operations as well as Smart Contract invocations to perform payments. For more details about the different types of transactions, see the [Operations and Transactions](../../../learn/fundamentals/transactions) documentation.
...
To learn more about the differences between Stellar assets and contract tokens, see the [Tokens](https://developers.stellar.org/docs/tokens) overview.
...
await rpc.getAccount(destinationId).catch((error) => {
...
const sourceAccount = await rpc
  .getAccount(sourceKeys.publicKey())
  .catch((error) => {
    if (error instanceof StellarSdk.NotFoundError) {
      throw new Error("The destination account does not exist!");
    } else throw error;
...
try {
  ...
  const result = await rpc
    ...
    // We can use this hash to poll the transaction status later.
    .then((reply) => {
