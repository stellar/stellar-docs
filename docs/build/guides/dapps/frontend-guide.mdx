...
The value gotten from the transaction result is stored in a form called `scval` due to the limited [number of types](../../../learn/encyclopedia/contract-development/types/built-in-types.mdx) that exist in soroban today. This value is transmitted about in the form of `xdr` which can be converted to an `scVal` using the sdk. The value is then parsed as `u32` which is a 32-bit unsigned integer. We will learn more about converting these types when [working with contract specs](../conventions/work-contractspec-js.mdx)
