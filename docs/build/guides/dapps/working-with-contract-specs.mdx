...
## Working with the JavaScript SDK and AssembledTransaction

The JavaScript SDK provides a powerful abstraction called `AssembledTransaction` that simplifies working with Soroban smart contracts. When you use the contract client generated from TypeScript bindings, every contract method returns an `AssembledTransaction` instance.

### What is AssembledTransaction?

`AssembledTransaction` is a wrapper class that handles the complete lifecycle of a Soroban transaction:
...
Here's a comprehensive example of working with AssembledTransaction:
...
const { signTransaction } = basicNodeSigner(keypair, Networks.TESTNET);

// Example 1: Basic contract invocation
async function basicExample() {
  // Calling a contract method returns an AssembledTransaction
  const tx = await contract.increment({
    user: keypair.publicKey(),
    value: 5,
  });

  // Option 1: Sign and send in one step
  const result = await tx.signAndSend({ signTransaction });
  console.log("Result:", result.result);

  // Option 2: Separate sign and send steps
  const signedTx = await tx.sign({ signTransaction });
  const sentTx = await signedTx.send();
  console.log("Transaction hash:", sentTx.sendTransactionResponse.hash);
}

// Example 2: Simulation without sending
async function simulationExample() {
  const tx = await contract.getValue({ user: keypair.publicKey() });

  // For read-only operations, you can get the result without signing
  const simulationResult = await tx.simulate();
  console.log("Simulated result:", simulationResult.result);
}
...
async function errorHandlingExample() {
  try {
    ...
    if (result.getTransactionResponse.status === "FAILED") {
      console.error("Transaction failed:", result.getTransactionResponse);
      // Parse error from result
      const error = result.parseError();
...
async function eventsExample() {
  ...
  // Access events emitted by the contract
  if (result.getTransactionResponse.status === "SUCCESS") {
    const meta = result.getTransactionResponse.resultMetaXdr;
    // Parse events from the transaction meta
    const events = result.getTransactionResponse.events || [];
...
// Example 5: Custom transaction options
...
async function batchOperationsExample() {
  ...
  const operations = [
    { method: "approve", args: { spender: "GBXX...", amount: BigInt(1000) } },
    { method: "transfer", args: { to: "GCYY...", amount: BigInt(500) } },
  ];

  for (const op of operations) {
    const tx = await contract[op.method](op.args);
    const result = await tx.signAndSend({ signTransaction });
    console.log(`${op.method} result:`, result.result);
  }
}
```

### AssembledTransaction Methods

The `AssembledTransaction` class provides several useful methods:

- **`simulate()`**: Simulates the transaction without requiring signatures
- **`sign(options)`**: Signs the transaction and returns a new AssembledTransaction
- **`send()`**: Sends an already-signed transaction to the network
- **`signAndSend(options)`**: Combines signing and sending in one step
- **`parseError()`**: Extracts and parses error information from a failed transaction
- **`getTransactionResponse`**: Access the full transaction response after sending

### Best Practices

1. **Use TypeScript bindings**: Generate TypeScript bindings for your contracts to get full type safety:
...
2. **Handle errors gracefully**: Always check transaction status and handle failures:
...
3. **Use simulation for read operations**: For view functions that don't modify state:

   ```typescript
   const tx = await contract.getBalance({ account: "GBXX..." });
   const balance = await tx.simulate();
   console.log("Balance:", balance.result);
   ```

4. **Manage transaction fees**: Be aware of resource fees and set appropriate limits:
   ```typescript
   const result = await tx.signAndSend({
     signTransaction,
     fee: "10000", // Set maximum fee you're willing to pay
   });
   ```
...
### Working with Complex Types in JavaScript

When working with the JavaScript SDK and AssembledTransaction, you'll often need to handle complex Soroban types. Here's how to work with various data structures:

```typescript
import { Client, basicNodeSigner } from "@stellar/stellar-sdk/contract";
import { Keypair, Networks, scval, xdr } from "@stellar/stellar-sdk";

// Example: Working with Maps and Vectors
async function complexTypesExample() {
  const contract = new Client({
    contractId: "YOUR_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  ...
  // Working with a Map parameter
  const mapData = new Map([
    ["key1", "value1"],
    ["key2", "value2"],
  ]);

  // Working with a Vector parameter
  const vectorData = ["item1", "item2", "item3"];

  const tx = await contract.processData({
    dataMap: mapData,
    dataVector: vectorData,
    timestamp: BigInt(Date.now()),
  });

  const result = await tx.signAndSend({ signTransaction });
  console.log("Complex operation result:", result.result);
}

// Example: Working with Custom Structs
async function structExample() {
  const contract = new Client({
    contractId: "YOUR_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  ...
  // TypeScript bindings will generate the correct types for your structs
  ...
  const tx = await contract.createProfile({ profile });
  const result = await tx.signAndSend({ signTransaction });
}
...
async function enumExample() {
  ...
  // TypeScript bindings will generate enum types
  enum Status {
    Pending = "Pending",
    Active = "Active",
    Completed = "Completed",
...
### Advanced AssembledTransaction Patterns

```typescript
// Example: Transaction with authorization
async function authorizationExample() {
  ...
  const tx = await contract.authorizedTransfer({
    from: keypair.publicKey(),
    to: "GBXX...",
    amount: BigInt(1000),
  });

  // The transaction may require authorization entries
  const simulationResult = await tx.simulate();

  // Sign authorization entries if needed
  if (simulationResult.auth && simulationResult.auth.length > 0) {
    // Authorization entries will be automatically handled by signAndSend
    const result = await tx.signAndSend({
      signTransaction,
      // Additional signers if needed for multi-sig scenarios
      additionalSigners: [additionalKeypair],
    });
  }
}

// Example: Restoring expired contract data
async function restoreExample() {
  ...
  try {
    const tx = await contract.getData({ key: "important_data" });

    // If the data has expired, restore it first
    const result = await tx.signAndSend({
      signTransaction,
      restore: true, // Automatically restore expired entries
    });
  } catch (error) {
    if (error.message.includes("HostStorageError")) {
      console.log("Data needs to be restored");
      // Handle restoration logic
    }
  }
}

// Example: Optimizing gas and fees
async function gasOptimizationExample() {
  ...
  const tx = await contract.heavyComputation({ input: BigInt(42) });

  // Get simulation results to estimate resources
  const simulation = await tx.simulate();

  console.log("Estimated CPU instructions:", simulation.instructions);
  console.log(
    "Estimated memory bytes:",
    simulation.readBytes + simulation.writeBytes,
  );

  // Set a custom fee based on simulation
  const result = await tx.signAndSend({
    signTransaction,
    fee: String(simulation.minResourceFee * 2), // Add buffer for safety
  });
}

// Example: Working with contract events in detail
async function detailedEventsExample() {
  const contract = new Client({
    contractId: "YOUR_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  ...
  if (result.getTransactionResponse.status === "SUCCESS") {
    ...
    for (const event of events) {
      ...
      const decodedTopics = event.topics.map((topic) =>
        scval.fromXDR(topic, "base64"),
      );

      const decodedData = event.data
        ? scval.fromXDR(event.data, "base64")
        : null;
...
### Real-World Example: Token Contract Integration

Here's a complete example of integrating with a Stellar Asset Contract (SAC) using AssembledTransaction:
...
async function tokenContractExample() {
  // Initialize SAC client for USDC on testnet
  ...
  const sender = Keypair.fromSecret("SENDER_SECRET_KEY");
  const { signTransaction } = basicNodeSigner(sender, Networks.TESTNET);

  // 1. Check balance
  const balanceTx = await tokenClient.balance({
    id: sender.publicKey(),
  });
  const balance = await balanceTx.simulate();
  console.log("Current balance:", balance.result);

  // 2. Transfer tokens
  const recipient = "GBXX...";
  const amount = BigInt(1000000); // 1 USDC (assuming 6 decimals)

  const transferTx = await tokenClient.transfer({
    from: sender.publicKey(),
    to: recipient,
    amount: amount,
  });

  const transferResult = await transferTx.signAndSend({ signTransaction });

  if (transferResult.getTransactionResponse.status === "SUCCESS") {
    console.log("Transfer successful!");

    // 3. Check new balance
    const newBalanceTx = await tokenClient.balance({
      id: sender.publicKey(),
    });
    const newBalance = await newBalanceTx.simulate();
    console.log("New balance:", newBalance.result);
  }

  // 4. Approve spending allowance
  const spender = "GCYY...";
  const allowance = BigInt(5000000); // 5 USDC

  const approveTx = await tokenClient.approve({
    from: sender.publicKey(),
    spender: spender,
    amount: allowance,
    expiration_ledger: 1000000, // Optional expiration
  });

  await approveTx.signAndSend({ signTransaction });

  // 5. Check allowance
  const allowanceTx = await tokenClient.allowance({
    from: sender.publicKey(),
    spender: spender,
  });
  const currentAllowance = await allowanceTx.simulate();
  console.log("Spender allowance:", currentAllowance.result);
}
```
...
```typescript
// JavaScript/TypeScript example with AssembledTransaction
import { Client, basicNodeSigner } from "@stellar/stellar-sdk/contract";
import { Keypair, Networks } from "@stellar/stellar-sdk";

async function incrementExample() {
  // Initialize the contract client
  const contract = new Client({
    contractId: "YOUR_INCREMENT_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  });
  ...
  // Call increment - returns AssembledTransaction
  const tx = await contract.increment();

  // Sign and send the transaction
  const result = await tx.signAndSend({ signTransaction });

  // The result contains the new counter value
...
async function helloExample() {
  // Initialize the contract client
  const contract = new Client({
    contractId: "YOUR_HELLO_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  });
  ...
  // Call hello with a string parameter - returns AssembledTransaction
  const tx = await contract.hello({ to: "John" });

  // Sign and send the transaction
  const result = await tx.signAndSend({ signTransaction });

  // The result is automatically converted from ScVec<ScString> to string[]
  console.log("Hello result:", result.result); // ["Hello", "John"]
}
```
...
```typescript
// JavaScript/TypeScript - AssembledTransaction handles response conversion automatically
import { Client, basicNodeSigner } from "@stellar/stellar-sdk/contract";
import { Keypair, Networks } from "@stellar/stellar-sdk";
...
async function getResponseExample() {
  ...
  const tx = await contract.hello({ to: "World" });

  // Sign and send - AssembledTransaction handles all the complexity
  const result = await tx.signAndSend({ signTransaction });

  // The result is automatically decoded from ScVec<ScString> to string[]
  console.log("Decoded result:", result.result); // ["Hello", "World"]

  // You can also access the raw transaction response if needed
  console.log("Transaction hash:", result.sendTransactionResponse.hash);
  console.log("Transaction status:", result.getTransactionResponse.status);

  // For manual transaction tracking, you can use the hash
  const txHash = result.sendTransactionResponse.hash;

  // Later, you can manually check the transaction
  const { SorobanRpc } = await import("@stellar/stellar-sdk");
  const server = new SorobanRpc.Server("https://soroban-testnet.stellar.org");
  const txResponse = await server.getTransaction(txHash);

  if (txResponse.status === "SUCCESS") {
    // AssembledTransaction already handled this parsing for you
    console.log("Manual check - success!");
  }
}
```
