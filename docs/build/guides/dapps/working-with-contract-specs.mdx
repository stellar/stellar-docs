...
The JSON code block shows the contract spec, while RUST code blocks show the contract for each example. TypeScript examples demonstrate usage with the JavaScript SDK's `AssembledTransaction` pattern.

:::

### JavaScript SDK and AssembledTransaction
...
#### Advanced patterns with AssembledTransaction

```typescript
import { Client, basicNodeSigner } from "@stellar/stellar-sdk/contract";
import { Keypair, Networks } from "@stellar/stellar-sdk";

// Example: Working with authorization
...
1. **Generate TypeScript bindings** for full type safety:
   ```bash
...
3. **Use simulation for read operations**:
   ```typescript
...
```typescript
// TypeScript with AssembledTransaction
interface State {
  count: number;
  last_incr: number;
}

// The contract client handles conversion automatically
...
```typescript
// TypeScript with AssembledTransaction
// Tuple structs are represented as arrays
...
// Usage with contract client
const tx = await contract.setEnum({
  value: { tag: "A", values: undefined }
});
// Or with a value
const tx2 = await contract.setEnum({
  value: { tag: "B", values: [42] }
});
...
```typescript
// JavaScript/TypeScript with AssembledTransaction
import { Client, basicNodeSigner } from "@stellar/stellar-sdk/contract";
import { Keypair, Networks } from "@stellar/stellar-sdk";

// TypeScript types generated from contract bindings
type TimeBoundKind = 
  | { tag: "Before", values: void }
  | { tag: "After", values: void };
...
async function depositWithTimelock() {
  const contract = new Client({
    contractId: "YOUR_TIMELOCK_CONTRACT_ID",
    networkPassphrase: Networks.TESTNET,
    rpcUrl: "https://soroban-testnet.stellar.org",
  });
  ...
  // Create a time bound that expires after a certain timestamp
  const timeBound: TimeBound = {
    kind: { tag: "After", values: undefined },
    timestamp: BigInt(Date.now() / 1000 + 3600) // 1 hour from now
  };

  // Call the deposit function with complex parameters
...
- For JavaScript/TypeScript developers, `AssembledTransaction` simplifies the entire contract interaction process
