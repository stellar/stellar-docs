...
## What are contract specs? {#what-are-contract-specs}
...
## Generating contract specs {#generating-contract-specs}
...
Sometimes, you may not have access to the contract's source code or the ability to compile it. In such cases, you must use the [`stellar contract fetch`](../../../tools/developer-tools/cli/stellar-cli.mdx#stellar-contract-fetch) command to download the contract's Wasm binary and generate the spec.

Finally, we use the [`stellar bindings`](../../../tools/developer-tools/cli/stellar-cli.mdx#stellar-contract-bindings-json) command to generate the contract spec from the Wasm binary.

### Fetching the contract binary {#fetching-the-contract-binary}
...
### Generating the contract spec from Wasm {#generating-the-contract-spec-from-wasm}

```bash
stellar contract bindings json --wasm contract.wasm > abi.json
```

## Understanding the contract specification {#understanding-the-contract-specification}
...
## Working with native Soroban types {#working-with-native-soroban-types}
...
### 1. Invoking a contract function with no parameters {#1-invoking-a-contract-function-with-no-parameters}
...
### 2. Invoking a contract function with one or more parameters {#2-invoking-a-contract-function-with-one-or-more-parameters}
...
### 3. Getting responses from contracts {#3-getting-responses-from-contracts}
...
Ideally, to retrieve this value, we need to use the [`getTransaction`](../../../data/rpc/api-reference/methods/getTransaction.mdx) RPC method using the response hash of the transaction that invoked the contract function.
...
### Struct with named fields {#struct-with-named-fields}
...
### Struct with unnamed fields {#struct-with-unnamed-fields}
...
### Enum (unit and tuple variants) {#enum-unit-and-tuple-variants}
...
### Enum (integer variants) {#enum-integer-variants}
...
## Reading contract events {#reading-contract-events}

Reading contract events is similar to reading transaction results. You can use the [`getEvents`](../../../data/rpc/api-reference/methods/getEvents.mdx) RPC method to get the list of events associated with a contract.
