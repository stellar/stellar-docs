...
1. Create the Ledger Footprint
2. Build a transaction that includes the `extendFootprintTtl` operation with your target TTL and attach the Soroban transaction data (using the footprint).
...
async function extendPersistentEntryTTL(contractId, storageKey, sourceKeypair) {
  ...
  const sorobanData = new StellarSdk.SorobanDataBuilder()
    ...
    .build();
  transaction.sorobanData = sorobanData;
...
async function extendContractCodeTTL(contractId, sourceKeypair) {
  ...
  let transaction = new StellarSdk.TransactionBuilder(account, {
    ...
    .addOperation(
      StellarSdk.Operation.extendFootprintTtl({
        extendTo: 500_000, //The number of ledgers past the LCL by which to extend
        footprint: footprint,
      }),
    )
    .setTimeout(30)
