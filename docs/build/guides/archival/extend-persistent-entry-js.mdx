...
In Stellar, each Soroban operation must be the only Soroban operation in a single transaction. A single `ExtendFootprintTTLOp` extends _every_ entry listed in the transaction's read-only footprint, applying the same `extendTo` target to all of them, so multiple ledger entries can be extended together in one operation. The real constraint is that shared `extendTo` offset: because one operation applies a single target, entries that need _different_ target TTLs must be split across separate operations (and therefore separate transactions). The example below extends the contract's code and a persistent entry in two separate transactions, which is only required when they need different targets — entries sharing the same target can instead be placed together in one footprint.
