...
When interacting with assets in a smart contract, the [Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx) is not different from any other token that implements the Stellar [SEP-41 Token Interface](../../../tokens/token-interface.mdx).
...
E.g. for USDC, it would be `--asset USDC:G...` For the native asset, XLM, `--asset native`. See the [deploy SAC](../../../tools/cli/cookbook/deploy-stellar-asset-contract.mdx) guide for more details.

:::info[Clients]

A client created by [`token::TokenClient`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.TokenClient.html) implements the functions defined by any contract that implements the [SEP-41 Token Interface](../../../tokens/token-interface.mdx). But with [CAP-46-6 smart contract standardized asset](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0046-06.md), the Stellar Asset Contract exposes additional functions such as `mint`. To access the additional functions, another client needs to be used: [`token::StellarAssetClient`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.StellarAssetClient.html). This client only implements the functions from CAP-46-6, which are not part of the SEP-41 interface.
