...
Since we haven't locked down the signer(s) of our issuer account, it's still entirely possible for the `GCS5...` issuer account to create and submit to the network any `payment` operations it wants. The `Admin` address of the SAC and the signing permissions of the account operate and function independently of one another.
