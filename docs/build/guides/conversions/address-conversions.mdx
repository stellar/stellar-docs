...
The default string format for `Address` is so called 'string key' (or 'strkey'), a short base64-encoded string. For the default externally owned accounts strkey always starts with `G` and for all the contracts it starts with `C`. The exact details on the format can be found [here](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0023.md), but they are not necessary to understand this section.
...
const scValBuffer = scVal.toBuffer();
