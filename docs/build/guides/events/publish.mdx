...
A more realistic example can be found in the way the [token interface](../../../tokens/token-interface.mdx) works. For example, the interface requires an event to be published every time the `transfer` function is invoked, with the following information:
