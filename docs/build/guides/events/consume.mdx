...
Once events have been ingested into a database, for instance as done in the [ingest guide](ingest.mdx), they can be consumed without having the need to query again Stellar RPC. In the following, we will show how we can consume these events.

Let's get started!

## First, get some events in a DB

Continuing right where we left in the [ingest guide](ingest.mdx), we will use the ORM models to add a few more events.
...
Remember that events published by Soroban are XDR encoded. We can use [stellar-sdk](https://stellar-sdk.readthedocs.io) to convert back and forth between values and XDR representation.
...
While we are at it, we can make the results more readable or usable in Python by using the conversion helper provided by [stellar-sdk](https://stellar-sdk.readthedocs.io).
