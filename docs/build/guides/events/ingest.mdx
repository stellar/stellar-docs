...
First, we'll need to query the events from Stellar RPC. This simple example makes an RPC request using the [`getEvents` method](../../../data/apis/rpc/api-reference/methods/getEvents.mdx), filtering for all `transfer` events that are emitted by the native XLM contract.
