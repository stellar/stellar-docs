...
The `testutils` are automatically enabled inside [Rust unit tests](https://doc.rust-lang.org/rust-by-example/testing/unit_testing.html) inside the same crate as your contract. If you write tests from another crate, you'll need to require the `testutils` feature for those tests and enable the `testutils` feature when running your tests with `cargo test --features testutils` to be able to use those test utilities.
...
Configuring a `release-with-logs` profile can be useful if you need to build a `.wasm` file that has logs enabled for printing debug logs when using the [`stellar-cli`](setup.mdx#install-the-stellar-cli). Note that this is not necessary to access debug logs in tests or to use a step-through-debugger.
...
See the [logging example](../example-contracts/logging.mdx) for more information about how to log.
