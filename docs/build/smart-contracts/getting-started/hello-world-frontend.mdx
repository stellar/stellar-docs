...
This tutorial wired a frontend to a contract by hand so you could see each piece: generate the bindings, install the package, keep the deployed contract and the generated client in sync. That loop is the same on every project, and [Stellar Scaffold](../../../tools/scaffold-stellar.mdx) exists to automate it — it scaffolds the frontend, regenerates type-safe TypeScript clients whenever your contracts change, and manages deployments across networks, so you spend your time on the app instead of the plumbing between contract and UI.
