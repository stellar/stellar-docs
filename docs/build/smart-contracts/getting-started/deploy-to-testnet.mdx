...
Friendbot can also fund contract addresses directly. If your deployed contract needs a native asset balance (for example, to pay for its own transactions or storage rent), you can fund it using the same Friendbot URL with the contract's C... address.
