...
The [Simple Account example] implements the smallest possible contract account: each `require_auth` call delegates to one ed25519 public key. It shows how to store that key, run `__check_auth`, and surface authorization failures. Use this as the baseline before moving to the [Complex Account example](./complex-account.mdx) for multisig or policy enforcement.
...
`__check_auth` runs whenever another contract invokes `require_auth` on this account. The implementation loads the stored key, verifies the signature, and panics on failure so the upstream `require_auth` call rejects. Once you need multiple keys or policy logic, follow the same pattern shown in Complex Account.
...
`try_invoke_contract_check_auth` mimics the host path for `require_auth`, so the test proves both the success case and a failure case with random bytes. Follow the same structure for any account: create a keypair, call `init`, invoke `try_invoke_contract_check_auth` for the passing and failing scenarios, and assert on the results.

## Build the Contract

To produce the WASM artifact, run:
