---
title: Contract Deployment and Initialization from Another Contract
sidebar_label: Deployer
description: Deploy a smart contract on behalf of any other address using the SDK deployer() function and initialize it atomically.
sidebar_position: 70
---
...
[deployer example]: https://github.com/stellar/soroban-examples/tree/main/deployer
...
Ref: https://github.com/stellar/soroban-examples/tree/main/deployer
...
The `deploy_contract()` function performs the actual deployment using the provided executable, here `ContractExecutable::Wasm(wasm_hash)`. The implementation of the new contract is defined by the Wasm file uploaded under `wasm_hash`. `constructor_args` are the arguments that will be passed to the constructor of the contract that is being deployed. If the deployed contract has no constructor, empty argument vector should be passed.
...
Open the [`deployer/deployer/src/test.rs`](https://github.com/stellar/soroban-examples/tree/main/deployer/deployer/src/test.rs) file to follow along.
...
The deployment requires authorization from the admin. As mentioned above, the authorization necessary for `deploy_contract` function is performed on behalf of the deployer contract and is implicit. This can be verified in the test by examining `env.auths()`.
