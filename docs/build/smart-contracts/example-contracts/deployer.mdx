...
The `deploy_contract()` function performs the actual deployment using the provided `ContractExecutable`. Wrapping the `wasm_hash` in `ContractExecutable::Wasm(...)` selects deployment from an uploaded Wasm file. `constructor_args` are the arguments that will be passed to the constructor of the contract that is being deployed. If the deployed contract has no constructor, empty argument vector should be passed.
