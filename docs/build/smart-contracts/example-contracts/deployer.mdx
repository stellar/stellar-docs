...
The `deploy_contract()` function performs the actual deployment using the provided executable. Wrapping `wasm_hash` in `ContractExecutable::Wasm(...)` tells the deployer to use the Wasm file uploaded under `wasm_hash` as the implementation of the new contract. `constructor_args` are the arguments that will be passed to the constructor of the contract that is being deployed. If the deployed contract has no constructor, empty argument vector should be passed.
