...
The [workspace example](https://github.com/stellar/soroban-examples/tree/v23.0.0/workspace) demonstrates how multiple smart contracts can be developed, tested, and built side-by-side in the same Rust workspace.
...
The [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) knows the `contract_a_interface` is not intended to be compiled into a .wasm, because the `contract_a_interface`'s `Cargo.toml` has its `crate-type` configured as `rlib` (rust library). Nice!

## Run the Contract

If you have [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) installed, you can invoke contract the functions. Both contracts must be deployed.
