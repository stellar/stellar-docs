---
title: BLS Signature
description: A contract account with BLS signature verification.
sidebar_position: 17
---
...
<head>
  ...
  <meta
    property="og:title"
    content="A contract account with BLS signature verification."
  />
  <meta
    property="og:description"
    content="A basic contract account implementing the `FastAggregateVerify` function in [BLS Signatures](https://www.ietf.org/archive/id/draft-irtf-cfrg-bls-signature-05.html#name-fastaggregateverify) draft."
  />
</head>
...
The [BLS signature example](https://github.com/stellar/soroban-examples/tree/v23.0.0/bls_signature) illustrates how to implement BLS signature verification inside a contract account.

This example is based off of the [Complex Account example](./complex-account.mdx). Although the main goal is to illustrate the practical use of the BLS12-381 functionalities in a relevant setting.

It is good to have an understanding of how a contract account works, so walk through the [Simple Account example](./simple-account.mdx) first, but it is not required.
...
Open the [`bls_signature/src/test.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/bls_signature/src/test.rs) file to follow along.
