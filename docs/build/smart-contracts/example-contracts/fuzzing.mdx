...
title: Fuzz Testing
description: Fuzz test smart contracts with cargo-fuzz and customize the input to fuzz tests with arbitrary crate; demonstrate how to adapt fuzz tests to reusable property tests.
---

The [fuzzing example](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing) demonstrates how to fuzz test Soroban contracts with [`cargo-fuzz`](https://docs.rs/cargo-fuzz) and customize the input to fuzz tests with the [`arbitrary`](https://docs.rs/arbitrary/latest/arbitrary/trait.Arbitrary.html) crate. It also demonstrates how to adapt fuzz tests into reusable property tests with the [`proptest`](https://docs.rs/proptest) and [`proptest-arbitrary-interop`](https://docs.rs/proptest-arbitrary-interop) crates. It builds on the [timelock example](https://github.com/stellar/soroban-examples/tree/v23.0.0/timelock).
...
First go through the [setup](../../smart-contracts/getting-started/setup.mdx) process to get your development environment configured, then clone the `v23.0.0` tag of `soroban-examples` repository:
...
In Rust, fuzzing is most often performed with the [`cargo-fuzz`](https://docs.rs/cargo-fuzz) tool, which drives LLVM's [`libfuzzer`](https://llvm.org/docs/LibFuzzer.html), though other fuzzing tools are available.
...
In Rust, multiple fuzzers are maintained by the [`rust-fuzz`](https://github.com/rust-fuzz) GitHub organization, which also maintains a "trophy case" of Rust bugs found through fuzzing.

## About the Example

The example used for this tutorial is based on the [`timelock`](https://github.com/stellar/soroban-examples/tree/v23.0.0/timelock) example program, with some changes to demonstrate fuzzing.
...
There are special considerations to note in the configuration of both the [contract's manifest](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/Cargo.toml) and the [fuzzing crate's manifest](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/Cargo.toml).
...
Note that cargo has a [feature/bug that inhibits LTO](https://github.com/stellar/soroban-docs/pull/476) of cdylibs when a crate is both a "cdylib" and "rlib". This can be worked around by building the contract with either `soroban contract build` or `cargo rustc --crate-type cdylib` instead of the typical `cargo build`.

:::

The contract crate must also provide the "testutils" feature. When "testutils" is activated, the Soroban SDK's [`contracttype`](https://docs.rs/soroban-sdk/latest/soroban_sdk/attr.contracttype.html) macro emits additional code needed for running fuzz tests.
...
First let's look at [`fuzz_target_1.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/fuzz_targets/fuzz_target_1.rs). This fuzz test does two things: it first deposits an arbitrary amount, then it claims an arbitrary amount.
...
The most important column here is `cov`. This is a cumulative measure of branches covered by the fuzzer. When this number stops increasing the fuzzer has probably explored as much of the program as it can. The other columns are described in the [`libfuzzer` documentation](https://llvm.org/docs/LibFuzzer.html#output).
...
See the [`libfuzzer` documentation](https://llvm.org/docs/LibFuzzer.html#output) for more.

## Accepting Soroban Types as Input with the `SorobanArbitrary` Trait

Inputs to the `fuzz_target!` macro must implement the [`Arbitrary`](https://docs.rs/arbitrary/latest/arbitrary/trait.Arbitrary.html) trait, which accepts bytes from the fuzzer driver and converts them to Rust values. Soroban types though are managed by the host environment, and so must be created from an [`Env`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html) value, which is not available to the fuzzer driver. The [`SorobanArbitrary`](https://docs.rs/soroban-sdk/latest/soroban_sdk/testutils/arbitrary/trait.SorobanArbitrary.html) trait, implemented for all Soroban contract types, exists to bridge this gap: it defines a _prototype_ pattern whereby the `fuzz_target` macro creates prototype values that the fuzz program can convert to contract values with the standard soroban conversion traits, [`FromVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.FromVal.html) or [`IntoVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.IntoVal.html).
...
- `i32`, `u32`, `i64`, `u64`, `i128`, `u128`, [`I256`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.I256.html), [`U256`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.U256.html), `()`, and `bool`,
...
All user-defined contract types, those with the [`contracttype`](https://docs.rs/soroban-sdk/latest/soroban_sdk/attr.contracttype.html) attribute, automatically derive `SorobanArbitrary`. Note that `SorobanArbitrary` is only derived when the "testutils" Cargo feature is active. This implies that, in general, to make a Soroban contract fuzzable, the contract crate must define a "testutils" Cargo feature, that feature should turn on the "soroban-sdk/testutils" feature, and the fuzz test, which is its own crate, must turn that feature on.

## A More Complex Fuzz Test

The [`fuzz_target_2.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/fuzz_targets/fuzz_target_2.rs) example, demonstrates the use of `SorobanArbitrary`, the advancement of time, and more advanced fuzzing techniques.
...
This shows how to use the `SorobanArbitrary::Prototype` associated type to define inputs to the fuzzer. A Soroban [`Address`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Address.html) can only be created with an [`Env`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html), so cannot be generated directly by the `Arbitrary` trait. Instead we use the fully-qualified name of the `Address` prototype, `<Address as SorobanArbitrary>::Prototype`, to ask for `Address`'s prototype instead. Then when our fuzzer needs the `Address` we instantiate it with the [`FromVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.FromVal.html) trait:
...
To advance time we maintain a [`LedgerSnapshot`](https://docs.rs/soroban-ledger-snapshot/latest/soroban_ledger_snapshot/struct.LedgerSnapshot.html), defined in the [`soroban-ledger-snapshot`](https://docs.rs/soroban-ledger-snapshot) crate. For each step we call [`Env::from_snapshot`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html#method.from_snapshot) to create a fresh environment to execute the step, then [`Env::to_snapshot`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html#method.to_snapshot) to create a new snapshot to use in the following step.
...
In addition to fuzz testing, Soroban supports property testing in the style of quickcheck, by using the [`proptest`](https://docs.rs/proptest) and [`proptest-arbitrary-interop`](https://docs.rs/proptest-arbitrary-interop) crates in conjunction with the `SorobanArbitrary` trait.
...
The [`proptest.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/src/proptest.rs) file is a translation of `fuzz_target_1.rs` to a property test.
