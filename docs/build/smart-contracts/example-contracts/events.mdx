...
The [events example](https://github.com/stellar/soroban-examples/tree/v23.0.0/events) demonstrates how to publish events from a contract. This example is an extension of the [storing data example](../getting-started/storing-data.mdx).

:::tip[Whisk Changes]

With the release of Whisk, Protocol 23, the syntax for publishing smart contract events has changed. In order to provide the most up-to-date information, this example has been updated to include the new patterns. Find more detailed information in the [Rust SDK documentation](https://docs.rs/soroban-sdk/latest/soroban_sdk/_migrating/v23_contractevent/index.html).
...
Event data will, by default, conform to the data structure in the defined `struct`. The `data_format` can also be specified as `vec` or `single-value`. Again, please refer to the [Rust SDK documentation](https://docs.rs/soroban-sdk/latest/soroban_sdk/_migrating/v23_contractevent/index.html) for more details.
...
If you have [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) installed, you can invoke contract functions in the using it.
