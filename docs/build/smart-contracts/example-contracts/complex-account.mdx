...
:::info Protocol 28: new `ContractExecutable::ExternalRef` variant

The `verify_authorization_policy` example below matches on `Context::CreateContractHostFn(_)` and `Context::CreateContractWithCtorHostFn(_)` without inspecting their contents, which remains safe. But as of Protocol 28, the `ContractExecutable` carried by those contexts can be `ContractExecutable::Wasm`, as before, or the new `ContractExecutable::ExternalRef` variant introduced by [CAP-85](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0085.md). If your own account inspects the context's `executable` field and matches on `ContractExecutable` exhaustively, add a match arm for `ContractExecutable::ExternalRef`. A contract account built with a pre-Protocol-28 SDK version cannot unpack this new variant and will panic when it tries. See the [SDK migration guide](https://docs.rs/soroban-sdk/latest/soroban_sdk/_migrating/v28_check_auth_executable/index.html) for a worked example.
