...
1. **Signing (Transaction Signers)**: Once the transaction is completely filled out, the
...
1. **Submitting (Transaction Submitter)**: After signing, the transaction must be valid and can now
   ...
   number order. Transactions are typically submitted using [Horizon](/api/introduction/), but you
   can also submit the transaction directly to an instance of
   [Stellar Core](..//run-core-node/index.mdx).

1. **Propagating (Validator)**: Once Stellar Core has determined that a transaction is valid, it
...
1. **Crafting a candidate transaction set (Validator)**: When it's time to close the ledger, each
...
1. **Nominating a transaction set (Validator)**: Once each validator has crafted a candidate
...
1. **Transaction apply order is determined (Validator Network)**: Once SCP agrees on a particular
...
1. **Fees are collected (Validator)**: [Fees](./fees.mdx) are collected for all transactions
   simultaneously.

1. **Application (Validator)**: Each transaction is applied in the order previously determined.
...
1. **Protocol Upgrades (Validator)**: Finally, upgrades are run if an upgrade took place. This
