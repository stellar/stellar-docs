...
- asset_balance_changes
  - array
  - An array of [Stellar Asset Contract](https://soroban.stellar.org/docs/tokens/token-interface) balance update events that occurred as a side effect of the `InvokeHostFunctionOp` (only present when the type of `InvokeHostFunctionOp` is `HostFunctionTypeHostFunctionTypeInvokeContract`).
    - asset_type
      - string
