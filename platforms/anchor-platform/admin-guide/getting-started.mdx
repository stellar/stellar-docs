...
The Anchor Platform requires the connection to either the Horizon or RPC server to function properly. Either the Horizon or RPC must be configured.
...
<CodeExample>
  ```bash # dev.env STELLAR_NETWORK_TYPE=rpc
...
<CodeExample>
  ```bash # dev.env STELLAR_NETWORK_TYPE=horizon
