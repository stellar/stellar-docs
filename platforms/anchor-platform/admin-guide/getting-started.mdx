...
1. Use Stellar CLI to find the secret key and copy it to your clipboard:

   **On macOS:**

   ```bash
   stellar keys secret anchor-platform | tee >(pbcopy)
   ```

   **On Linux:**

   ```bash
   stellar keys secret anchor-platform | tee >(xclip -selection clipboard)
   ```

   This command displays the secret key (seed) for the `anchor-platform` keypair and automatically copies it to your clipboard. The output will look like:

   ```
   SAV7...2ZXSCKH
   ```

   The secret key is now in your clipboard and ready to paste in the next step.

2. Open the [Stellar Demo Wallet](https://demo-wallet.stellar.org) in your browser.
3. Click on **"Provide a secret key (testnet only)"** button.
4. Paste the secret key from step 1 into the input field and submit.
5. You should now see `XLM` available under the **Balances** section, indicating your account is loaded.
6. Click on **"Add home domain"** and enter the following URL:
   ```
   http://localhost:8080
   ```
   This connects the demo wallet to your local Anchor Platform instance running on port 8080.
7. You should now be able to perform SEP transactions (deposits, withdrawals, etc.) with your local Anchor Platform instance.
