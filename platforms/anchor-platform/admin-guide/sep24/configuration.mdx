...
`SEP24_INITIAL_USER_DEADLINE_SECONDS` is the time in seconds that defines the user action deadline before a transaction automatically moves to the next status. It determines the `user_action_required_by` field, which indicates the deadline for statuses with automatic movement. Check [JSON-RPC Methods][json-rpc-methods] for usage examples.
