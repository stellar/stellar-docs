...
<CodeExample>
  ```bash # dev.env SEP10_ENABLED=true SEP10_HOME_DOMAIN=localhost:8080
  SECRET_SEP10_SIGNING_SEED="a Stellar private key" SECRET_SEP10_JWT_SECRET="a
...
<CodeExample>
  ```bash # dev.env SEP45_ENABLED=true SEP45_WEB_AUTH_DOMAIN=localhost:8080
...
:::info To enable SEP-45 (passkey authentication), the RPC connection must be configured. For detailed instructions on configuring the RPC connection, please refer to the [RPC Configuration Guide](../getting-started.mdx). :::
