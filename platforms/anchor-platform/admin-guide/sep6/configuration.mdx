...
If businesses want to set a deadline for user actions on transactions, they should use the `initial_user_deadline_seconds` parameter to define the time (in seconds) the user has to act before the transaction automatically moves to the next status (e.g., `expired` or `refunded`). The `user_action_required_by` field will be populated with the deadline timestamp for statuses where automatic status movement occurs after the deadline. Check [JSON-RPC Methods][json-rpc-methods] for usage examples.
