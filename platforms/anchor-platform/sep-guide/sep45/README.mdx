...
Stellar smart wallet applications create authenticated sessions with Stellar anchors by proving them, or their users, have sufficient control over a contract account. Once authenticated, the smart wallet application uses a session token provided by the anchor in subsequent requests to the anchor's standardized services.

The SEP-45 does not replace SEP-10 but also supports contact accounts (`C`) while SEP-10 only supports classic and muxed accounts (`G` and `M`). Services wishing to support all accounts should implement both SEPs.
...
2. The **Client** verifies that each authorization entry does not contain any sub-invocations
3. The **Client** verifies that the `contract_address` in each authorization entry matches the `WEB_AUTH_CONTRACT_ID` from the **Server**'s `stellar.toml`
4. The **Client** verifies that the `function_name` in each authorization entry is `web_auth_verify`
5. The **Client** verifies that the `args` map in each authorization entry match the expected values
6. The **Client** signs the authorization entry where `credentials.address.address` is the **Client Account** using the secret key(s) of the signer(s) for the **Client Account**
7. The **Client** submits the signed authorization entries back to the **Server** using the token endpoint
8. The **Server** verifies the signed authorization entries and responds with a JWT that represents the authenticated session
...
SEP45_JWT_TIMEOUT=900
SEP45_AUTH_TIMEOUT=86400
SECRET_SEP_45_JWT_SECRET=your_jwt_secret_key_here
```
...
`SEP45_HOME_DOMAINS` - A comma-separated list of home domains that are allowed to authenticate with the anchor. Home domains are the domains hosting the `stellar.toml` file. This file tells clients where to find the authentication endpoint (`WEB_AUTH_FOR_CONTRACTS_ENDPOINT`) and which public key (`SIGNING_KEY`) the server will use for challenges. Examples:

- `localhost:8080`
- `ap.stellar.org, *.sdp.stellar.org`
- `example.com`

`SEP45_JWT_TIMEOUT` - The time in seconds that the JWT token is valid. The default is `900` seconds (15 minutes).

`SEP45_AUTH_TIMEOUT` - The time in seconds that the authentication challenge is valid. The default is `86400` seconds (24 hours).

`SECRET_SEP_45_JWT_SECRET` - The JWT encryption key used to sign and verify SEP-45 JWT tokens. This is required when SEP-45 is enabled.
...
`WEB_AUTH_CONTRACT_ID` - The contract ID of the SEP-45 contract. This is the contract that will be used to construct the challenge. The contract must implement the `web_auth_verify` function as described in the [SEP-45 specification][sep-45].
