...
This config parameter is optional, set as FALSE by default. Controls whether Horizon will accept HTTP requests to the `/tx` API endpoint and forward to the network. Refer to [Channel Accounts](../../../docs/encyclopedia/channel-accounts) for some recommendations on optional client transaction submission optimizations.
