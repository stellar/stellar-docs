...
- `DISTRIBUTION_ACCOUNT.STELLAR.DB_VAULT`
  ...
  - **Transaction throughput**: Approximately 40 transactions per second.
...
In multi-tenant deployments, selecting `DISTRIBUTION_ACCOUNT.STELLAR.ENV` means the tenant shares the host's distribution account. That removes per-tenant fund segregation, so use it only when you intentionally want single-tenant behavior.
