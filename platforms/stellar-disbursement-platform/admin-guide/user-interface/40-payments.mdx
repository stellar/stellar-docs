...
- Payment Details: Each payment is listed with the following details:
  ...
  - Status: The current status of the payment. This can be "Success" if the payment has been completed, “Pending” if the payment has been submitted to the TSS but not yet to the Stellar network provided that the distribution account is type `DISTRIBUTION_ACCOUNT.STELLAR.<STORAGE_METHOD>`, or "Ready" if the payment is yet to be processed.
