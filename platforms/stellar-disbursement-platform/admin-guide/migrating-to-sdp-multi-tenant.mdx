...
In the multi-tenant version, tenants funds are islated from each other **unless the tenant is created with `"distribution_account_type": "DISTRIBUTION_ACCOUNT.STELLAR.ENV"`**. Here are the possible values for the `distribution_account_type` field:
