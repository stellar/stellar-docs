...
In the multi-tenant version, tenants funds are isolated from each other **unless the tenant is created with `"distribution_account_type": "DISTRIBUTION_ACCOUNT.STELLAR.ENV"`**. Here are the possible values for the `distribution_account_type` field:
- `DISTRIBUTION_ACCOUNT.STELLAR.DB_VAULT`, the distribution account private key is stored in the database, encrypted using the `DISTRIBUTION_ACCOUNT_ENCRYPTION_PASSPHRASE`, and segregated per tenant. The private key is generated by the system during Tenant creation and no human intervention is required.
- `DISTRIBUTION_ACCOUNT.CIRCLE.DB_VAULT`, the distribution account will pull funds from the tenant's [Circle](https://www.circle.com) account, and the Circle secret gets stored in the database, encrypted using the `DISTRIBUTION_ACCOUNT_ENCRYPTION_PASSPHRASE`, and segregated per tenant. The Circle API_KEY is provided by the tenant themselves once they have access to the dashboard.
- ⚠️`DISTRIBUTION_ACCOUNT.STELLAR.ENV`, this is NOT RECOMMENDED for production environments unless you're on single-tenant mode. With this value configured, the distribution account private key is read from the environment variable `DISTRIBUTION_SEED`, which is the same distribution account as the HOST.
...
distributionAccountType="DISTRIBUTION_ACCOUNT.STELLAR.ENV"
...
```sql
    SELECT admin.import_tenant_data_from_v1_to_v2('tenant_name')
```
</CodeExample>
