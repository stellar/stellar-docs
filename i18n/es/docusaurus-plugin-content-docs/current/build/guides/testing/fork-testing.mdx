...
El [Soroban Rust SDK] y el [Stellar CLI](../../../tools/cli/README.mdx) se unen para hacer posible las pruebas con datos de mainnet.

## Cómo Escribir Pruebas con Datos de Mainnet

El siguiente es un ejemplo de una prueba que incluye un contrato de dependencia en la prueba, en lugar de simularlo. La prueba está escrita para probar el [contrato increment-with-pause](https://github.com/stellar/soroban-examples/blob/main/increment_with_pause/src/lib.rs) y el [contrato pause](https://github.com/stellar/soroban-examples/blob/main/pause/src/lib.rs). El contrato tiene una función `increment` que aumenta el valor de un contador en uno en cada invocación. El contrato depende del contrato pause para controlar si la funcionalidad de incremento está pausada.

Las siguientes pruebas configuran el contrato `increment-with-pause`, usando una imagen del ledger que ya tiene desplegado el contrato dependencia `pause` junto con sus datos de contrato.
