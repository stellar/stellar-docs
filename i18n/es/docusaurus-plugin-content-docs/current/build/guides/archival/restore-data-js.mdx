...
La operación manual (`RestoreFootprintOp`), en su mayor parte, ya no es necesaria a partir del Protocolo 23, ya que las entradas archivadas se restauran automáticamente cuando aparecen en una footprint durante `InvokeHostFunctionOp`. `RestoreFootprintOp` puede usarse en casos raros [descritos en esta sección](../../../learn/fundamentals/contract-development/storage/state-archival.mdx#contract-data-automatic-restoration).
