...
Desarrolla los contratos con el siguiente comando (no olvides cambiar el directorio de trabajo a `soroban-project` primero antes de ejecutarlo), y revisa el directorio de construcción `target/wasm32-unknown-unknown/release/` para los archivos `.wasm` compilados.
