---
title: Usando __check_auth de maneras interesantes
---

<head>
  <title>Usando __check_auth de maneras interesantes</title>
  <meta charSet="utf-8" />
...
## Tutorial 1: restricción basada en el tiempo en transferencias de tokens

Imaginemos un contrato de cuenta multi-sig en el que ciertas acciones, como las fichas de transferencia, tengan que ser controladas por el tiempo transcurrido entre las transferencias consecutivas. Por ejemplo, sólo se debería permitir una transferencia de token si ha transcurrido un cierto período de tiempo desde la última transferencia. Esto puede ser útil en escenarios en los que desea limitar la frecuencia de las transacciones para prevenir el uso indebido o para implementar limitación de tasas.

### Conceptos base

#### Restricciones de tiempo

La idea es hacer cumplir un intervalo de tiempo mínimo entre las transferencias consecutivas de token. Cada token puede tener su propio límite de tiempo, y el contrato rastreará la última hora de transferencia para cada token.

#### Almacenamiento

El contrato almacenará el límite de tiempo y la última hora de transferencia de cada token en su almacenamiento para realizar un seguimiento de estos valores a través de las transacciones.

### Pasarela del código

#### Contrato de estructura e importaciones

```rust
#![no_std]

usa soroban_sdk::{
    auth::{Context, CustomAccountInterface}, contrato, contracterror, contractimpl, contracttype, symbol_short, Dirección,
...
enum DataKey {
    ...
    Signer(BytesN<32>),
    TimeLimit(Dirección),
    LastTransferTime(Dirección),
}

#[contracterror]
#[derive(Copiar, Clone, Debug, Eq, ParcialEq, ParcialOrd, Ord)]
#[repr(u32)]
pub enum AccError {
    NotEnoughSigners = 1,
    NegativeAmount = 2,
...
2. Almacena el recuento de firmantes en el almacenamiento del contrato
...
impl AccountContract {
    ...
    pub fn init(env: Env, signers: Vec<BytesN<32>>) {
        para firmar en firmantes. ter() {
            env.storage().instance(). et(&DataKey::Signer(signer), &());
        }
        env. torage()
            . nstance()
...
#### Ajustando límites de tiempo

1. Permite establecer un límite de tiempo para un token específico
2. Asegura que sólo el propio contrato puede establecer estos límites al requerir la autorización del contrato

```rust
#[contractimpl]

impl AccountContract {
    pub fn set_time(env: Env, token: Dirección, time_limit: u64) {
        env. urrent_contract_address(). equire_auth();
        env. torage()
            . nstance()
            . et(&DataKey::TimeLimit(token), &time_limit);
    }
}
```

#### Autenticación personalizada y autorización

1. Autenticar las firmas
2. Comprueba si todos los firmantes requeridos han firmado
3. Iteriza a través del contexto de autorización para verificar la política de autorización
...
1. Verifica que las firmas están en el orden correcto y que cada firmante está autorizado
...
#### Verificación de política de autorización

1. Comprueba si la función que se llama es una función de transferencia o aprobación
2. Impide la restricción basada en el tiempo comparando la hora actual con la última transferencia
3. Actualiza la última transferencia si la transferencia está permitida
...
fn verify_authorization_policy(
        ...
        si contract_contexto. n_name != TRANSFER_FN
            && contract_context. n_name ! Symbol::new(env, "aprobación")
        {
            return Ok(());
        }

        let current_time = env. arista(). imestamp();
        let time_limit: Opción<u64> =
            env. torage()
                . nstance()
                . et::<_, u64>(&DataKey::TimeLimit(contract_context.contract. lone()));

        if let Some(limit) = time_limit {
            let last_transfer_time: u64 = env. torage()
                . nstance()
                . et::<_, u64>(&DataKey::LastTransferTime(contract_context. ontract.clone()))
                . nwrap_or(0);
            ...
            env. torage()
                . nstance()
                . et(&DataKey::LastTransferTime(contract_context. ontract.clone()), &current_time);
        }
        Ok(())
}
```
...
El contrato comienza inicializando con un conjunto de firmantes autorizados. Permite establecer un límite de tiempo para las transferencias de tokens, que controla la frecuencia con la que se puede transferir un token. La función \_\_check_auth es el núcleo del proceso de autorización, asegurándose de que todas las firmas necesarias son válidas y comprobando la restricción basada en el tiempo para transferencias de token. Si el tiempo requerido no ha pasado desde la última transferencia, el contrato denegará la operación, aplicando el límite deseado. Al rastrear el último tiempo de transferencia y aplicar un intervalo de tiempo mínimo entre transferencias, el contrato limita efectivamente la frecuencia de las transferencias de tokens, resolviendo la cuestión del abuso potencial mediante transacciones rápidas consecutivas.
...
## Tutorial 2: implementar una cartera inteligente (WebAuthn)

Imagine un mundo donde las contraseñas tradicionales sean obsoletas. En este mundo, WebAuthn (Web Authentication) se ha convertido en el estándar para interacciones seguras en línea. Alice, un entusiasmo de blockchain, quiere crear una cartera que aprovecha la tecnología WebAuthn para mejorar la seguridad. Ella decide implementar una cartera WebAuthn en Stellar, permitiendo a los usuarios administrar sus activos digitales usando las características biométricas de su dispositivo o claves de seguridad (e. ., YubiKey, Google Titan Security Key, etc.).

### Conceptos base

WebAuthn es un estándar web para autenticación sin contraseña. Permite a los usuarios autenticarse utilizando biométricos, como huellas dactilares o reconocimiento facial.

La cartera WebAuthn implementada en este tutorial será:

1. Registrar y administrar credenciales de usuario (claves públicas)
2. Autenticar usuarios para firmar transacciones
3. Diferenciar entre usuarios de administración y usuarios regulares
4. Permitir actualizaciones de contratos para futuras mejoras

:::info

El crédito de código de este tutorial va a [@kalepail](https://github.com/kalepail) trabajar en contraseñas, que puedes explorar más [here](https://github.com/kalepail/passkey-kit).
...
Esta sección establece la estructura del contrato e importa los componentes necesarios del SDK Soroban.
...
usa soroban_sdk::{
    ...
    contrato, contracterror, contractimpl, contracttype,
    crypto::Hash,
...
```rust
#[contracterror]
#[derive(Copiar, Clonar, Eq, PartialEq, Debug)]
pub esum Error {
    Notencontrado = 1,
    NotPermitido = 2,
    ClientDataJsonChallengeIncorrecto = 3,
...
Estas funciones manejan la adición y eliminación de firmantes, la actualización del contrato y la gestión de conteos de administración.
...
Esto implementa la lógica de autenticación principal para la cartera WebAuthn.
...
Esta función proporciona la funcionalidad de codificación de URL Base64 usada en el proceso WebAuthn.
...
#### Explicación por escrito del código

El contrato del monedero WebAuthn administra las credenciales de usuario y la autenticación. Permite añadir y eliminar firmantes, distinguiendo entre usuarios de administración y usuarios regulares. La función de añadir registra nuevos firmantes, almacenar las claves de administración de forma persistente y las claves regulares temporalmente. La función remover borra firmantes, y la actualización permite actualizaciones del contrato. El núcleo de la seguridad de la cartera es la función `__check_auth`, que verifica firmas de WebAuthn. Comprueba la firma contra la clave pública almacenada, verifica el JSON de los datos del cliente y asegura que el desafío coincida con el valor esperado. El contrato utiliza las capacidades de almacenamiento de Soroban para gestionar claves y conteos de administración, con diferentes TTL (Time To Live) para el almacenamiento persistente y temporal.

#### Casos de prueba

Estos son casos de prueba para asegurar que nuestra cartera WebAuthn está funcionando correctamente. Utilizaremos las utilidades de pruebas del Soroban SDK para crear y ejecutar estas pruebas.
...
Esta implementación del monedero WebAuthn proporciona una forma segura y fácil de usar para gestionar los activos digitales en Stellar. Aproveche los beneficios de seguridad de WebAuthn al tiempo que mantiene la flexibilidad necesaria para las interacciones con blockchain.
