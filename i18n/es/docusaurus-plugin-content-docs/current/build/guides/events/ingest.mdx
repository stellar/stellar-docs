...
Vamos a ingerir eventos en una tabla llamada `StellarEvent`. En SQLAlchemy, esto se traduce en una clase, también llamada modelo de base de datos:
...
Primero, necesitaremos consultar los eventos desde Stellar RPC. Este ejemplo sencillo realiza una solicitud RPC usando el [`método getEvents`], filtrando todos los eventos `transfer` que emite el contrato nativo de XLM.
...
Facilita tu vida con un [`sessionmaker`](https://docs.sqlalchemy.org/en/20/orm/session_basics.html#using-a-sessionmaker) de SQLAlchemy al hacer transacciones (por ejemplo, añade más de un registro en una sola llamada a la base de datos).
