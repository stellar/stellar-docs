...
Algunos términos de archivo del estado que utilizaremos en esta guía se describen en la [sección de archivo del estado](/docs/learn/fundamentals/contract-development/storage/state-archival#terms-and-semantics).
...
Aquí estaremos utilizando el [método `getLedgerEntries`](/docs/data/apis/rpc/api-reference/methods/getLedgerEntries#requesting-a-contracts-wasm-code) para obtener el código WASM del contrato y también la [operación `restoreFootprint`](/docs/learn/fundamentals/transactions/list-of-operations#restore-footprint) para restaurar la instancia del contrato.
