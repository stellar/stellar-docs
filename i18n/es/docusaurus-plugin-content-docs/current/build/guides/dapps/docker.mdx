...
Para ilustrar el proceso, tomemos un ejemplo de nuestra [ejemplo de dapp de crowdfunding](https://github.com/stellar/soroban-example-dapp). Para construir la imagen de Docker, utilizas un comando que está encapsulado dentro de nuestro Makefile:
...
Puedes agilizar el proceso de despliegue utilizando un script para ejecutar el contenedor de Docker. El siguiente script es un envoltorio para la [`stellar/quickstart` imagen de Docker](https://hub.docker.com/r/stellar/quickstart), que proporciona una forma rápida de ejecutar una red Stellar. Puedes encontrar un ejemplo del script `quickstart.sh` ubicado en el directorio raíz de la [ejemplo de dapp de crowdfunding](https://github.com/stellar/soroban-example-dapp).
