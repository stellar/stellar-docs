...
El estándar [SEP-24](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md) define la forma estándar en que los anchors y las billeteras interactúan en nombre de los usuarios. Las billeteras utilizan este estándar para facilitar el exchange entre activos on-chain (como stablecoins) y activos off-chain (como fiat, u otros activos de red como BTC).
...
Para mejorar la experiencia del usuario, el estándar [SEP-24](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md) admite el envío de KYC del usuario al ancla a través de [SEP-9](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0009.md). A su vez, el ancla rellenará esta información en el popup interactivo.

:::info

Mientras que [SEP-9](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0009.md) admite el envío de datos binarios, la versión actual del SDK no ofrece tal funcionalidad.

:::

:::note

En este momento, el [SEP-9](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0009.md) aceptado aún no está tipificado de manera estricta. La mejora de la tipificación se ofrecerá en versiones futuras.
...
En el flujo típico, la billetera obtendría datos de la transacción para notificar a los usuarios sobre las actualizaciones de estado. Esto se realiza a través de los endpoints `GET /transaction` y `GET /transactions` de [SEP-24](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md).
...
El código para enviar transacciones a Stellar debería ser desarrollado cuidadosamente. El SDF tiene una página de documentación dedicada a [enviar transacciones y manejar errores con gracia](../../../data/apis/horizon/api-reference/errors/error-handling.mdx). Aquí hay varias cosas que debes tener en cuenta:
