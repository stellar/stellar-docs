---
title: Código de ejemplo del gasoducto de Ingestión
sidebar_position: 30
---

Código completo para una pequeña muestra de un tubería de ingestión de red estelar usando el [SDK de Ingestión](overview. dx#the-ingestion-sdk-packages) para publicar datos derivados a un broker de mensajes remoto. Demostrar la unidad de eventos, el procesamiento distribuido con un microservicio de ejemplo (Python script) como suscriptor.

Este ejemplo utiliza ZeroMQ [goczmq](https://github.com/zeromq/goczmq) Go wrapper SDK, que requiere que algunas [bibliotecas dependientes también estén instaladas en la máquina anfitrión](https://github.com/zeromq/goczmq?tab=readme-ov-file#dependencies).

Pon estos archivos en un directorio, compile y ejecute con `go build -o pipeline ./.; ./pipeline`
...
Un script Python que demuestra cómo ahora hemos distribuido la arquitectura de procesamiento y eventos mediante el aprovechamiento del Broker MQ para empujar el modelo de datos de pago de aplicaciones derivadas a otros microservicios. Asegúrate de `pip install pyzmq`
