...
- No hay riesgo de que el administrador reciba tokens no deseados. Si el token requiere una trustline, y una no existe en la cuenta (es decir, el administrador no ha optado por mantener ese activo), la invocación simplemente fallará. Si es un token de contrato exclusivo de Soroban, la entrada de almacenamiento del nuevo saldo será pagada por quien invoque esta función. El administrador no pierde ningún fondo, y el saldo no tendrá un impacto significativo en su cuenta.
...
Seguimos las [convenciones típicas de contrato](../../guides/conventions/error-enum.mdx) y creamos un `enum` para registrar nuestros errores.
