...
description: Prueba de fuzz para contratos inteligentes con cargo-fuzz y personaliza la entrada para las pruebas de fuzz con crate arbitrario; demuestra cómo adaptar las pruebas de fuzz a pruebas de propiedad reutilizables.
---
El [ejemplo de fuzzing](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing) demuestra cómo probar contratos Soroban con [`cargo-fuzz`](https://docs.rs/cargo-fuzz) y personalizar la entrada para las pruebas de fuzz con crate [`arbitrary`](https://docs.rs/arbitrary/latest/arbitrary/trait.Arbitrary.html). También demuestra cómo adaptar las pruebas de fuzz en pruebas de propiedad reutilizables con los crates [`proptest`](https://docs.rs/proptest) y [`proptest-arbitrary-interop`](https://docs.rs/proptest-arbitrary-interop). Se basa en el [ejemplo de timelock](https://github.com/stellar/soroban-examples/tree/v23.0.0/timelock).
...
Primero sigue el proceso de [configuración](../../smart-contracts/getting-started/setup.mdx) para preparar tu entorno de desarrollo, luego clona la etiqueta `v23.0.0` del repositorio `soroban-examples`:
...
En Rust, el fuzzing se realiza más a menudo con la herramienta [`cargo-fuzz`](https://docs.rs/cargo-fuzz), que utiliza [`libfuzzer`](https://llvm.org/docs/LibFuzzer.html) de LLVM, aunque hay otras herramientas de fuzzing disponibles.
...
En Rust, múltiples fuzzers son mantenidos por la organización de GitHub [`rust-fuzz`](https://github.com/rust-fuzz), que también mantiene una "vitrina de trofeos" de errores de Rust encontrados a través del fuzzing.


## Acerca del Ejemplo

El ejemplo utilizado para este tutorial está basado en el programa de ejemplo [`timelock`](https://github.com/stellar/soroban-examples/tree/v23.0.0/timelock), con algunos cambios para demostrar el fuzzing.
...
Hay consideraciones especiales a tener en cuenta en la configuración tanto del [manifiesto del contrato](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/Cargo.toml) como del [manifiesto de la crate de fuzzing](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/Cargo.toml).
...
Ten en cuenta que cargo tiene una [característica/error que inhibe LTO](https://github.com/stellar/soroban-docs/pull/476) de cdylibs cuando una crate es tanto "cdylib" como "rlib". Esto se puede solucionar compilando el contrato con `soroban contract build` o `cargo rustc --crate-type cdylib` en lugar del típico `cargo build`.

:::


La crate del contrato también debe habilitar la característica "testutils". Cuando "testutils" está activado, el macro [`contracttype`](https://docs.rs/soroban-sdk/latest/soroban_sdk/attr.contracttype.html) del SDK Soroban emite código adicional necesario para ejecutar pruebas de fuzzing.
...
Primero veamos [`fuzz_target_1.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/fuzz_targets/fuzz_target_1.rs). Esta prueba de fuzzing hace dos cosas: primero deposita una cantidad arbitraria y luego reclama una cantidad arbitraria.
...
En lugar de una función `main`, `cargo-fuzz` usa un punto de entrada especial definido por el macro [`fuzz_target!`](https://docs.rs/libfuzzer-sys/latest/libfuzzer_sys/macro.fuzz_target.html) Este macro acepta un closure Rust que recibe `input`, cualquier tipo Rust que implemente el trait [`Arbitrary`](https://docs.rs/arbitrary/latest/arbitrary/trait.Arbitrary.html). Aquí hemos definido una estructura, `Input`, que deriva de `Arbitrary`.


`cargo-fuzz` será responsable de generar `input` y llamar repetidamente a este closure.

Para probar un contrato Soroban, debemos configurar un [`Env`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html). Ten en cuenta que hemos deshabilitado el presupuesto de CPU y memoria: esto nos permitirá fuzzear rutas de código arbitrariamente complejas sin preocuparnos por quedarnos sin presupuesto; podemos asumir que quedarse sin presupuesto durante una transacción siempre falla correctamente, cancelando la transacción; no es algo que necesitemos fuzzear.


Consulta el código fuente de [`fuzz_target_1.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/fuzz_targets/fuzz_target_1.rs) para configuración adicional de este contrato.
...
La columna más importante aquí es `cov`. Esta es una medida acumulativa de ramas cubiertas por el fuzzer. Cuando este número deja de aumentar, probablemente el fuzzer ha explorado tanto del programa como puede. Las otras columnas se describen en la documentación de [`libfuzzer`](https://llvm.org/docs/LibFuzzer.html#output).
...
Consulta la [documentación de `libfuzzer`](https://llvm.org/docs/LibFuzzer.html#output) para más información.


## Aceptar tipos Soroban como entrada con el trait `SorobanArbitrary`

Las entradas al macro `fuzz_target!` deben implementar el trait [`Arbitrary`](https://docs.rs/arbitrary/latest/arbitrary/trait.Arbitrary.html), que acepta bytes del driver de fuzzing y los convierte en valores Rust. Sin embargo, los tipos Soroban son gestionados por el entorno host, y por lo tanto deben crearse a partir de un valor [`Env`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html), que no está disponible para el driver de fuzzing. El trait [`SorobanArbitrary`](https://docs.rs/soroban-sdk/latest/soroban_sdk/testutils/arbitrary/trait.SorobanArbitrary.html), implementado para todos los tipos de contrato Soroban, existe para salvar esta diferencia: define un patrón de _prototipo_ por el que el macro `fuzz_target` crea valores prototipo que el programa de fuzzing puede convertir en valores de contrato con los traits estándar de conversión de soroban, [`FromVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.FromVal.html) o [`IntoVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.IntoVal.html).
...
- `i32`, `u32`, `i64`, `u64`, `i128`, `u128`, [`I256`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.I256.html), [`U256`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.U256.html), `()`, y `bool`,
- [`Error`](https://docs.rs/soroban-sdk/latest/soroban_sdk/xdr/enum.Error.html),
- [`Bytes`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Bytes.html), [`BytesN`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.BytesN.html), [`Vec`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Vec.html), [`Map`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Map.html),
- [`Address`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Address.html), [`Symbol`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Symbol.html),
- [`Val`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Val.html),


Todos los tipos de contrato definidos por el usuario, aquellos con el atributo [`contracttype`](https://docs.rs/soroban-sdk/latest/soroban_sdk/attr.contracttype.html), derivan automáticamente `SorobanArbitrary`. Ten en cuenta que `SorobanArbitrary` solo se deriva cuando la característica Cargo "testutils" está activa. Esto implica que, en general, para hacer un contrato Soroban susceptible de fuzzing, la crate del contrato debe definir una feature Cargo "testutils", esa feature debe activar la feature "soroban-sdk/testutils", y la prueba de fuzzing, que es una crate propia, debe activar esa feature.

## Una prueba de fuzzing más compleja

El ejemplo [`fuzz_target_2.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/fuzz/fuzz_targets/fuzz_target_2.rs) demuestra el uso de `SorobanArbitrary`, el avance del tiempo, y técnicas de fuzzing más avanzadas.
...
Esto muestra cómo usar el tipo asociado `SorobanArbitrary::Prototype` para definir las entradas al fuzzer. Una [`Address`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Address.html) Soroban solo puede crearse con un [`Env`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html), por lo que no puede generarse directamente con el trait `Arbitrary`. En lugar de eso usamos el nombre completamente calificado del prototipo de `Address`, `<Address as SorobanArbitrary>::Prototype`, para pedir el prototipo de `Address`. Cuando el fuzzer necesita la `Address` la instanciamos con el trait [`FromVal`](https://docs.rs/soroban-sdk/latest/soroban_sdk/trait.FromVal.html):
...
Para avanzar el tiempo mantenemos una [`LedgerSnapshot`](https://docs.rs/soroban-ledger-snapshot/latest/soroban_ledger_snapshot/struct.LedgerSnapshot.html), definido en la crate [`soroban-ledger-snapshot`](https://docs.rs/soroban-ledger-snapshot). Para cada paso llamamos a [`Env::from_snapshot`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html#method.from_snapshot) para crear un ambiente nuevo para ejecutar el paso, luego a [`Env::to_snapshot`](https://docs.rs/soroban-sdk/latest/soroban_sdk/struct.Env.html#method.to_snapshot) para crear una nueva imagen para usar en el paso siguiente.
...
Además de las pruebas de fuzzing, Soroban admite pruebas de propiedad al estilo de quickcheck, usando las crates [`proptest`](https://docs.rs/proptest) y [`proptest-arbitrary-interop`](https://docs.rs/proptest-arbitrary-interop) junto con el trait `SorobanArbitrary`.
...
El archivo [`proptest.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/fuzzing/src/proptest.rs) es una traducción de `fuzz_target_1.rs` a una prueba de propiedad.
