---
title: Registro
description: Utiliza el registro para depurar tus contratos inteligentes.
sidebar_position: 40
---
...
El [ejemplo de registro](https://github.com/stellar/soroban-examples/tree/v23.0.0/hello_world) demuestra cómo registrar con el propósito de depuración.
...
Los registros en los contratos solo son visibles en las pruebas, o al ejecutar contratos utilizando [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli). Los registros solo se compilan en el contrato si la opción del compilador Rust `debug-assertions` está habilitada.


:::tip

Los registros no son un sustituto para la depuración paso a paso. Las pruebas de Rust para Soroban se pueden depurar paso a paso en tu IDE habilitado para Rust. Consulta [pruebas](../getting-started/hello-world.mdx#run-the-tests) para más detalles.

:::

:::caution

Los registros no son accesibles por dapps y otras aplicaciones. Consulta el [ejemplo de eventos](./events.mdx) para saber cómo producir eventos estructurados.
...
O bien, omite la configuración del entorno de desarrollo y abre este ejemplo en [GitHub Codespaces](https://github.com/codespaces/new?repo=stellar/soroban-examples&editor=web) o en [Code Anywhere](https://app.codeanywhere.com/#https://github.com/stellar/soroban-examples).
...
El macro [`log!`](https://docs.rs/soroban-sdk/latest/soroban_sdk/macro.log.html) registra una cadena. Todos los registros que ocurren durante la ejecución se envían a stdout en [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) y están disponibles para que las pruebas validen o impriman.
...
Los registros solo se almacenan en los entornos de Soroban que tienen el registro habilitado. Los únicos entornos de Soroban donde el registro está habilitado son en las pruebas de Rust, y en el [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli).
...
El macro [`log!`](https://docs.rs/soroban-sdk/latest/soroban_sdk/macro.log.html) construye una cadena a partir de la cadena de formato y una lista de argumentos. Los argumentos se sustituyen dondequiera que aparezca el valor `{}` en la cadena de formato.
...
El crate `std`, que contiene la biblioteca estándar de Rust, se importa para que la prueba pueda usar los macros `std::vec!` y `std::println!`. Dado que los contratos deben usar `#![no_std]`, las pruebas en contratos deben importar manualmente `std` para usar funcionalidades de `std` como imprimir en stdout.
...
El contrato se registra con el entorno utilizando el tipo de contrato. Estamos especificando exactamente en qué dirección de contrato debe desplegarse el contrato en la prueba, usando la función `env.register_at`. Esto facilita asegurar que la salida de los logs proviene de la dirección de contrato relevante.
...
También pueden imprimirse en stdout.
...
Si tienes [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) instalado, puedes invocar funciones del contrato usándolo. Especifica la opción `-v` para habilitar registros detallados.
