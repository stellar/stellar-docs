...
sidebar_position: 70
title: Desplegador
description: Desplegar un contrato inteligente en nombre de cualquier otra dirección utilizando el SDK `deployer()` e inicializarlo atómicamente.
---
import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";
import { getPlatform } from "@site/src/helpers/getPlatform";

El [ejemplo del desplegador](https://github.com/stellar/soroban-examples/tree/v23.0.0/deployer) demuestra cómo desplegar contratos utilizando un contrato.
...
Primero completa el proceso de [Configuración](../getting-started/setup.mdx) para tener tu entorno de desarrollo listo, luego clona la etiqueta `v23.0.0` del repositorio `soroban-examples`:
...
La función del SDK `env.deployer()` incluye algunas utilidades relacionadas con el despliegue. Aquí usamos el tipo de 'deployer' más genérico, `with_address(env.current_contract_address(), salt)`.
...
`with_address()` recibe la dirección del `deployer` y el `salt`. Ambos se utilizan para derivar la dirección del contrato desplegado de manera determinista. No es posible volver a desplegar un contrato que ya existe.

:::tip

La llamada `env.deployer().with_address(env.current_contract_address(), salt)` puede ser reemplazada por la función `env.deployer().with_current_contract(salt)` para mayor brevedad.
...
Abre el archivo [`deployer/deployer/src/test.rs`](https://github.com/stellar/soroban-examples/tree/v23.0.0/deployer/deployer/src/test.rs) para seguir el contenido.

#### Contrato a desplegar

Empieza importando el contrato de prueba Wasm que se va a desplegar.
...
Si tienes [`stellar-cli`](../getting-started/setup.mdx#install-the-stellar-cli) instalado, puedes invocar la función del contrato para desplegar el contrato de prueba.
