...
- Los endpoints de la API KYC [`GET /customer`](../../api-reference/callbacks/get-customer.api.mdx) y [`PUT /customer`](../../api-reference/callbacks/put-customer.api.mdx) para solicitar y recopilar los datos KYC de los clientes
- El endpoint RFQ de la API [`GET /rate`](../../api-reference/callbacks/get-rates.api.mdx) para proporcionar las tasas FX entre los activos on y off-chain soportados
...
- Solicitudes [`JSON-RPC`](../../api-reference/platform/rpc/methods/README.mdx) para actualizar los estados de las transacciones de la Plataforma Anchor
...
Esta guía no ofrece una implementación ejemplo de los endpoints, pero puedes encontrar más información sobre los esquemas de solicitud y respuesta en la [Referencia API de la Plataforma Anchor](../../README.mdx), y las secciones a continuación ampliarán conceptos importantes para comprender al implementar los endpoints.

## Endpoints de Callback de Cliente

La Plataforma Anchor nunca almacena la información personal identificable (PII) de tus clientes, sino que actúa como servidor proxy entre las aplicaciones cliente y tu negocio, reenviando solicitudes y respuestas a la otra parte. Actualmente, las solicitudes y respuestas son casi idénticas a las definidas en la [especificación API SEP-12 KYC](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md).

### Identificación de Clientes

Los clientes pueden identificarse usando dos enfoques.

El primer enfoque utiliza una cuenta Stellar y un memo. Al usar la Plataforma Anchor para facilitar pagos transfronterizos, la organización remitente utiliza su propia cuenta Stellar, la que se usa para autenticarse via [Autenticación Stellar SEP-10](../sep10/README.mdx), para registrar clientes con tu negocio. Los memos se utilizan para distinguir clientes únicos que provienen de la misma organización remitente.
...
Puedes probar tu implementación con la [Demo Wallet Stellar](https://demo-wallet.stellar.org) siguiendo los pasos a continuación.
...
[![Demo Wallet Despu S de](/assets/anchor-platform/sep31/demo-wallet-widget.png)](/assets/anchor-platform/sep31/demo-wallet-widget.png)
...
Una vez que la organización remitente haya registrado a los clientes involucrados en la transacción, necesitará solicitar una cotización, o tasa FX, a tu negocio. La Plataforma Anchor solicita esta información a tu servidor de negocio usando el endpoint [`GET /rate`](../../api-reference/callbacks/get-rates.api.mdx).

### Cotizaciones Firmes vs Indicativas

Las solicitudes de cotización tendrán un parámetro `type` que será [`indicative`](https://www.investopedia.com/terms/i/indicativequote.asp) o [`firm`](https://www.investopedia.com/terms/f/firmquote.asp). Si `type=firm`, tu respuesta debe incluir el campo `id` y `expires_at` (fecha y hora) y reservar la liquidez necesaria para cumplir esta cotización hasta que expire. Si `type=indicative`, no debes devolver los campos `id` ni `expires_at` porque la tasa proporcionada no se usará en una transacción.
...
[![Flujo Sep31 Diagram](/assets/anchor-platform/sep31/transition-diagram.png)](/assets/anchor-platform/sep31/transition-diagram.png)
...
Puedes crear una [plantilla](../sep24/integration.mdx#making-json-rpc-requests) para realizar solicitudes JSON-RPC a la Plataforma Anchor.

Este capítulo también incluye información sobre el formato de la [solicitud](../sep24/integration.mdx#json-rpc-request)/[respuesta](../sep24/integration.mdx#json-rpc-response) y los [códigos de error](../sep24/integration.mdx#error-codes) que puede devolver la Plataforma Anchor.
