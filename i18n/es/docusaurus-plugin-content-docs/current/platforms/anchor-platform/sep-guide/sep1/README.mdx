...
Para una lista completa de todos los atributos disponibles en `stellar.toml`, consulta la [especificación SEP-1](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0001.md). Necesitarás añadir secciones adicionales como `[[CURRENCIES]]`, `TRANSFER_SERVER`, `TRANSFER_SERVER_SEP0024`, `WEB_AUTH_ENDPOINT`, `WEB_AUTH_FOR_CONTRACTS_ENDPOINT`, `DIRECT_PAYMENT_SERVER`, etc., conforme configures los activos soportados y otros SEPs.
...
También puedes alojar tu archivo `stellar.toml` usando un servidor de archivos estático como [nginx](https://www.nginx.com) o una CDN. Si eliges este enfoque:
