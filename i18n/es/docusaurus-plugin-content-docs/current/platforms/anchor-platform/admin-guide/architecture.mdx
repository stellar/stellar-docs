...
El cliente es una aplicación, como un monedero o remitente de transferencias, que actúa en nombre de un usuario y realiza solicitudes al sistema. Los clientes hacen solicitudes al componente servidor SEP de la Anchor Platform usando conjuntos de estándares llamados [SEPs][seps] (Stellar Ecosystem Proposals).
...
El servidor de plataforma es un componente interno. Debe estar alojado en una red privada y no debe ser accesible desde Internet. Este servidor permite al negocio obtener y actualizar el estado de las transacciones usando su [API][platform-api].
