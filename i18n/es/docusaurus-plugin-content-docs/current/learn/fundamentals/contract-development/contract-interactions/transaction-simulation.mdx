...
Consulta la [visión general de autorizaciones](../authorization.mdx) y la sección de autorización de transacciones [auth-data](stellar-transaction.mdx#authorization-data) para información general sobre la autorización en Soroban: esta sección se refiere específicamente a cómo funciona la simulación junto con los requisitos de autorización.

El [mecanismo de simulación de transacciones](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx) de Soroban puede usarse para precomputar los árboles de [`SorobanAuthorizedInvocation`](https://github.com/stellar/stellar-xdr/blob/v22.0/Stellar-transaction.x#L558) que deben ser autorizados por las `Address` para que todas las comprobaciones `require_auth` pasen. Puede invocarse de dos formas diferentes:
...
Estos registros se agregan a un árbol de [`SorobanAuthorizedInvocation`](https://github.com/stellar/stellar-xdr/blob/v22.0/Stellar-transaction.x#L558) y se marcan como exitosos. Luego, tras terminar la invocación, la simulación de transacciones devuelve todos los árboles registrados, así como valores nonce generados aleatoriamente para las firmas esperadas.
...
Desde la perspectiva del desarrollador, la diferencia entre estos modos es si las entradas de autorización están presentes en la operación [`InvokeHostFunction`](../../../../learn/fundamentals/transactions/list-of-operations.mdx#invoke-host-function) enviada a [`simulateTransaction`](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx). Los [ejemplos a continuación](#typescript-utilities) detallan esta distinción, pero en resumen, pasar `auth` a [`Operation.invokeContractFunction`](https://stellar.github.io/js-stellar-sdk/Operation.html#.invokeContractFunction) (que es una envoltura conveniente para [`invokeHostFunction`](https://stellar.github.io/js-stellar-sdk/Operation.html#.invokeHostFunction)) implicará el modo enforcement.
...
En este ejemplo, usaremos el modo de autorización ["enforcement"](#enforcing-mode) de la simulación de transacciones [simulation](../../../../data/apis/rpc/api-reference/methods/simulateTransaction.mdx), que, al recibir entradas de autorización firmadas, asegura que sean las firmas necesarias y suficientes para la ejecución de la transacción.
