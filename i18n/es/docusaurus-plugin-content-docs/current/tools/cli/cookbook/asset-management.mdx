...
El [stellar-cli](../stellar-cli.mdx) se puede usar para emitir y administrar activos en la red Stellar.

Esta guía asume que el [stellar-cli](../stellar-cli.mdx) ha sido configurado para usar testnet o una red local de prueba. Para usar testnet, ejecuta el siguiente comando.
...
Para crear un activo Stellar usando el [stellar-cli](../stellar-cli.mdx), se realizan varios pasos:
...
Para otras opciones, consulta el manual del comando [stellar tx new set-options](../stellar-cli.mdx#stellar-tx-new-set-options).
