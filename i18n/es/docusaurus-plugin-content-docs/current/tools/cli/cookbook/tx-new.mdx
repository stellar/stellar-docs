...
Crea y financia una nueva cuenta Stellar. La cuenta `alice` fue financiada por [friendbot](https://developers.stellar.org/docs/learn/fundamentals/networks#friendbot). Sin embargo, `bob` y `charlie` no fueron financiados. Por lo tanto, podemos usar el comando `create-account` para financiarlos.
