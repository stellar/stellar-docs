...
Horizon ofrece respuestas como objetos JSON formateados de acuerdo a [HAL](https://en.wikipedia.org/wiki/Hypertext_Application_Language). El formato HAL hace que Horizon sea más explorable, pagina las respuestas y conecta los recursos de los padres e hijos. Consumir este formato es simple usando una de las muchas [librerías de código abierto disponibles](https://github.com/mikekelly/hal_specification/wiki/Libraries) para la mayoría de los principales lenguajes de programación.

HAL es sólo JSON con dos nombres de atributos reservados:

- `_enlaces`
- `_embedded`

Si una respuesta es un único registro, la sección `_links` proporcionará enlaces a cualquier registro padre o hijo, y no habrá propiedad `_embedded`.

Si una respuesta es una colección, la sección `_links` proporcionará [pagination](./pagination/README. dx) enlaces, y la lista de registros de la respuesta se anidará bajo la propiedad `_embedded`.
...
- \_embedded
  - matriz
  - Presentar al consultar un punto final que responda con una colección de registros.
