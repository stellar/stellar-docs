...
Additionally, even some information that is currently available from [Horizon](#todo:horizon) will also require indexing services in the future, as Horizon is [eventually deprecated](#todo:horizon-sunset).
...
### 1. Off-the-shelf APIs for common data

This is the kind of stuff you might get from Horizon today, but includes more than that. For example, this can also include the "every time an NFT changed ownership" kind of data mentioned above.
...
- [Alchemy](#todo:alchemy): most popular choice on Ethereum; now under contract to expand their service to Stellar, with a launch date of Q2 2026. (TODO: is this accurate?)
- [Allium](#todo:allium): currently building Stellar support, launching Q1 2026 (accurate?)
- [Goldsky](#todo:goldsky): mostly focused on indexer-use-case-#2 (see below), but also under contract to provide an API service specifically for Stellar. Planned launch: Q1 2026. (accurate?)
- [OBSRVR](https://www.withobsrvr.com/): a home-grown Stellar-native offering, providing both RPC services and [Obsrvr Gateway](https://www.withobsrvr.com/products/gateway), promising "powerful APIs for real-time data fetching, transaction processing, and easy integration into existing systems." The current maturity of this offering is hard to determine.
- [Horizon](#todo:horizon): as mentioned above, Horizon is nearing [end-of-life](#todo:horizon-sunset), but for now it is the only way to get some of this data. And it's free! And well-known by the Stellar ecosystem.

### 2. Streaming & transforming data to a custom app database
...
- [The Graph](https://thegraph.com/docs/en/about/): one of the earliest and most popular-at-the-time options on Ethereum. They now offer three main products:
  ...
  2. **Token API**, for Indexing Use Case #1 described above
  3. **Substreams**, for Use Case #3 described below

  The Graph offers [early-stage](https://github.com/streamingfast/substreams/issues/693) Stellar support for Substreams, with no current plans to expand Subgraph support to Stellar.
...
- [Goldsky](https://goldsky.com/): one of the currently-most-loved options on Ethereum for this use-case. Goldsky provides [two main products](https://docs.goldsky.com/subgraph-vs-mirror):
  ...
  2. **Mirror**, also called **Pipelines**: a highly efficient tool to Extract, Transform, & Load (ETL) data into your own database. Goldsky [already supports Stellar](https://docs.goldsky.com/chains/stellar), and as of Q4 2025 is working closely with Stellar to improve the shape of their core Stellar datasets to make it easier for app builders to access common data.
...
- [SubQuery](https://subquery.network/): Decentralized Indexer SDK, Decentralized RPCs, & AI Apps. Supports 300+ chains; claims to be 3.9x faster than The Graph. The most direct and aggressive challenger to TheGraph's decentralized model. Differentiates on speed and multi-chain support. Current Stellar support is so-so, with some early adopters leaving for Goldsky due to unoptimized RPC calls running up large bills with RPC providers.

- [OnFinality](https://onfinality.io/): a big player in the Polkadot ecosystem, now expanding to other blockchains. OnFinality provides data _hosting_ services for your SubQuery data. SubQuery powers your the Extract, Transform, and Load (ETL) pipeline; OnFinality is where the data can be loaded _to_.

- [Allium](#todo:allium): in addition to their Portfolio APIs offering, Allium is also under contract with SDF to build out tools for both Indexing Use Case #2 (this one) and Indexing Use Case #3 (see below), with target launch date of Q1 2026. (TODO: accurate?)

### 3. Blockchain-Flavored Big-Data Analytics
