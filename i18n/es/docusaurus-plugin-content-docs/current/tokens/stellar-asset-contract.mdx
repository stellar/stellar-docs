...
El [módulo `token`] del SDK de Rust contiene dos traits, y estructuras cliente correspondientes, para interactuar con SACs.

1. El [trait `TokenInterface`] y la [estructura `TokenClient`] pueden usarse para invocar el "subconjunto básico" de la funcionalidad SAC. Implementan la común [Interfaz Token SEP-41], e incluyen funciones como `transfer`, `burn`, `allowance`, etc.
2. El [trait `StellarAssetInterface`] y la [estructura `StellarAssetClient`] pueden utilizarse para invocar el "subconjunto extendido" de la funcionalidad SAC. Ellos _extienden_ la común interfaz token SEP-41 con funcionalidad administrativa. Además de todas las funciones de `TokenInterface`, estos traits contienen funciones administrativas como `set_admin`, `clawback`, `set_authorized`, etc.
...
[módulo `token`]: https://docs.rs/soroban-sdk/latest/soroban_sdk/token/index.html
[trait `TokenInterface`]: https://docs.rs/soroban-sdk/latest/soroban_sdk/token/trait.TokenInterface.html
[estructura `TokenClient`]: https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.TokenClient.html
[trait `StellarAssetInterface`]: https://docs.rs/soroban-sdk/latest/soroban_sdk/token/trait.StellarAssetInterface.html
[Estructura `StellarAssetClient`]: https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.StellarAssetClient.html
