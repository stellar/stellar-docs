...
El término "custom token" ha sido desaprobado en favor de "contract token". Ve la conversación en el [Stellar Developer Discord](https://discord.com/channels/897514728459468821/966788672164855829/1359276952971640953).

:::

# Contrato de Activos Stellar (SAC)

El Contrato de Activo Stellar (SAC) es una implementación de [CAP-46-6 Activo Estandarizado de Contrato Inteligente](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0046-06.md) y [SEP-41 Interfaz de Token] para los [activos](../learn/fundamentals/stellar-data-structures/assets.mdx) Stellar.
...
El SAC implementa la [Interfaz de Token SEP-41](./token-interface.mdx), que es similar al ampliamente utilizado estándar de token ERC-20. Contracts that depend on only the SEP-41 portion of the SAC's interface, are also compatible with any contract token that implements SEP-41.
...
Se puede desplegar utilizando el [Stellar CLI] como se muestra [aquí](../tools/cli/cookbook/deploy-stellar-asset-contract.mdx).

O el [Stellar SDK] puede usarse como se muestra [aquí](../learn/fundamentals/contract-development/contract-interactions/stellar-transaction.mdx#xdr-usage) llamando a `InvokeHostFunctionOp` con `HOST_FUNCTION_TYPE_CREATE_CONTRACT` y `CONTRACT_ID_FROM_ASSET`. El recurso token tendrá un identificador determinista, que será el hash sha256 de `HashIDPreimage::ENVELOPE_TYPE_CONTRACT_ID_FROM_ASSET` xdr especificado [aquí](https://github.com/stellar/stellar-xdr/blob/dc23adf60e095a6ce626b2b09128e58a5eae0cd0/Stellar-transaction.x#L661).
...
El [módulo `token`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/index.html) del SDK de Rust contiene dos traits, y estructuras cliente correspondientes, para interactuar con SACs.

1. El [trait `TokenInterface`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/trait.TokenInterface.html) y la [estructura `TokenClient`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.TokenClient.html) pueden usarse para invocar el "subconjunto básico" de la funcionalidad SAC. Implementan la común [Interfaz Token SEP-41], e incluyen funciones como `transfer`, `burn`, `allowance`, etc.
2. El [trait `StellarAssetInterface`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/trait.StellarAssetInterface.html) y la [estructura `StellarAssetClient`](https://docs.rs/soroban-sdk/latest/soroban_sdk/token/struct.StellarAssetClient.html) pueden utilizarse para invocar el "subconjunto extendido" de la funcionalidad SAC. Ellos _extienden_ la común interfaz token SEP-41 con funcionalidad administrativa. Además de todas las funciones de `TokenInterface`, estos traits contienen funciones administrativas como `set_admin`, `clawback`, `set_authorized`, etc.
