...
En despliegues multi-inquilino, seleccionar `DISTRIBUTION_ACCOUNT.STELLAR.ENV` hace que el inquilino comparta la misma cuenta de distribución que el host. Esto elimina el aislamiento de fondos por inquilino, así que utilízalo únicamente cuando busques un comportamiento de instancia única.
