...
- `CHANNEL_ACCOUNT_ENCRYPTION_PASSPHRASE` - 🔑 Clave privada ed25519 compatible con Stellar utilizada para cifrar/descifrar las cuentas de canal. Si no se establece, tomará el valor de `DISTRIBUTION_SEED`. Mantén este valor alineado con el secreto heredado al migrar desde 1.x para poder descifrar las cuentas de canal existentes.
- `DISTRIBUTION_ACCOUNT_ENCRYPTION_PASSPHRASE` - 🔑 Clave privada ed25519 compatible con Stellar utilizada para cifrar/descifrar las cuentas de distribución en memoria.
...
- `BROKER_URLS` - Lista de URLs de arranque del broker separadas por comas.
- `CONSUMER_GROUP_ID` - Identificador del grupo de consumidores utilizado por los servicios de la SDP.
- `EVENT_BROKER_TYPE` - Selecciona la estrategia. Opciones: `KAFKA`, `SCHEDULER`, `NONE`. Por defecto: `KAFKA`.
- `KAFKA_SECURITY_PROTOCOL` - Protocolo de seguridad de Kafka. Opciones: `PLAINTEXT`, `SASL_PLAINTEXT`, `SASL_SSL`, `SSL`.
- `KAFKA_SASL_USERNAME` - 🔑 Usuario SASL de Kafka cuando se usan protocolos basados en SASL.
- `KAFKA_SASL_PASSWORD` - 🔑 Contraseña SASL de Kafka cuando se usan protocolos basados en SASL.
- `KAFKA_SSL_ACCESS_KEY` - 🔑 Clave privada del cliente de Kafka (PEM) al usar autenticación mutua TLS.
- `KAFKA_SSL_ACCESS_CERTIFICATE` - 🔑 Certificado del cliente de Kafka (PEM) que corresponde a la clave de acceso.

### Configuración de Multi-tenancy

Utiliza estas variables de entorno para aprovisionar y operar inquilinos a través del servicio Admin:

- `ADMIN_ACCOUNT` - Nombre de usuario para la cabecera de autenticación básica del API Admin.
- `ADMIN_API_KEY` - 🔑 Clave API emparejada con `ADMIN_ACCOUNT` para autenticar el API Admin.
- `ADMIN_PORT` - Puerto expuesto por el servicio Admin. Por defecto: 8003.
- `INSTANCE_NAME` - Nombre mostrado en el `stellar.toml` generado.
- `SINGLE_TENANT_MODE` - Establécelo en `true` para ejecutar una instancia de un solo inquilino (útil para desarrollo local). Al habilitarlo, configura también el inquilino predeterminado mediante [`POST /tenants/default-tenant`](../api-reference/default-tenant.api.mdx).
- `TENANT_XLM_BOOTSTRAP_AMOUNT` - Monto de XLM que la cuenta de distribución del host deposita al inicializar un inquilino.
...
Las siguientes configuraciones están relacionadas con las cuentas de distribución utilizadas para enviar fondos a los destinatarios. Mantén estos valores alineados con los configurados en el Servicio Central de la SDP para que ambos servicios compartan las mismas credenciales del host.
...
### Configuración de Event Broker

Si se utiliza un Event Broker para el procesamiento asíncrono, será necesario configurar el TSS para usarlo.

- `EVENT_BROKER_TYPE` - El tipo de broker de eventos a usar. Opciones: "KAFKA", "SCHEDULER", "NONE". Predeterminado: "KAFKA".
...
Variables de entorno del planificador:

- `SCHEDULER_PAYMENT_JOB_SECONDS` - Intervalo en segundos para el trabajo que sincroniza pagos entre la SDP y el TSS. Mínimo 5s.
- `SCHEDULER_RECEIVER_INVITATION_JOB_SECONDS` - Intervalo en segundos para el trabajo que envía invitaciones a los receptores. Mínimo 5s.

En el lado de Anchor Platform, debemos definir las siguientes variables:

- `SEP10_HOME_DOMAINS="localhost:8000, *.stellar.local:8000"` - Lista separada por comas de dominios home utilizados para [SEP-10]. Debe contener los dominios de las instancias inquilinas de la SDP.
- `SEP10_HOME_DOMAIN=""` - Debe ser una cadena vacía para la versión multi-inquilino.
- `SEP10_WEB_AUTH_DOMAIN=<localhost:8080>` - El dominio home de la instancia de Anchor Platform.
