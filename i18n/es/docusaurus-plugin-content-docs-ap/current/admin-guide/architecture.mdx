...
The Payment Observer monitors the Stellar blockchain using Stellar RPC or Horizon, automatically detects payments related to the business, and updates the corresponding transactions in the Anchor Platform's database. Si también utilizas el servicio de [events], los pagos a tus cuentas desencadenarán un callback HTTP realizado a tu servidor de negocios.
