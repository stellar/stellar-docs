...
Nota que necesitarás crear otro archivo para tu implementación de producción que use la frase de paso de la red pública, las URLs de servicio de producción, tus cuentas de distribución de Mainnet y clave de firma, así como las cuentas de emisión de Mainnet de los activos que utiliza tu servicio.
...
Esto le indicará a la Anchor Platform que debe hospedar el archivo especificado por `SEP1_TOML_VALUE` en `./well-known/stellar.toml`.

Alternativamente, tu archivo `stellar.toml` podría ser hospedado utilizando un servidor de archivos estático apropiado como [nginx]. Mientras tu archivo de información incluya las URLs apropiadas que apuntan a la Anchor Platform, esto funcionará perfectamente.
