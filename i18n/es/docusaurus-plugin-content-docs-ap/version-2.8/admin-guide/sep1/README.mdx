...
Creamos un archivo llamado `dev.stellar.toml` usando el contenido a continuación como punto de partida. Para el conjunto completo de atributos, consulta la [especificación SEP-1][sep-1].
...
Ten en cuenta que necesitarás crear otro archivo para tu implementación en producción que utilice la frase de contraseña de la red pública, las URLs de servicio de producción, tus cuentas de distribución de Mainnet y la clave de firma, así como las cuentas de emisión de Mainnet de los activos que utiliza tu servicio.
...
Esto indicará a la Anchor Platform que debe alojar el archivo especificado por `SEP1_TOML_VALUE` en `./well-known/stellar.toml`.

Alternativamente, tu archivo `stellar.toml` podría ser alojado utilizando un servidor de archivos estáticos adecuado como [nginx]. Siempre que tu archivo de información incluya las URLs apropiadas que apuntan a la Anchor Platform, esto funcionará perfectamente.
