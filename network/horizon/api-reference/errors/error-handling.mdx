...
3. Timeouts: Horizon may respond with a `504` HTTP code. This response is not an error but a warning that your transaction hasn't been accepted by the network yet. There could be many possible reasons for the timeout, the most common of which is network congestion, but it could also be due to other transient issues. There are a couple things you should to do best handle this response.
   ...
   3. **Resubmitting the transaction**: If the transaction status poll has not returned anything or if your transaction has timed out based on the time bounds you specified, you should attempt a resubmission of the same transaction.
