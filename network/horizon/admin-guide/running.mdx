...
The Horizon process [requires access to a Postgres 12 database](./configuring.mdx#initialize-horizon-database). First use the common Kubernetes CLI tool `kubectl` from your workstation to create a Kubernetes secret on the intended namespace of the Kubernetes cluster which will hold the Horizon database URL.
