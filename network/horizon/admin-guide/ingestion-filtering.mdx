...
- If both asset and account filters are as enabled and each filter is provisioned with atleast one whitelist rule, then transactions are stored on database when any single whitelist rule from either filter matches on the given transaction.
