...
- If both asset and account filters are configured as enabled and each filter is provisioned at least one white list rule, then transactions are stored on database when any single white list rule from either filter matches on the given transaction.
