...
1. `/transactions_async`: Horizon submits a transaction to core in an asynchronous manner and passes the relevant Stellar Core response back immediately to the client. It is then the client's responsibility to poll for the status of that transaction.
...
Submissions using the `/transactions_async` endpoint return an immediate response back from core. There are different actions that clients can take based on the specific `tx_status` returned:
