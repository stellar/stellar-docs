...
1. `PENDING`: the submission is successful but the txn is still waiting to be included in the ledger. You should use the `GET /transactions/:hash` endpoint to poll for the submitted txn and check if it makes into the ledger.
