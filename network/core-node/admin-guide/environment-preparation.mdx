...
The configuration fields that control the automatic maintenance behavior are:

- `AUTOMATIC_MAINTENANCE_PERIOD`,
- `AUTOMATIC_MAINTENANCE_COUNT`, and
- `KNOWN_CURSORS`.

By default, stellar-core will perform this automatic maintenance, so be sure to disable it until you have done the appropriate data ingestion in downstream systems.
...
Archive sections can also be configured with `put` and `mkdir` commands to cause the instance to publish to that archive (for nodes configured as [full validators](../README.mdx#full-validators)).
