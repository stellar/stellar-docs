...
When the network time is later than the `upgradetime` specified when the `upgrades` command was used, the validator will vote to update the network to the value specified in that `upgrades` command. If the network time is passed the `upgradetime` by more than 12 hours, the upgrade will be ignored.
...
Changes to network wide settings have to be orchestrated properly between validators as well as non validating nodes. The general process our validators have used to propose and vote on network settings is as follows:
...
This careful orchestration is undertaken to avoid nodes misbehaving. An improper plan may cause issues such as:
...
like the `baseReserve`. The `upgrades` endpoint in stellar-core will require
...
This means that someone wishing to propose a setting upgrade will need to create
a smart contract that writes the `ConfigUpgradeSet` bytes into `ContractData`
(remember, a `Temporory` entry is required), invoke it to write the upgrade xdr,
and then share the serialized `ConfigUpgradeSetKey` to vote on.
