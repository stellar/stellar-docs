...
Use the `NETWORK_PASSPHRASE` field to specify whether your node connects to the [Testnet](/docs/learn/fundamentals/networks) or the Mainnet network.
...
You specify your node's database in the aptly named `DATABASE` field of your config file, which you can can read more about in the [complete example config][complete-example-database]. It defaults to an in-memory database, but you can specify a path as per the example.
...
For the most part, the contents of both the database and buckets directories can be ignored as they are managed by Stellar Core.

## Validating Node

If you want to validate, you must generate a public/private key for your node.
Nodes shouldn't share keys. You should carefully _secure your private key_. If
it is compromised, someone can send false messages to the network and those
...
If you don't include a `NODE_SEED` or set `NODE_IS_VALIDATOR=true`, your node
will still watch SCP and see all the data in the network, but it will not send
validation messages.

If you run multiple validators, make sure to set a common `HOME_DOMAIN` for them by setting the `NODE_HOME_DOMAIN` property to the same value. This will ensure your nodes get grouped correctly during [quorum set generation](#home-domains-array).
You also need to include your other nodes in your config file.
...
following specs laid out in [SEP-20].
...
Here is an example, which creates two `[[HOME_DOMAINS]]` tables:
...
Here is an example, which creates thre `[[VALIDATORS]]` tables:
