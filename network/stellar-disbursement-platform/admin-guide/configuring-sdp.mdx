...
**Notes:**
* Configurations that are tagged with *[secret]* are sensitive and should be stored securely.
...
* `SENTRY_DSN` - *[secret]* The DSN (client key) of the Sentry project. If not provided, Sentry will not be used.
* `ENVIRONMENT` - The environment where the application is running. Example: "development", "staging", "production". Default: "development".
* `DATABASE_URL` - *[secret]* The connection string for the PostgreSQL database. Format is `postgres://username:password@host:port/database?sslmode=disable`. Default: "postgres://localhost:5432/sdp?sslmode=disable".
...
* `TWILIO_ACCOUNT_SID` - *[secret]* The Twilio account SID.
* `TWILIO_AUTH_TOKEN` - *[secret]* The Twilio auth token.
...
* `SEP10_SIGNING_PRIVATE_KEY` - *[secret]* The private key of the Stellar account that signs the SEP-10 transactions. It's also used to sign URLs.
...
* `SEP24_JWT_SECRET` - *[secret]* The secret used to sign the JWT token for SEP-24 transactions. This secret is used during the receiver wallet registration flow.
...
* `EC256_PRIVATE_KEY` - *[secret]* The EC256 Private Key used to sign the authentication token. This EC key needs to be at least as strong as prime256v1 (P-256).
...
* `RECAPTCHA_SITE_SECRET_KEY` - *[secret]* The reCAPTCHA site secret key used to validate reCAPTCHA responses.
...
* `ANCHOR_PLATFORM_OUTGOING_JWT_SECRET` - *[secret]* The JWT secret used to create a JWT token used to send requests to the anchor platform.
...
* `CHANNEL_ACCOUNT_ENCRYPTION_PASSPHRASE` - *[secret]* A Stellar-compliant ed25519 private key used to encrypt/decrypt the channel accounts' private keys. When not set, it will default to the value of the 'DISTRIBUTION_SEED' option.
...
* `DISTRIBUTION_ACCOUNT_ENCRYPTION_PASSPHRASE` - *[secret]* A Stellar-compliant ed25519 private key used to encrypt/decrypt the in-memory distribution accounts' private keys. It's mandatory when the distribution-signer-type is set to "DISTRIBUTION_ACCOUNT_DB".
* `DISTRIBUTION_PUBLIC_KEY` - The public key of the HOST's Stellar distribution account, used to create channel accounts.
* `DISTRIBUTION_SEED` - *[secret]* The private key of the HOST's Stellar distribution account, used to create channel accounts.
...
* `KAFKA_SASL_USERNAME` - *[secret]* Kafka SASL Username.
...
* `KAFKA_SSL_ACCESS_CERTIFICATE` - *[secret]* Kafka SSL Access Certificate in PEM format that matches with the Kafka Access Key.
* `KAFKA_SSL_ACCESS_KEY` - *[secret]* The Kafka Access Key (keystore) in PEM format.
