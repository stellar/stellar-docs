...
MPP was co-developed by Stripe and Tempo and launched last month, and it is live on Stellar. Where XRO-2 handles one payment per request, MPP introduces *sessions*: an agent pre-authorizes a spending limit up front, then streams micro-payments within that session, settling in bulk at the end. If your agent is querying a data feed thousands of times an hour, MPP sessions are the right model — you avoid the overhead of a per-request authorization for every single call.
